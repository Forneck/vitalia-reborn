...
static void init_boards(void)
{
    ...
    for (i = 0; i < NUM_OF_BOARDS; i++) {
        if ((BOARD_RNUM(i) = real_object(BOARD_VNUM(i))) == NOTHING) {
            log1("SYSERR: Fatal board error: board vnum %d does not exist!", BOARD_VNUM(i));
            fatal_error = 1;
        }
        num_of_msgs[i] = 0;
...
SPECIAL(gen_board)
{
    ...
    if (cmd != ACMD_WRITE && cmd != ACMD_LOOK && cmd != ACMD_EXAMINE && cmd != ACMD_READ && cmd != ACMD_REMOVE)
        return (0);
...
int board_write_message(int board_type, struct char_data *ch, char *arg, struct obj_data *board)
{
    ...
    snprintf(buf, sizeof(buf), "%s %-12s :: %s%s%s", tmstr, buf2, COLOR_LEV(ch) >= C_CMP ? color_on : "", arg,
             CCNRM(ch, C_CMP));
