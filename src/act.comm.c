...
ACMD(do_gsay)
{
    ...
    if (!*argument)
        ...
        send_to_group(ch, ch->group, "%s%s%s falou por telepatia com o grupo,'%s'%s\r\n", CCGRN(ch, C_NRM),
                      CCGRN(ch, C_NRM), GET_NAME(ch), argument, CCNRM(ch, C_NRM));
...
static void perform_tell(struct char_data *ch, struct char_data *vict, char *arg)
{
    ...
    snprintf(buf, sizeof(buf), "%s$n falou telepaticamente com você, '%s'%s", CCBLU(vict, C_NRM), arg,
             CCNRM(vict, C_NRM));
    ...
    if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT))
        ...
        snprintf(buf, sizeof(buf), "%sVocê fala telepaticamente com $N, '%s'%s", CCBLU(ch, C_NRM), arg,
                 CCNRM(ch, C_NRM));
...
static int is_tell_ok(struct char_data *ch, struct char_data *vict)
{
    if (!ch)
        ...
        act("$L está escrevendo uma mensagem no momento. Tente novamente mais tarde.", FALSE, ch, 0, vict,
            TO_CHAR | TO_SLEEP);
...
ACMD(do_write)
{
    ...
    if (!*papername) {
        ...
        send_to_char(ch, "Escrever? Com o quê?  Aonde?  O quê você está tentando fazer?!?\r\n");
        return;
    }
...
ACMD(do_gen_comm)
{
    ...
    int hist_type[] = {
        HIST_SHOUT, HIST_GOSSIP, HIST_AUCTION, HIST_GRATS, HIST_PRAY, HIST_HOLLER,
    };
    ...
    const char *com_msgs[][4] = {
        {"Você não pode gritar!\r\n", "grita", "Desligue o noshout antes!\r\n", KYEL},

        {"Você não pode tagarelar!!\r\n", "tagarela", "Você não está neste canal!\r\n", KYEL},

        {"Você não pode leiloar!!\r\n", "leiloa", "Você não está neste canal!\r\n", KMAG},

        {"Você não pode parabenizar!\r\n", "parabeniza", "Você nem está neste canal!\r\n", KGRN},

        {"Você não pode tagarelar as tuas emocoes!\r\n", "tagarela", "Você nem está neste canal!\r\n", KYEL},
    ...
    if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT))
        ...
        snprintf(buf1, sizeof(buf1), "%sVocê %s, '%s%s'%s", COLOR_LEV(ch) >= C_CMP ? color_on : "", com_msgs[subcmd][1],
    ...
    if (!*argument)
        ...
        if (!IS_NPC(ch) && PRF_FLAGGED(ch, PRF_NOREPEAT))
            ...
            if (PLR_FLAGGED(ch, PLR_HERETIC)) {
                send_to_char(ch, "Você tem a nítida impressão de que suas preces não serão ouvidas...\r\n");
                return;
            }
            snprintf(buf1, sizeof(buf1), "%s: <%d> reza, '%s'\r\n", GET_NAME(ch), GET_LEVEL(ch), argument);

            for (d = descriptor_list; d; d = d->next) {
                if (IS_PLAYING(d) && (GET_LEVEL(d->character) >= LVL_GOD) && (!PRF_FLAGGED(d->character, PRF_NOWIZ)) &&
                    (d != ch->desc)) {
                    parse_at(buf1);
