...
{
    char arg[MAX_INPUT_LENGTH];
    protocol_t *pProtocol = ch->desc ? ch->desc->pProtocol : NULL;
    
    if (!ch->desc || !pProtocol) {
        send_to_char(ch, "Este comando só funciona para jogadores conectados.\r\n");
        return;
    }
    ...
    if (!*arg) {
        /* Show current status */
        send_to_char(ch, "Estado da compressão MCCP:\r\n");
        send_to_char(ch, "  MCCP2 suportado: %s\r\n", 
                     pProtocol->bMCCP2 ? "Sim" : "Não");
        send_to_char(ch, "  MCCP3 suportado: %s\r\n", 
                     pProtocol->bMCCP3 ? "Sim" : "Não");
        send_to_char(ch, "  Compressão ativa: %s\r\n", 
                     pProtocol->bCompressing ? "Sim" : "Não");
        send_to_char(ch, "\r\nUse 'mccp on' para ativar ou 'mccp off' para desativar.\r\n");
        return;
    }
    
    if (is_abbrev(arg, "on") || is_abbrev(arg, "ativar") || is_abbrev(arg, "ligado")) {
        if (!pProtocol->bMCCP2 && !pProtocol->bMCCP3) {
            send_to_char(ch, "Seu cliente não suporta compressão MCCP.\r\n");
            return;
        }
        
        if (pProtocol->bCompressing) {
            send_to_char(ch, "A compressão MCCP já está ativa.\r\n");
            return;
        }
        
        if (ProtocolMCCPStart(ch->desc)) {
            send_to_char(ch, "Compressão MCCP ativada.\r\n");
        } else {
            send_to_char(ch, "Não foi possível ativar a compressão MCCP.\r\n");
        }
    }
    else if (is_abbrev(arg, "off") || is_abbrev(arg, "desativar") || is_abbrev(arg, "desligado")) {
        if (!pProtocol->bCompressing) {
            send_to_char(ch, "A compressão MCCP já está desativada.\r\n");
            return;
        }
        
        ProtocolMCCPStop(ch->desc);
        send_to_char(ch, "Compressão MCCP desativada.\r\n");
    }
    else {
        send_to_char(ch, "Uso: mccp [on|off|ativar|desativar]\r\n");
    }
}
