...
static void medit_save_to_disk(zone_vnum foo) { save_mobiles(real_zone(foo)); }
...
void medit_save_internally(struct descriptor_data *d)
{
    ...
    if ((new_rnum = add_mobile(OLC_MOB(d), OLC_NUM(d))) == NOBODY) {
        log1("medit_save_internally: add_mobile failed.");
        return;
    }
...
static void medit_disp_mob_flags(struct descriptor_data *d)
{
    ...
    for (i = 0; i < NUM_MOB_FLAGS; i++) {
        if (medit_illegal_mob_flag(i))
            continue;
        write_to_output(d, "%s%2d%s) %-20.20s  %s", grn, ++count, nrm, action_bits[i], !(++columns % 2) ? "\r\n" : "");
    }
...
static void medit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    cyn, OLC_NUM(d), nrm, grn, nrm, yel, genders[(int)GET_SEX(mob)], nrm, grn, nrm, yel, GET_ALIAS(mob),
                    grn, nrm, yel, GET_SDESC(mob), grn, nrm, yel, GET_LDESC(mob), grn, nrm, yel, GET_DDESC(mob));
    ...
    write_to_output(d,
                    ...
                    attack_hit_text[(int)GET_ATTACK(mob)].singular, grn, nrm, grn, nrm, cyn, flags, grn, nrm, cyn,
                    flag2, grn, nrm, grn, nrm, cyn, OLC_SCRIPT(d) ? "Set." : "Not Set.", grn, nrm, grn, nrm, grn, nrm);
...
static void medit_disp_stats_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    "(%sB%s) Exp Points:  %s[%s%10ld%s]%s  (%sE%s) Alignment: %s[%s%5d%s]%s\r\n"
                    "(%sC%s) Gold:        %s[%s%10d%s]%s\r\n\r\n",
                    cyn, yel, OLC_NUM(d), cyn, nrm, cyn, nrm, cyn, yel, GET_LEVEL(mob), cyn, nrm, cyn, nrm, cyn, nrm,
                    cyn, nrm, cyn, yel, GET_HIT(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_NDD(mob), cyn, nrm, cyn, nrm,
                    cyn, yel, GET_MANA(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_SDD(mob), cyn, nrm, cyn, nrm, cyn, yel,
                    GET_MOVE(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_DAMROLL(mob), cyn, nrm,

                    count_color_chars(buf) + 28, buf, yel, GET_NDD(mob) + GET_DAMROLL(mob), nrm, yel,
                    (GET_NDD(mob) * GET_SDD(mob)) + GET_DAMROLL(mob), nrm,

                    cyn, nrm, cyn, yel, GET_AC(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_HITROLL(mob), cyn, nrm, cyn, nrm,
                    cyn, yel, GET_EXP(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_ALIGNMENT(mob), cyn, nrm, cyn, nrm, cyn,
                    yel, GET_GOLD(mob), cyn, nrm);
    ...
    if (CONFIG_MEDIT_ADVANCED) {
        ...
        write_to_output(d,
                        ...
                        cyn, nrm, cyn, yel, GET_STR(mob), GET_ADD(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_INT(mob), cyn,
                        nrm, cyn, nrm, cyn, yel, GET_SAVE(mob, SAVING_PARA), cyn, nrm, cyn, nrm, cyn, yel, GET_WIS(mob),
                        cyn, nrm, cyn, nrm, cyn, yel, GET_SAVE(mob, SAVING_ROD), cyn, nrm, cyn, nrm, cyn, yel,
                        GET_DEX(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_SAVE(mob, SAVING_PETRI), cyn, nrm, cyn, nrm,
                        cyn, yel, GET_CON(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_SAVE(mob, SAVING_BREATH), cyn, nrm,
                        cyn, nrm, cyn, yel, GET_CHA(mob), cyn, nrm, cyn, nrm, cyn, yel, GET_SAVE(mob, SAVING_SPELL),
                        cyn, nrm);
    }
