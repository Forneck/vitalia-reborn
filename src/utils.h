...
#define CHECK_PLAYER_SPECIAL(ch, var) \
	(*(((ch)->player_specials == &dummy_mob) ? (log1("SYSERR: Mob using '"#var"' at %s:%d.", __FILE__, __LINE__), &(var)) : &(var)))
#else
...
#define FLYING(ch)		(AFF_FLAGGED(ch, AFF_FLYING) || SECT(IN_ROOM(ch)) == SECT_AIR_FLOW || SECT(IN_ROOM(ch)) == SECT_FLYING)

#define SWIMMING(ch) (SECT(IN_ROOM(ch)) == SECT_WATER_SWIM || \
