...
#define CREATE(result, type, number)                                                                                   \
    do {                                                                                                               \
        if ((number) * sizeof(type) <= 0)                                                                              \
            log1("SYSERR: Zero bytes or less requested at %s:%d.", __FILE__, __LINE__);                                \
...
#define DISPOSE(point)                                                                                                 \
    do {                                                                                                               \
        if (!(point)) {                                                                                                \
            log1("SYSERR: Freeing null pointer %s:%d", __FILE__, __LINE__);                                            \
        } else                                                                                                         \
            free(point);                                                                                               \
        point = NULL;                                                                                                  \
    } while (0)
...
#    define CHECK_PLAYER_SPECIAL(ch, var)                                                                              \
        (*(((ch)->player_specials == &dummy_mob)                                                                       \
               ? (log1("SYSERR: Mob using '" #var "' at %s:%d.", __FILE__, __LINE__), &(var))                          \
               : &(var)))
#else
...
#define ZONE_CLIMATE(rnum) (zone_table[(rnum)].climate)
...
#define GET_INCARNATIONS(ch) ((ch)->player.incarnations)
/** Height of ch. */
...
/** Current BREA points of ch. */
#define GET_BREATH(ch) ((ch)->points.breath)
/** Maximum BREA points of ch. */
#define GET_MAX_BREATH(ch) ((ch)->points.max_breath)
/** Gold on ch. */
...
#define GET_DEATH(ch) CHECK_PLAYER_SPECIAL((ch), ((ch)->player_specials->saved.num_death))
...
#define SENDOK(ch)                                                                                                     \
    (((ch)->desc || SCRIPT_CHECK((ch), MTRIG_ACT)) && (to_sleeping || AWAKE(ch)) && !PLR_FLAGGED((ch), PLR_WRITING))
...
#define CAN_SEE(sub, obj)                                                                                              \
    (SELF(sub, obj) || ((GET_REAL_LEVEL(sub) >= (IS_NPC(obj) ? 0 : GET_INVIS_LEV(obj))) && IMM_CAN_SEE(sub, obj)))
/* End of CAN_SEE */
...
#define PERS(ch, vict) (CAN_SEE(vict, ch) ? GET_NAME(ch) : (GET_LEVEL(ch) > LVL_IMMORT ? "um imortal" : "alguem"))

/** If vict can see obj, return obj short description, else return
 * "something". */
#define OBJS(obj, vict) (CAN_SEE_OBJ((vict), (obj)) ? (obj)->short_description : "algo")

/** If vict can see obj, return obj name, else return "something". */
#define OBJN(obj, vict) (CAN_SEE_OBJ((vict), (obj)) ? fname((obj)->name) : "algo")
...
#define CAN_GO(ch, door)                                                                                               \
    (EXIT(ch, door) && (EXIT(ch, door)->to_room != NOWHERE) && !IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED))
...
#define IS_DIAGONAL(dir) (((dir) == NORTHWEST) || ((dir) == NORTHEAST) || ((dir) == SOUTHEAST) || ((dir) == SOUTHWEST))
...
#define IS_BARD(ch) (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_BARD))
#define IS_RANGER(ch) (!IS_NPC(ch) && (GET_CLASS(ch) == CLASS_RANGER))
...
#define FLYING(ch)                                                                                                     \
    (AFF_FLAGGED(ch, AFF_FLYING) || SECT(IN_ROOM(ch)) == SECT_AIR_FLOW || SECT(IN_ROOM(ch)) == SECT_FLYING)

#define SWIMMING(ch) (SECT(IN_ROOM(ch)) == SECT_WATER_SWIM || SECT(IN_ROOM(ch)) == SECT_UNDERWATER)
...
/** Maximum iterations for advanced pathfinding */
...
#define CONFIG_NEWBIE_START config_info.room_nums.newbie_start_room
