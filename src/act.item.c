...
static void get_from_container(struct char_data *ch, struct obj_data *cont, char *arg, int mode,
							   int amount);
...
static void perform_put(struct char_data *ch, struct obj_data *obj, struct obj_data *cont)
{
	...
	if ((GET_OBJ_VAL(cont, 0) > 0) &&
		...
		act("Você não pode se livrar de $p, parece ser uma MALDIÇÃO!", FALSE, ch, obj, NULL,
			TO_CHAR);
	else
	{
		obj_from_char(obj);
      ...
      act("Você não pode tocar em $p!", FALSE, ch, cont, 0, TO_CHAR);
		...
		else
		{
			...
			for (cont = ch->carrying; cont; cont = cont->next_content)
				...
				{
					if ((GET_OBJ_TYPE(cont) == ITEM_CONTAINER) || ((GET_OBJ_TYPE(cont) == ITEM_CORPSE) && GET_OBJ_VAL(cont, 0) == 0))
					{
						found = 1;
  ...
  if (PLR_FLAGGED(ch, PLR_JAILED)) 
	...
	else
	{
		if (mode != SCMD_JUNK)
		{
			...
			{
				send_to_char(ch,
							 "Você joga um punhado de ouro para o ar que desaparece em uma nuvem de fumaça!\r\n");
				act("$n joga um punhado de ouro para o ar que desaparece em uma nuvem de fumaça!!",
		...
		else
		{
			...
			snprintf(buf, sizeof(buf), "$n solta %s  que desaparece em uma nuvem de fumaça!",
					 money_desc(amount));
...
ACMD(do_drop)
{
	...
	case SCMD_DONATE:
		...
		num_don_rooms = (CONFIG_DON_ROOM_1 != NOWHERE) * 2 +
			(CONFIG_DON_ROOM_2 != NOWHERE) + (CONFIG_DON_ROOM_3 != NOWHERE) + 1;
...
static void perform_give(struct char_data *ch, struct char_data *vict, struct obj_data *obj)
{
	...
	else if (IS_CARRYING_N(vict) >= CAN_CARRY_N(vict) && GET_LEVEL(ch) < LVL_IMMORT
...
void name_from_drinkcon(struct obj_data *obj)
{
	...
	{
		log1("SYSERR: Can't remove liquid '%s' from '%s' (%d) item.", liqname, obj->name,
			 obj->item_number);
		/* SYSERR_DESC: From name_from_drinkcon(), this error comes about if
		   the object noted (by keywords and item vnum) does not contain the
		   liquid string being searched for. */
		return;
	}

	liqlen = strlen(liqname);
	CREATE(new_name, char, strlen(obj->name) - strlen(liqname));	/* +1 for
																	   NUL, -1 
																	   for
																	   space */
	...
	for (cur_name = obj->name; cur_name; cur_name = next)
	{
		...
		strncat(new_name, cur_name, cpylen);	/* strncat: OK (size
												   precalculated) */
	}
...
static void wear_message(struct char_data *ch, struct obj_data *obj, int where)
{
	const char *wear_messages[][2] = {
		...
		{"$n desliza $p em seu dedo anelar direito.",
		 "Você desliza $p em seu dedo anelar direito."},

		{"$n desliza $p em seu dedo anelar esquerdo.",
		 "Você desliza $p em seu dedo anelar esquerdo."},

		{"$n veste $p em volta do pescoço.",
		 "Você veste $p em volta do pescoço."},

		{"$n veste $p em volta do pescoço.",
		 "Você veste $p em volta do pescoço."},

		{"$n veste $p no corpo.",
		 "Você veste $p no corpo."},

		{"$n veste $p na cabeça.",
		 "Você veste $p na cabeça."},

		{"$n coloca $p nas pernas.",
		 "Você coloca $p nas pernas."},

		{"$n veste $p nos pés.",
		 "Você veste $p nos pés."},

		{"$n coloca $p nas mãos.",
		 "Você coloca $p nas mãos."},

		{"$n veste $p nos braços.",
		 "Você veste $p nos braços."},

		{"$n prende $p em volta do braço como um escudo.",
		 "Você começa a usar $p como um escudo."},

		{"$n veste $p sobre o corpo.",
		 "Você veste $p sobre o corpo."},

		{"$n veste $p em volta da cintura.",
		 "Você veste $p em volta da cintura."},

		{"$n coloca $p em volta do pulso direito.",
		 "Você coloca $p em volta de seu pulso direito."},

		{"$n coloca $p em volta do pulso esquerdo.",
		 "Você coloca $p em volta de seu pulso esquerdo." },
    ...
    { "$n prende $p às costas, como asas.",
      "Você começa a usar $p como asas." },

    { "$n veste $p em sua orelha direita.",
      "Você veste $p em sua orelha direita." },

    { "$n veste $p em sua orelha esquerda.",
      "Você veste $p em sua orelha esquerda." },

    { "$n coloca $p em seu rosto.",
      "Você coloca $p em seu rosto." },

    { "$n coloca $p em seu nariz.",
...
static void perform_wear(struct char_data *ch, struct obj_data *obj, int where)
{
 ...
 ITEM_WEAR_WIELD,
  ...
  if ((where == WEAR_WIELD) && GET_EQ(ch, WEAR_HOLD) &&
      OBJ_FLAGGED(obj, ITEM_TWO_HANDS)) {
    send_to_char(ch, "Você precisa das duas mãos para usar esta arma.\r\n");
    return;
  } else if ((where == WEAR_HOLD) && GET_EQ(ch, WEAR_WIELD) &&
	     OBJ_FLAGGED(GET_EQ(ch, WEAR_WIELD), ITEM_TWO_HANDS)) {
    send_to_char(ch,"%s", already_wearing[where]);
    return;
  }
