...
static void sedit_rooms_menu(struct descriptor_data *d)
{
    ...
    for (i = 0; S_ROOM(shop, i) != NOWHERE; i++) {
        ...
        write_to_output(d, "%2d - [%s%5d%s] - %s%s%s\r\n", i, cyn, S_ROOM(shop, i), nrm, yel, world[rnum].name, nrm);
    }
    write_to_output(d,
                    "\r\n"
                    "%sA%s) Add a new room.\r\n"
...
static void sedit_namelist_menu(struct descriptor_data *d)
{
    ...
    for (i = 0; S_BUYTYPE(shop, i) != NOTHING; i++) {
        write_to_output(d, "%2d - %s%15s%s - %s%s%s\r\n", i, cyn, item_types[S_BUYTYPE(shop, i)], nrm, yel,
...
static void sedit_shop_flags_menu(struct descriptor_data *d)
{
    ...
    for (i = 0; i < NUM_SHOP_FLAGS; i++) {
        write_to_output(d, "%s%2d%s) %-20.20s   %s", grn, i + 1, nrm, shop_bits[i], !(++count % 2) ? "\r\n" : "");
    }
...
static void sedit_no_trade_menu(struct descriptor_data *d)
{
    ...
    for (i = 0; i < NUM_TRADERS; i++) {
        write_to_output(d, "%s%2d%s) %-20.20s   %s", grn, i + 1, nrm, trade_letters[i], !(++count % 2) ? "\r\n" : "");
    }
...
static void sedit_types_menu(struct descriptor_data *d)
{
    ...
    for (i = 0; i < NUM_ITEM_TYPES; i++) {
        write_to_output(d, "%s%2d%s) %s%-20s%s  %s", grn, i, nrm, cyn, item_types[i], nrm,
                        !(++count % 3) ? "\r\n" : "");
    }
...
static void sedit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    cyn, OLC_NUM(d), nrm, grn, nrm, cyn, S_KEEPER(shop) == NOBODY ? -1 : mob_index[S_KEEPER(shop)].vnum,
                    nrm, yel, S_KEEPER(shop) == NOBODY ? "None" : mob_proto[S_KEEPER(shop)].player.short_descr, grn,
                    nrm, cyn, S_OPEN1(shop), nrm, grn, nrm, cyn, S_CLOSE1(shop), grn, nrm, cyn, S_OPEN2(shop), nrm, grn,
                    nrm, cyn, S_CLOSE2(shop), grn, nrm, cyn, S_BUYPROFIT(shop), nrm, grn, nrm, cyn, S_SELLPROFIT(shop),
                    grn, nrm, yel, S_NOITEM1(shop), grn, nrm, yel, S_NOITEM2(shop), grn, nrm, yel, S_NOCASH1(shop), grn,
                    nrm, yel, S_NOCASH2(shop), grn, nrm, yel, S_NOBUY(shop), grn, nrm, yel, S_BUY(shop), grn, nrm, yel,
                    S_SELL(shop), grn, nrm, cyn, buf1, grn, nrm, cyn, buf2, grn, nrm, grn, nrm, grn, nrm, grn, nrm, grn,
                    nrm);
