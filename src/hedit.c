...
ACMD(do_oasis_hedit)
{
    ...
    mudlog(CMP, MAX(LVL_IMMORT, GET_INVIS_LEV(d->character)), TRUE, "OLC: %s starts editing help files.",
...
static void hedit_save_to_disk(struct descriptor_data *d)
{
    ...
    if (!(fp = fopen(index_name, "w"))) {
        log1("SYSERR: Could not write help index file");
        return;
    }
...
static void hedit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    "%sQ%s) Quit\r\n"
                    "Enter choice : ",
                    nrm, grn, nrm, yel, OLC_HELP(d)->entry, grn, nrm, yel, OLC_HELP(d)->min_level, grn, nrm);
            ...
            switch (*arg) {
                ...
                case 'N':
                    ...
                    if (OLC_ZNUM(d) > top_of_helpt) {
                        write_to_output(d, "Do you wish to add the '%s' help file? ", OLC_STORAGE(d));
                        OLC_MODE(d) = HEDIT_CONFIRM_ADD;
                    } else {
...
ACMD(do_hindex)
{
    ...
    for (i = 0; i < top_of_helpt; i++) {
        if (is_abbrev(argument, help_table[i].keywords) && (GET_LEVEL(ch) >= help_table[i].min_level))
            len += snprintf(buf + len, sizeof(buf) - len, "%-20.20s%s", help_table[i].keywords,
                            (++count % 3 ? "" : "\r\n"));
        else if (strstr(help_table[i].keywords, argument) && (GET_LEVEL(ch) >= help_table[i].min_level))
            len2 += snprintf(buf2 + len2, sizeof(buf2) - len2, "%-20.20s%s", help_table[i].keywords,
                             (++count2 % 3 ? "" : "\r\n"));
    }
    if (count % 3)
