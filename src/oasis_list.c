        ...
        if (IS_SET_AR((mob_proto[num].char_specials.saved.act), mob_flag)) {
            ...
            if ((mob = read_mobile(num, REAL)) != NULL) {
                ...
                len += snprintf(buf + len, sizeof(buf) - len, "%s%3d. %s[%s%5d%s]%s Level %s%-3d%s %s%s\r\n",
                                CCNRM(ch, C_NRM), ++found, CCCYN(ch, C_NRM), CCYEL(ch, C_NRM), GET_MOB_VNUM(mob),
                                CCCYN(ch, C_NRM), CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), GET_LEVEL(mob), CCNRM(ch, C_NRM),
        ...
        if ((mob_proto[num].player.level) == mob_level) {
            if ((mob = read_mobile(num, REAL)) != NULL) {
                ...
                len += snprintf(buf + len, sizeof(buf) - len, "%s%3d. %s[%s%5d%s]%s %s%s\r\n", CCNRM(ch, C_NRM),
                                ++found, CCCYN(ch, C_NRM), CCYEL(ch, C_NRM), GET_MOB_VNUM(mob), CCCYN(ch, C_NRM),
                                CCNRM(ch, C_NRM), GET_NAME(mob), CCNRM(ch, C_NRM));
                extract_char(mob); /* Finished with the mob - remove it from the MUD */
                if (len > sizeof(buf))
                    break;
            }
                ...
                switch (itemtype) {
                    case ITEM_LIGHT:
                        ...
                        if (v1 == -1)
                            tmp_len = snprintf(buf + len, sizeof(buf) - len,
                                               "%s%3d%s) %s[%s%5d%s]%s INFINITE%s %s%s\r\n", QGRN, ++found, QNRM, QCYN,
                                               QYEL, ov, QCYN, QBRED, QCYN, obj_proto[r_num].short_description, QNRM);
                        else
                            tmp_len =
                                snprintf(buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%5d%s]%s (%-3dhrs) %s%s%s\r\n",
                                         QGRN, ++found, QNRM, QCYN, QYEL, ov, QCYN, QNRM, v1, QCYN,
                    ...
                    case ITEM_STAFF:
                        ...
                        tmp_len = snprintf(buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%8d%s]%s (%dx%s) %s%s%s\r\n",
                                           QGRN, ++found, QNRM, QCYN, QYEL, ov, QCYN, QNRM, v1, skill_name(v2), QCYN,
                                           obj_proto[r_num].short_description, QNRM);
                        break;
                    ...
                    case ITEM_FIREWEAPON:
                        ...
                        tmp_len = snprintf(buf + len, sizeof(buf) - len,
                                           "%s%3d%s) %s[%s%8d%s]%s (%d Avg Dam) %s%s%s\r\n", QGRN, ++found, QNRM, QCYN,
                                           QYEL, ov, QCYN, QNRM, v1, QCYN, obj_proto[r_num].short_description, QNRM);
                        break;
                    ...
                    case ITEM_FOUNTAIN:
                        if (v1 != -1)
                            tmp_len =
                                snprintf(buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%8d%s]%s (Max: %d) %s%s%s\r\n",
                                         QGRN, ++found, QNRM, QCYN, QYEL, ov, QCYN, QNRM, v1, QCYN,
                                         obj_proto[r_num].short_description, QNRM);
                        else
                            tmp_len = snprintf(buf + len, sizeof(buf) - len,
                                               "%s%3d%s) %s[%s%8d%s] %sINFINITE%s %s%s\r\n", QGRN, ++found, QNRM, QCYN,
                                               QYEL, ov, QCYN, QBRED, QCYN, obj_proto[r_num].short_description, QNRM);
                        break;
                    ...
                    case ITEM_FOOD:
                        ...
                        if (v2 != 0)
                            tmp_len = snprintf(buf + len, sizeof(buf) - len,
                                               "%s%3d%s) %s[%s%8d%s]%s (%dhrs) %s%s %sPoisoned!%s\r\n", QGRN, ++found,
                                               QNRM, QCYN, QYEL, ov, QCYN, QNRM, v1, QCYN,
                                               obj_proto[r_num].short_description, QBGRN, QNRM);
                        else
                            tmp_len =
                                snprintf(buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%8d%s]%s (%dhrs) %s%s%s\r\n",
                                         QGRN, ++found, QNRM, QCYN, QYEL, ov, QCYN, QNRM, v1, QCYN,
                                         obj_proto[r_num].short_description, QNRM);
                        break;
                    ...
                    case ITEM_MONEY:
                        tmp_len = snprintf(buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%8d%s] %s%s (%s%d coins%s)\r\n",
                                           ...
                                           obj_proto[r_num].short_description, QNRM, QYEL, v1, QNRM);
                        break;
...
static void perform_obj_aff_list(struct char_data *ch, char *arg)
{
    ...
    if (apply <= 0 || apply >= NUM_APPLIES) {
        ...
        for (i = 0; i < MAX_OBJ_LIST; i++) {
            if ((r_num = real_object(lst[i].vobj)) != NOTHING) {
                tmp_len = snprintf(
                    buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%5d%s] %s%3d %s%-*s %s[%s]%s%s\r\n", QGRN, ++found,
                    QNRM, QCYN, QYEL, lst[i].vobj, QCYN, QYEL, lst[i].val, QCYN,
                    42 + count_color_chars(obj_proto[num].short_description), obj_proto[r_num].short_description, QYEL,
                    item_types[obj_proto[num].obj_flags.type_flag], QNRM, obj_proto[num].proto_script ? " [TRIG]" : "");
                len += tmp_len;
                if (len > sizeof(buf))
                    break;
            }
    ...
    for (i = 0; i < MAX_OBJ_LIST; i++) {
        if ((r_num = real_object(lst[i].vobj)) != NOTHING) {
            tmp_len = snprintf(
                buf + len, sizeof(buf) - len, "%s%3d%s) %s[%s%8d%s] %s%3d %s%-*s %s[%s]%s%s\r\n", QGRN, ++found, QNRM,
                QCYN, QYEL, lst[i].vobj, QCYN, QYEL, lst[i].val, QCYN,
                42 + count_color_chars(obj_proto[num].short_description), obj_proto[r_num].short_description, QYEL,
                item_types[obj_proto[r_num].obj_flags.type_flag], QNRM, obj_proto[r_num].proto_script ? " [TRIG]" : "");
            len += tmp_len;
        }
    }
    page_string(ch->desc, buf, TRUE);
    ...
    for (num = 0; num <= top_of_objt; num++) {
        if (is_name(arg, obj_proto[num].name)) {
            ...
            tmp_len = snprintf(buf + len, sizeof(buf) - len, "%s%4d%s) %s[%s%5d%s] %s(%s%3d%s)%s %-*s%s [%s]%s%s\r\n",
                               QGRN, ++found, QNRM, QCYN, QYEL, ov, QCYN, QNRM, QGRN, obj_index[num].number, QNRM, QCYN,
            ...
            if (is_abbrev(arg, "help")) {
                ...
                if (!*arg2) {
                    ...
                    for (i = 0; i < NUM_MOB_FLAGS; i++) {
                        send_to_char(ch, "%s%2d%s-%s%-14s%s", CCNRM(ch, C_NRM), i, CCNRM(ch, C_NRM), CCYEL(ch, C_NRM),
                                     action_bits[i], CCNRM(ch, C_NRM));
                        if (!((i + 1) % 4))
                            send_to_char(ch, "\r\n");
                    }
                    send_to_char(ch, "\r\n");
...
ACMD(do_oasis_links)
{
    ...
    if (!*arg) {
        send_to_char(ch, "Syntax: links <zone_vnum> ('.' for zone you are standing in)\r\n");
        return;
    }
                ...
                if (world[nr].dir_option[j]) {
                    ...
                    if (to_room != NOWHERE && (zrnum != world[to_room].zone))
                        send_to_char(ch, "%3d %-30s%s at %5d (%-5s) ---> %5d\r\n",
                                     zone_table[world[to_room].zone].number, zone_table[world[to_room].zone].name, QNRM,
...
static void list_rooms(struct char_data *ch, zone_rnum rnum, room_vnum vmin, room_vnum vmax)
{
    ...
    for (i = 0; i <= top_of_world; i++) {
        ...
        if ((world[i].number >= bottom) && (world[i].number <= top)) {
            ...
            len += snprintf(buf + len, sizeof(buf) - len, "%4d) [%s%-5d%s] %s%-*s%s %s", counter, QGRN, world[i].number,
                            QNRM, QCYN, count_color_chars(world[i].name) + 44, world[i].name, QNRM,
            ...
            for (j = 0; j < DIR_COUNT; j++) {
                ...
                if (world[W_EXIT(i, j)->to_room].zone != world[i].zone)
                    len += snprintf(buf + len, sizeof(buf) - len, "(%s%d%s)", QYEL, world[W_EXIT(i, j)->to_room].number,
                                    QNRM);
            }
    ...
    for (i = 0; i <= top_of_mobt; i++) {
        if (mob_index[i].vnum >= bottom && mob_index[i].vnum <= top) {
            ...
            len += snprintf(buf + len, sizeof(buf) - len, "%s%4d%s) [%s%-5d%s] %s%-*s %s[%4d]%s%s\r\n", QGRN, counter,
                            QNRM, QGRN, mob_index[i].vnum, QNRM, QCYN,
                            count_color_chars(mob_proto[i].player.short_descr) + 44, mob_proto[i].player.short_descr,
                            QYEL, mob_proto[i].player.level, QNRM, mob_proto[i].proto_script ? " [TRIG]" : "");
            if (len > sizeof(buf))
                break;
        }
    }
...
static void list_objects(struct char_data *ch, zone_rnum rnum, obj_vnum vmin, obj_vnum vmax)
{
    ...
    for (i = 0; i <= top_of_objt; i++) {
        if (obj_index[i].vnum >= bottom && obj_index[i].vnum <= top) {
            ...
            len +=
                snprintf(buf + len, sizeof(buf) - len, "%s%4d%s) [%s%-5d%s] %s%-*s %s[%s]%s%s\r\n", QGRN, counter, QNRM,
                         QGRN, obj_index[i].vnum, QNRM, QCYN, count_color_chars(obj_proto[i].short_description) + 44,
                         obj_proto[i].short_description, QYEL, item_types[obj_proto[i].obj_flags.type_flag], QNRM,
...
static void list_shops(struct char_data *ch, zone_rnum rnum, shop_vnum vmin, shop_vnum vmax)
{
    ...
    for (i = 0; i <= top_shop; i++) {
        if (SHOP_NUM(i) >= bottom && SHOP_NUM(i) <= top) {
            ...
            send_to_char(ch, "%s%4d%s) [%s%-5d%s] [%s%-5d%s]", QGRN, counter, QNRM, QGRN, SHOP_NUM(i), QNRM, QGRN,
                         i + 1, QNRM);

            /* Thanks to Ken Ray for this display fix. -Welcor */
            for (j = 0; SHOP_ROOM(i, j) != NOWHERE; j++)
                send_to_char(ch, "%s%s[%s%-5d%s]%s", ((j > 0) && (j % 6 == 0)) ? "\r\n                      " : " ",
    ...
    for (i = 0; i <= top_of_zone_table; i++) {
        if (zone_table[i].number >= bottom && zone_table[i].number <= top) {
            if ((!use_name) || (is_name(name, zone_table[i].builders))) {
                ...
                tmp_len =
                    snprintf(buf + len, sizeof(buf) - len, "[%s%3d%s] %s%-*s %s%-1s%s\r\n", QGRN, zone_table[i].number,
                             QNRM, QCYN, count_color_chars(zone_table[i].name) + 30, zone_table[i].name, QYEL,
...
void print_zone(struct char_data *ch, zone_vnum vnum)
{
    ...
    send_to_char(ch,
                 ...
                 "%s   Triggers    = %s%d\r\n"
                 "%s   Quests      = %s%d%s\r\n",
                 QGRN, QCYN, zone_table[rnum].number, QGRN, QCYN, zone_table[rnum].name, QGRN, QCYN,
                 zone_table[rnum].builders, QGRN, QCYN, zone_table[rnum].lifespan, QGRN, QCYN, zone_table[rnum].age,
                 QGRN, QCYN, zone_table[rnum].bot, QGRN, QCYN, zone_table[rnum].top, QGRN, QCYN,
                 zone_table[rnum].reset_mode
                     ? ((zone_table[rnum].reset_mode == 1) ? "Reset when no players are in zone." : "Normal reset.")
                     : "Never reset",
                 QGRN, QCYN, buf, QGRN, QCYN, zone_table[rnum].min_level, QGRN, QCYN, zone_table[rnum].max_level, QGRN,
                 QGRN, QCYN, size_rooms, QGRN, QCYN, size_objects, QGRN, QCYN, size_mobiles, QGRN, QCYN, size_shops,
                 QGRN, QCYN, size_trigs, QGRN, QCYN, size_quests, QNRM);
}
...
static void list_triggers(struct char_data *ch, zone_rnum rnum, trig_vnum vmin, trig_vnum vmax)
{
    ...
    for (i = 0; i < top_of_trigt; i++) {
        ...
        if ((trig_index[i]->vnum >= bottom) && (trig_index[i]->vnum <= top)) {
            ...
            send_to_char(ch, "%4d) [%s%5d%s] %s%-45.45s%s ", counter, QGRN, trig_index[i]->vnum, QNRM, QCYN,
