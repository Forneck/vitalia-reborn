...
void tedit_string_cleanup(struct descriptor_data *d, int terminator)
{
    ...
    switch (terminator) {
        ...
        case STRINGADD_ABORT:
            write_to_output(d, "Edição abortada.\r\n");
            act("$n termina de editar alguns pergaminhos.", TRUE, d->character, 0, 0, TO_ROOM);
            break;
        default:
            log1("SYSERR: tedit_string_cleanup: Unknown terminator status.");
            break;
    }
...
ACMD(do_tedit)
{
    ...
    struct {
        ...
        int size;
        char *filename;
    } fields[] = {/* edit the lvls to your own needs */
                  ...
                  {"menu", LVL_IMPL, &menu, 2400, MENU_FILE},
                  ...
                  {"clan-policies", LVL_IMPL, &clanpolicies, 8192, CLANPOL_FILE},
                  {"rebegin", LVL_GRGOD, &rebegin, 2400, REBEGIN_FILE},
    ...
    if (!*field) {
        send_to_char(ch, "Textos disponiveis para serem editados:\r\n");
    ...
    if (*fields[l].cmd == '\n') {
        send_to_char(ch, "Opção inválida do editor de textos.\r\n");
        return;
    }

    if (GET_LEVEL(ch) < fields[l].level) {
        send_to_char(ch, "Você não tem poder suficiente!\r\n");
        return;
    }
    ...
    send_to_char(ch, "Edite o arquivo abaixo:\r\n\r\n");
    ...
    act("$n começa a editar um pergaminho.", TRUE, ch, 0, 0, TO_ROOM);
