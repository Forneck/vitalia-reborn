...
void load_banned(void)
{
    ...
    if (!(fl = fopen(BAN_FILE, "r"))) {
        if (errno != ENOENT) {
            log1("SYSERR: Unable to open banfile '%s': %s", BAN_FILE, strerror(errno));
        } else
            log1("   Ban file '%s' doesn't exist.", BAN_FILE);
        return;
    }
...
ACMD(do_ban)
{
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "%s has banned %s for %s players.", GET_NAME(ch), site,
...
ACMD(do_unban)
{
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "%s removed the %s-player ban on %s.", GET_NAME(ch),
...
void read_invalid_list(void)
{
    ...
    if (num_invalid >= MAX_INVALID_NAMES) {
        log1("SYSERR: Too many invalid names; change MAX_INVALID_NAMES in ban.c");
        exit(1);
    }
