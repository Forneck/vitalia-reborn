...
void run_autowiz(void)
{
	...
	if (CONFIG_USE_AUTOWIZ)
	{
		...
		if (res < sizeof(buf))
		{
			...
			int rval = system(buf);
			if (rval != 0)
				mudlog(BRF, LVL_IMMORT, TRUE, "Warning: autowiz failed with return value %d",
					   rval);
		}
		else
		...
		if (GET_POS(i) >= POS_STUNNED)
		{
			...
			if (AFF_FLAGGED(i, AFF_STINGING))	/* -- mp */
				if (damage(i, i, 5, SPELL_STINGING_SWARM) < 0)
					continue;
			if (!IS_NPC(i) && !ROOM_FLAGGED(IN_ROOM(i), ROOM_FROZEN)
				&& !PLR_FLAGGED(i, PLR_FROZEN))
			{
