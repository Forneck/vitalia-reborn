...
int save_zone(zone_rnum zone_num)
{
...
#endif
        log1("SYSERR: GenOLC: save_zone: Invalid real zone number %d. (0-%d)", zone_num, top_of_zone_table);
        return FALSE;
    }
    ...
    if (flag_tot == 0 && zone_table[zone_num].min_level == -1 && zone_table[zone_num].max_level == -1) {
        ...
        fprintf(zfile,
                ...
                (zone_table[zone_num].builders && *zone_table[zone_num].builders) ? zone_table[zone_num].builders
                                                                                  : "None.",
                (zone_table[zone_num].name && *zone_table[zone_num].name) ? convert_from_tabs(zone_table[zone_num].name)
                                                                          : "undefined",
        ...
        fprintf(zfile,
                ...
                (zone_table[zone_num].name && *zone_table[zone_num].name) ? convert_from_tabs(zone_table[zone_num].name)
                                                                          : "undefined",
                genolc_zone_bottom(zone_num), zone_table[zone_num].top, zone_table[zone_num].lifespan,
                zone_table[zone_num].reset_mode, zbuf1, zbuf2, zbuf3, zbuf4, zone_table[zone_num].min_level,
                zone_table[zone_num].max_level, zone_table[zone_num].climate);
    }
     ...
     * C (Check Mob) Room-Vnum Mob-Vnum Unused
    ...
    for (subcmd = 0; ZCMD(zone_num, subcmd).command != 'S'; subcmd++) {
        switch (ZCMD(zone_num, subcmd).command) {
            ...
            case 'C':
                ...
                arg3 = -1;
                comment = mob_proto[ZCMD(zone_num, subcmd).arg2].player.short_descr;
                break;
            case 'T':
        ...
        if (ZCMD(zone_num, subcmd).command != 'V')
            ...
            fprintf(zfile, "%c %d %d %d %d %s %s\n", ZCMD(zone_num, subcmd).command, ZCMD(zone_num, subcmd).if_flag,
                    arg1, arg2, arg3, ZCMD(zone_num, subcmd).sarg1, ZCMD(zone_num, subcmd).sarg2);
    }
    fputs("S\n$\n", zfile);
    fclose(zfile);
