...
/* Note, most includes for all platforms are in sysdep.h.  The list of files
   that is included is controlled by conf.h for that platform. */
...
/* normally signal SIGUSR2, currently orphaned in favor of emergency unban */
...
/* Appears to be orphaned right now... static RETSIGTYPE unrestrict_game(int
   sig); */
...
int main(int argc, char **argv)
{
    ...
    while ((pos < argc) && (*(argv[pos]) == '-')) {
        switch (*(argv[pos] + 1)) {
            ...
            case 'o':
                ...
                break;
            case 'C': /* -C<socket number> - recover from copyover,
                                 this is the control socket */
                fCopyOver = TRUE;
            ...
            case 's':
                ...
                break;
            case 'h':
                /* From: Anil Mahajan. Do NOT use -C, this is the copyover mode
                   and without the proper copyover.dat file, the game will go
                   nuts! */
                printf(
    ...
    /* Moved here to distinguish command line options and to show up in the
       log if stderr is redirected to a file. */
    log1("Loading configuration.");
    log1("%s", tbamud_version);

    if (chdir(dir) < 0) {
        perror("SYSERR: Fatal error changing to data directory");
        exit(1);
    }
    log1("Using %s as data directory.", dir);

    if (scheck)
        boot_world();
    else {
        log1("Running game on port %d.", port);
        init_game(port);
    }
...
void copyover_recover()
{
    ...
    log1("Copyover recovery initiated");

    fp = fopen(COPYOVER_FILE, "r");
    /* there are some descriptors open which will hang forever then ? */
    if (!fp) {
        perror("copyover_recover:fopen");
        log1("Copyover file not found. Exitting.\n\r");
        exit(1);
    }
    ...
    if (i != 1)
        log1("SYSERR: Error reading boot time.");
    ...
    for (;;) {
        ...
        if (fscanf(fp, "%d %ld %s %s %s\n", &desc, &pref, name, host, guiopt) != 5) {
            if (!feof(fp)) {
                if (ferror(fp))
                    log1("SYSERR: error reading copyover file %s: %s", COPYOVER_FILE, strerror(errno));
                else if (!feof(fp))
...
static void init_game(ush_int local_port)
{
    ...
    log1("Finding player limit.");
    max_players = get_max_players();

    /* If copyover mother_desc is already set up */
    if (!fCopyOver) {
        log1("Opening mother connection.");
    ...
    log1("Normal termination of game.");
}

/* init_socket sets up the mother descriptor - creates the socket, sets its
   options up, binds it, and listens. */
static socket_t init_socket(ush_int local_port)
{
...
#ifdef CIRCLE_WINDOWS
    {
        ...
        if (WSAStartup(wVersionRequested, &wsaData) != 0) {
            log1("SYSERR: WinSock not available!");
            exit(1);
        }

        /* 4 = stdin, stdout, stderr, mother_desc.  Windows might keep sockets
           and files separate, in which case this isn't necessary, but we will
           err on the side of caution. */
        if ((wsaData.iMaxSockets - 4) < max_players) {
            max_players = wsaData.iMaxSockets - 4;
        }
        log1("Max players set to %d", max_players);

        if ((s = socket(PF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
            log1("SYSERR: Error opening network connection: Winsock error #%d", WSAGetLastError());
            exit(1);
        }
    }
#else
    ...
    /* The GUSI sockets library is derived from BSD, so it defines SO_LINGER,
       even though setsockopt() is unimplimented. (from Dean Takemori) */
...
static int get_max_players(void)
{
    ...
    /* First, we'll try using getrlimit/setrlimit.  This will probably work on
       most systems.  HAS_RLIMIT is defined in sysdep.h. */
#    ifdef HAS_RLIMIT
    {
        struct rlimit limit;
...
#    endif
    ...
    if (max_descs <= 0) {
        log1("SYSERR: Non-positive max player limit!  (Set at %d using %s).", max_descs, method);
        exit(1);
    }
    log1("   Setting player limit to %d using %s.", max_descs, method);
    return (max_descs);
#endif /* CIRCLE_UNIX */
}

/* game_loop contains the main loop which drives the entire MUD.  It cycles
   once every 0.10 seconds and is responsible for accepting new new
   connections, polling existing connections for input, dequeueing output and
   sending it out to players, and calling "heartbeat" functions such as
   mobile_activity(). */
void game_loop(socket_t local_mother_desc)
{
    ...
    while (!circle_shutdown) {
        ...
        for (d = descriptor_list; d; d = next_d) {
            ...
            /* Not combined to retain --(d->wait) behavior. -gg 2/20/98 If no
               wait state, no subtraction.  If there is a wait state then 1 is
               subtracted. Therefore we don't go less than 0 ever and don't
               require an 'if' bracket. -gg 2/27/99 */
            if (d->character) {
        ...
        if (missed_pulses <= 0) {
            log1("SYSERR: **BAD** MISSED_PULSES NONPOSITIVE (%d), TIME GOING BACKWARDS!!", missed_pulses);
            missed_pulses = 1;
        }

        /* If we missed more than 30 seconds worth of pulses, just do 30 secs */
        if (missed_pulses > 30 RL_SEC) {
            log1("SYSERR: Missed %d seconds worth of pulses.", missed_pulses / PASSES_PER_SEC);
        ...
        /* Orphaned right now as signal trapping is used for emergency unban */
...
static void record_usage(void)
{
    ...
    log1("nusage: %-3d sockets connected, %-3d sockets playing", sockets_connected, sockets_playing);
...
static char *make_prompt(struct descriptor_data *d)
{
    ...
    if (d->showstr_count)
        snprintf(prompt, sizeof(prompt), "[%d/%d] Pressione ENTER para %s, 'A' para ajuda. > ", d->showstr_page,
        ...
        *prompt = '\0';
        ...
        if (PRF_FLAGGED(d->character, PRF_DISPAUTO) && len < sizeof(prompt)) {
            ...
            if (PRF_FLAGGED(d->character, PRF_DISPHP) && len < sizeof(prompt)) {
                if (GET_HIT(ch) << 2 < GET_MAX_HIT(ch)) {
                    ...
                    count = snprintf(prompt + len, sizeof(prompt) - len, "< %d\tgHp\tn ", GET_HIT(d->character));
                    if (count >= 0)
                        len += count;
                }
            }
            ...
            if (PRF_FLAGGED(d->character, PRF_DISPMANA) && len < sizeof(prompt)) {
                if (GET_MANA(ch) << 2 < GET_MAX_MANA(ch) && len < sizeof(prompt)) {
                    ...
                    count = snprintf(prompt + len, sizeof(prompt) - len, "%d\tgMn\tn ", GET_MANA(d->character));
                    if (count >= 0)
                        len += count;
                }
            }
            ...
            if (PRF_FLAGGED(d->character, PRF_DISPMOVE) && len < sizeof(prompt)) {
                if (GET_MOVE(ch) << 2 < GET_MAX_MOVE(ch) && len < sizeof(prompt)) {
                    ...
                    count = snprintf(prompt + len, sizeof(prompt) - len, "%d\tgMv\tn ", GET_MOVE(d->character));
                    if (count >= 0)
                        len += count;
                }
            }
        ...
        if (PRF_FLAGGED(d->character, PRF_BUILDWALK) && len < sizeof(prompt)) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%s-CONSTRUINDO %s", CBRED(d->character, C_NRM),
        ...
        if (AFF_FLAGGED(d->character, AFF_INVISIBLE) && len < sizeof(prompt)) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%s (invis)%s", CCWHT(d->character, C_NRM),
                             CCNRM(d->character, C_NRM));
            if (count >= 0)
                len += count;
        }
        ...
        if (GET_LAST_NEWS(d->character) < newsmod) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%s(news)%s", CBYEL(d->character, C_NRM),
                             CCNRM(d->character, C_NRM));
            if (count >= 0)
                len += count;
        }
        ...
        if (GET_LAST_MOTD(d->character) < motdmod) {
            count = snprintf(prompt + len, sizeof(prompt) - len, "%s(motd)%s", CCGRN(d->character, C_NRM),
                             CCNRM(d->character, C_NRM));
            if (count >= 0)
                len += count;
        }
...
static int get_from_q(struct txt_q *queue, char *dest, int *aliased)
{
    ...
    strcpy(dest, queue->head->text); /* strcpy: OK (mutual
                                                                        MAX_INPUT_LENGTH) */
...
size_t vwrite_to_output(struct descriptor_data *t, const char *format, va_list args)
{
    ...
    if (size < 0 || wantsize >= sizeof(txt)) {
        size = sizeof(txt) - 1;
        strcpy(txt + size - strlen(text_overflow),
               text_overflow); /* strcpy:
                                                                                          OK */
    }

    /* If the text is too big to fit into even a large buffer, truncate the
       new text to make it fit.  (This will switch to the overflow state
       automatically because t->bufspace will end up 0.) */
    ...
    strcpy(t->large_outbuf->text, t->output); /* strcpy: OK (size checked
                                                                                         previously) */
...
/* socket handling */
/* get_bind_addr: Return a struct in_addr that should be used in our call to
   bind().  If the user has specified a desired binding address, we try to
   bind to it; otherwise, we bind to INADDR_ANY. Note that inet_aton() is
   preferred over inet_addr() so we use it if we can.  If neither is
   available, we always bind to INADDR_ANY. */
static struct in_addr *get_bind_addr()
{
    ...
    if (CONFIG_DFLT_IP == NULL) {
        ...
        if (!parse_ip(CONFIG_DFLT_IP, &bind_addr)) {
            log1("SYSERR: DFLT_IP of %s appears to be an invalid IP address", CONFIG_DFLT_IP);
            bind_addr.s_addr = htonl(INADDR_ANY);
        }
    }
    ...
    if (bind_addr.s_addr == htonl(INADDR_ANY))
        log1("Binding to all IP interfaces on this host.");
    else
        log1("Binding only to IP address %s", inet_ntoa(bind_addr));
...
/* inet_aton's interface is the same as parse_ip's: 0 on failure, non-0 if
   successful. */
static int parse_ip(const char *addr, struct in_addr *inaddr) { return (inet_aton(addr, inaddr)); }
...
{
    log1(
        "SYSERR: warning: you're trying to set DFLT_IP but your system has no "
...
static int new_descriptor(socket_t s)
{
    ...
    if (sockets_connected >= CONFIG_MAX_PLAYING) {
        write_to_descriptor(desc, "Sinto, o VitaliaMUD esta' cheio neste momento... tente novamente mais tarde!\r\n");
        CLOSE_SOCKET(desc);
        return (0);
    }
    /* create a new descriptor */
    ...
    /* find the sitename */
    if (CONFIG_NS_IS_SLOW || !(from = gethostbyaddr((char *)&peer.sin_addr, sizeof(peer.sin_addr), AF_INET))) {
        ...
        strncpy(newd->host, (char *)inet_ntoa(peer.sin_addr),
                HOST_LENGTH); /* strncpy:
                                                                                                 OK
                                                                                                 (n->host:HOST_LENGTH+1)
                                                                                               */
        *(newd->host + HOST_LENGTH) = '\0';
    } else {
        strncpy(newd->host, from->h_name,
                HOST_LENGTH); /* strncpy: OK
                                                                                 (n->host:HOST_LENGTH+1)
                                                                               */
        *(newd->host + HOST_LENGTH) = '\0';
    }
...
/* Send all of the output that we've accumulated for a player out to the
   player's descriptor. 32 byte GARBAGE_SPACE in MAX_SOCK_BUF used for: 2
   bytes: prepended \r\n 14 bytes: overflow message 2 bytes: extra \r\n for
   non-comapct 14 bytes: unused */
static int process_output(struct descriptor_data *t)
{
    ...
    if (!t->pProtocol->WriteOOB)   /* add a prompt */
        strcat(i, make_prompt(t)); /* strcpy: OK (i:MAX_SOCK_BUF reserves
                                                              space) */
    ...
    if (result >= t->bufptr) {
        ...
        *(t->output) = '\0';
        ...
        /* If the overflow message or prompt were partially written, try to
           save them. There will be enough space for them if this is true.
           'result' is effectively unsigned here anyway. */
...
/* perform_socket_write: takes a descriptor, a pointer to text, and a text
   length, and tries once to send that text to the OS.  This is where we stuff
   all the platform-dependent stuff that used to be ugly #ifdef's in
   write_to_descriptor(). This function must return: -1 If a fatal error was
   encountered in writing to the descriptor.  0 If a transient failure was
   encountered (e.g. socket buffer full). >0 To indicate the number of bytes
   successfully written, possibly fewer than the number the caller requested
   be written. Right now there are two versions of this function: one for
   Windows, and one for all other platforms. */
...
ssize_t perform_socket_write(socket_t desc, const char *txt, size_t length)
{
    ...
    if (result == 0) {
        ...
        log1("SYSERR: Huh??  write() returned 0???  Please report this!");
        return (-1);
    }
...
static ssize_t perform_socket_write(socket_t desc, const char *txt, size_t length)
{
    ...
    /* result < 0, so an error was encountered - is it transient?
       Unfortunately, different systems use different constants to indicate
       this. */
...
/* write_to_descriptor takes a descriptor, and text to write to the
   descriptor. It keeps calling the system-level write() until all the text
...
static ssize_t perform_socket_read(socket_t desc, char *read_point, size_t space_left)
{
    ...
    if (ret == 0) {
        log1("WARNING: EOF on socket read (connection broken by peer)");
        return (-1);
    }
...
#    endif
    ...
    /* We don't know what happened, cut them off. This qualifies for a SYSERR
       because we have no idea what happened at this point. */
    perror("SYSERR: perform_socket_read: about to lose connection");
    return (-1);
}

/* ASSUMPTION: There will be no newlines in the raw input buffer when this
   function is called.  We must maintain that before returning. Ever wonder
   why 'tmp' had '+8' on it? The crusty old code could write
   ...
   which wasn't very happy in an unsigned variable.  Argh. So to fix the above,
   'tmp' lost the '+8' since it doesn't need it and the code has been changed
   to reserve space by accepting one less character. (Do you really need 256
   characters on a line?) -gg 1/21/2000 */
static int process_input(struct descriptor_data *t)
{
    ...
    do {
        if (space_left <= 0) {
            log1("WARNING: process_input: about to close connection: input overflow");
            return (-1);
        }

        /* Read # of "bytes_read" from socket, and if we have something, mark
           the sizeof data in the read_buf array as NULL */
        if ((bytes_read = perform_socket_read(t->descriptor, read_buf, space_left)) > 0)
            read_buf[bytes_read] = '\0';

        /* Since we have recieved atleast 1 byte of data from the socket, lets
           run it through ProtocolInput() and rip out anything that is Out Of
           Band */
        if (bytes_read > 0)
        ...
        *(read_point + bytes_read) = '\0'; /* terminate the string */
        ...
        /* on some systems such as AIX, POSIX-standard nonblocking I/O is
           broken, causing the MUD to hang when it encounters input not
           terminated by a newline.  This was causing hangs at the Password:
           prompt, for example. I attempt to compensate by always returning
           after the _first_ read, instead of looping forever until a read
           returns -1.  This simulates non-blocking I/O because the result is
           we never call read unless we know from select() that data is ready
           (process_input is only called if select indicates that this
           descriptor is in the read set).  JE 2/23/95. */
    ...
    while (nl_pos != NULL) {
        ...
        *write_point = '\0';
        ...
        if ((*tmp == '-') && (*(tmp + 1) == '-') && !(*(tmp + 2))) {
            write_to_output(t, "Todos os comandos na fila foram limpos.\r\n");
            flush_queues(t);  /* Flush the command queue */
            failed_subst = 1; /* Allow the read point to be moved, but don't
                                                 add to queue */
        }
...
/* Perform substitution for the '^..^' csh-esque syntax orig is the orig
   string, i.e. the one being modified.  subst contains the substition string,
   i.e. "^telm^tell" */
static int perform_subst(struct descriptor_data *t, char *orig, char *subst)
{
    ...
    if (!(second = strchr(first, '^'))) {
        write_to_output(t, "Substituição inválida.\r\n");
        return (1);
    }
    /* terminate "first" at the position of the '^' and make 'second' point to
    ...
    *(second++) = '\0';
    ...
    strncpy(newsub, orig, strpos - orig); /* strncpy: OK
                                                                                     (newsub:MAX_INPUT_LENGTH+5 >
                                                                                     orig:MAX_INPUT_LENGTH) */
    newsub[strpos - orig] = '\0';

    /* now, the replacement string */
    strncat(newsub, second,
            MAX_INPUT_LENGTH - strlen(newsub) -
                1); /* strncpy:
                                                                                       OK */
...
void close_socket(struct descriptor_data *d)
{
    ...
    if (d->snoop_by) {
        write_to_output(d->snoop_by, "Sua vítima não está mais entre nós.\r\n\r\n");
    ...
    if (d->character) {
        ...
        if (IS_PLAYING(d) || STATE(d) == CON_DISCONNECT) {
            ...
            act("$n perdeu a conexão.", TRUE, link_challenged, 0, 0, TO_ROOM);
...
static void check_idle_passwords(void)
{
    ...
    for (d = descriptor_list; d; d = next_d) {
        ...
        if (!d->idle_tics) {
            ...
            echo_on(d);
            write_to_output(d, "\r\nTempo esgotado... até mais.\r\n");
            STATE(d) = CON_CLOSE;
        }
    }
}

/* I tried to universally convert Circle over to POSIX compliance, but alas,
   some systems are still straggling behind and don't have all the appropriate
   defines.  In particular, NeXT 2.x defines O_NDELAY but not O_NONBLOCK.
   Krusty old NeXT machines! (Thanks to Michael Jones for this and various
   other NeXT fixes.) */
...
#        ifndef MEMORY_DEBUG
    if (!tics_passed) {
        log1("SYSERR: CHECKPOINT shutdown: tics not updated. (Infinite loop suspected)");
        abort();
    } else
        tics_passed = 0;
#        endif
...
/* This is an implementation of signal() using sigaction() for portability.
   ...
   do not restart select(), even if SA_RESTART is used. Note that NeXT 2.x is
   not POSIX and does not have sigaction; therefore, I just define it to be
...
#    ifndef CIRCLE_MACINTOSH
    ...
    /* user signal 2: unrestrict game.  Used for emergencies if you lock
       yourself out of the MUD somehow. (currently orphaned) */
...
void send_to_group(struct char_data *ch, struct group_data *group, const char *msg, ...)
{
    ...
    while ((tch = simple_list(group->members)) != NULL) {
        if (tch != ch && !IS_NPC(tch) && tch->desc && STATE(tch->desc) == CON_PLAYING) {
            write_to_output(tch->desc, "%s[%sGrupo%s]%s ", CCGRN(tch, C_NRM), CBGRN(tch, C_NRM), CCGRN(tch, C_NRM),
                            CCNRM(tch, C_NRM));
            va_start(args, msg);
...
void send_to_range(room_vnum start, room_vnum finish, const char *messg, ...)
{
    ...
    if (start > finish) {
        log1("send_to_range passed start room value greater then finish.");
        return;
    }
    if (messg == NULL)
        return;
        ...
        if (*orig == '$') {
            switch (*(++orig)) {
                ...
                case '$':
                    ...
                    break;
                default:
                    log1("SYSERR: Illegal $-code to act(): %c", *orig);
                    log1("SYSERR: %s", orig);
                    i = "";
                    break;
            }
...
char *act(const char *str, int hide_invisible, struct char_data *ch, struct obj_data *obj, void *vict_obj, int type)
{
    ...
    /* Warning: the following TO_SLEEP code is a hack. I wanted to be able to
       tell act to deliver a message regardless of sleep without adding an
       additional argument.  TO_SLEEP is 128 (a single bit high up).  It's
       ONLY legal to combine TO_SLEEP with one other TO_x command.  It's not
       legal to combine TO_x's with each other otherwise. TO_SLEEP only works
       because its value "happens to be" a single bit; do not change it to
       something else.  In short, it is a hack. */
    ...
    if (type == TO_GMOTE && !IS_NPC(ch)) {
        ...
        for (i = descriptor_list; i; i = i->next) {
            if (!i->connected && i->character && !PRF_FLAGGED(i->character, PRF_NOGOSS) &&
                !PLR_FLAGGED(i->character, PLR_WRITING) && !ROOM_FLAGGED(IN_ROOM(i->character), ROOM_SOUNDPROOF)) {
    ...
    if (ch && IN_ROOM(ch) != NOWHERE)
        ...
        log1("SYSERR: no valid target to act()!");
        return NULL;
    }
    ...
    for (; to; to = to->next_in_room) {
        ...
        if (type != TO_ROOM && to == vict_obj)
            continue;
        if (type == TO_DEAD && ALIVE(to) && !AFF_FLAGGED(to, AFF_TALKDEAD) && GET_LEVEL(to) < LVL_DEMIGOD)
            continue;
...
/* This may not be pretty but it keeps game_loop() neater than if it was
   inline. */
#if defined(CIRCLE_WINDOWS)
void circle_sleep(struct timeval *timeout) { Sleep(timeout->tv_sec * 1000 + timeout->tv_usec / 1000); }
    ...
    for (d = descriptor_list; d; d = d->next) {
        ...
        if (ch && !IS_NPC(ch) && d->connected == CON_PLAYING) {
            ...
            if (d->pProtocol->bGMCP) {
                ...
                sprintf(gmcp_buffer,
                        "{ \"name\": \"%s\", \"level\": %d, \"hp\": %d, \"maxhp\": %d, \"mana\": %d, \"maxmana\": %d, "
                        "\"moves\": %d, \"maxmoves\": %d }",
                        GET_NAME(ch), GET_LEVEL(ch), GET_HIT(ch), GET_MAX_HIT(ch), GET_MANA(ch), GET_MAX_MANA(ch),
                        GET_MOVE(ch), GET_MAX_MOVE(ch));
                GMCPSendData(d, "Char.Vitals", gmcp_buffer);

                /* Send character status */
                sprintf(gmcp_buffer,
                        "{ \"class\": \"%s\", \"race\": \"%s\", \"alignment\": %d, \"hunger\": %d, \"thirst\": %d }",
                        pc_class_types[ch->player.chclass], "Human", GET_ALIGNMENT(ch), GET_COND(ch, HUNGER),
                        GET_COND(ch, THIRST));
