...
/* normally signal SIGUSR2, currently orphaned in favor of Webster dictionary
   lookup static byte emergency_unban; */
...
void game_loop(socket_t local_mother_desc)
{
	...
	while (!circle_shutdown)
	{
			...
			{					/* else: we're playing normally. */
				if (aliased)	/* To prevent recursive aliases. */
					d->has_prompt = TRUE;	/* To get newline before next cmd
											   output. */
				else if (perform_alias(d, comm, sizeof(comm)))	/* Run it
																   through
																   aliasing
																   system */
		...
		/* Orphaned right now as signal trapping is used for Webster lookup if 
		   (emergency_unban) { emergency_unban = FALSE; mudlog(BRF,
		   LVL_IMMORT, TRUE, "Received SIGUSR2 - completely unrestricting game 
		   (emergent)"); ban_list = NULL; circle_restrict = 0; num_invalid =
		   0; } */
...
static char *make_prompt(struct descriptor_data *d)
{
	...
	else if (STATE(d) == CON_PLAYING && !IS_NPC(d->character))
	{
     ...
     count =  snprintf(prompt + len, sizeof(prompt) - len, "%s: %s\r\n", PERS(FIGHTING(d->character), d->character), gauge(0, 0, MAX(GET_HIT(FIGHTING(d->character)), 0), GET_MAX_HIT(FIGHTING(d->character)))); 
     	if (count >= 0)
					len += count;
     }
      else {
    count = snprintf(prompt + len, sizeof(prompt) - len, "%s", CCNRM(d->character, C_NRM));
		...
		{
			count =
				snprintf(prompt + len, sizeof(prompt) - len, "i%d ", GET_INVIS_LEV(d->character));
			if (count >= 0)
				len += count;
		}
...
size_t vwrite_to_output(struct descriptor_data * t, const char *format, va_list args)
{
	...
	strcpy(txt, ProtocolOutput(t, txt, (int *)&wantsize));	/* <--- Add this
															   line */
	...
	{
		size = sizeof(txt) - 1;
		strcpy(txt + size - strlen(text_overflow), text_overflow);	/* strcpy: 
																	   OK */
	}
...
static int new_descriptor(socket_t s)
{
	...
	{
		write_to_descriptor(desc,
							"Sinto, o VitaliaMUD esta' cheio neste momento... tente novamente mais tarde!\r\n");
		CLOSE_SOCKET(desc);
		return (0);
	}
	/* create a new descriptor */
		...
		!(from = gethostbyaddr((char *)&peer.sin_addr, sizeof(peer.sin_addr), AF_INET)))
	{
		...
		strncpy(newd->host, (char *)inet_ntoa(peer.sin_addr), HOST_LENGTH);	/* strncpy: 
																			   OK 
	...
	{
		strncpy(newd->host, from->h_name, HOST_LENGTH);	/* strncpy: OK
														   (n->host:HOST_LENGTH+1) 
														 */
		*(newd->host + HOST_LENGTH) = '\0';
	}
...
static int perform_subst(struct descriptor_data *t, char *orig, char *subst)
{
	...
	*(second++) = '\0';
	...
	strncat(newsub, second, MAX_INPUT_LENGTH - strlen(newsub) - 1);	/* strncpy: 
																	   OK */

	/* now, if there's anything left in the original after the string to
	   replaced, copy that too. */
	if (((strpos - orig) + strlen(first)) < strlen(orig))
		strncat(newsub, strpos + strlen(first), MAX_INPUT_LENGTH - strlen(newsub) - 1);	/* strncpy: 
																						   OK 
																						 */
...
void close_socket(struct descriptor_data *d)
{
	...
	if (d->character)
	{
		...
		if (IS_PLAYING(d) || STATE(d) == CON_DISCONNECT)
		{
			...
			mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(link_challenged)), TRUE,
...
void send_to_group(struct char_data *ch, struct group_data *group, const char *msg, ...)
{
	...
	while ((tch = simple_list(group->members)) != NULL)
	{
		...
		{
			write_to_output(tch->desc, "%s[%sGrupo%s]%s ",
							CCGRN(tch, C_NRM), CBGRN(tch, C_NRM), CCGRN(tch, C_NRM), CCNRM(tch,
																						   C_NRM));
			va_start(args, msg);
	...
	if (type == TO_GMOTE && !IS_NPC(ch))
	{
		...
		for (i = descriptor_list; i; i = i->next)
		{
				...
				!ROOM_FLAGGED(IN_ROOM(i->character), ROOM_SOUNDPROOF))
			{
				...
				sprintf(buf, "%s%s%s", CCYEL(i->character, C_NRM), str,
...
static void handle_webster_file(void)
{
	...
	if (!fl)
	{
		send_to_char(ch, "Parece que o dicionario está offline...\r\n");
		return;
	}
	...
	{
		const char *overflow = "\r\n**OVERFLOW**\r\n";
		strcpy(retval + sizeof(retval) - strlen(overflow) - 1, overflow);	/* strcpy: 
																			   OK 
																			 */
	}
	fclose(fl);

	send_to_char(ch, "Você obteve esta resposta do Merriam-Webster:\r\n");
