...
/* Saving throws for classes, scaled for levels 0-100.
 * Uses a formula-based approach: saves decrease linearly from the starting
 * value at level 1 to 1 at level 100. Level 0 always returns 90.
 *
 * The starting values (at level 1) for each class/type are based on the
 * original CircleMUD/tbaMUD values, preserving class balance:
 * - Magic Users: best vs rods/spells, worst vs physical
 * - Clerics: balanced, good vs spells
 * - Thieves: best vs physical, average vs magic
 * - Warriors: worst vs magic, good vs physical
 * - Rangers: similar to thieves
 * - Druids: similar to clerics
 * - Bards: balanced across all types
 */

/* Helper function to calculate saving throw using linear interpolation.
 * Returns: 90 at level 0, start_value at level 1, decreasing to 1 at level 100.
 */
static byte calc_save(int level, int start_value)
{
    if (level <= 0)
        return 90;
    if (level >= 100)
...
byte saving_throws(int class_num, int type, int level)
{
    ...
    /* Validate saving throw type */
    if (type < SAVING_PARA || type > SAVING_SPELL) {
        log1("SYSERR: Invalid saving throw type.");
        return 100;
    }

    return calc_save(level, save_starts[class_num][type]);
}
