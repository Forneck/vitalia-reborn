...
static struct map_info_type compact_door_info[] = {
    {DOOR_NONE, " "},          {DOOR_DOWN_AND_SE, "\tR\\\tn"}, {DOOR_UP_AND_NE, "\tR/\tn"}, {VDOOR_DIAGNW, "\tm+\tn"},
    {VDOOR_DIAGNE, "\tm+\tn"}, {VDOOR_EW, " \tm+\tn "},        {VDOOR_NS, " \tm+\tn "},     {DOOR_DIAGNW, "\\"},
    {DOOR_DIAGNE, "/"},        {DOOR_DOWN, "\tr-\tn"},         {DOOR_UP, "\tr+\tn"},        {DOOR_EW, "-"},
    {DOOR_NS, " | "}};
 ...
 *****************************************************************************/
static void MapArea(room_rnum room, struct char_data *ch, int x, int y, int min, int max, sh_int xpos, sh_int ypos,
                    bool worldmap);
                    ...
                    bool worldmap)
{
    ...
    for (door = 0; door < MAX_MAP_DIR; door++) {
        ...
        if (door < MAX_MAP_FOLLOW && xpos + door_offsets[door][0] >= 0 && xpos + door_offsets[door][0] <= ns_size &&
            ypos + door_offsets[door][1] >= 0 && ypos + door_offsets[door][1] <= ew_size) { /* Virtual exit */
        ...
        if ((pexit = world[room].dir_option[door]) != NULL && (pexit->to_room > 0) && (pexit->to_room != NOWHERE) &&
...
static char *CompactStringMap(int centre, int size)
{
    ...
    for (x = centre - size; x <= centre + size; x++) {
        ...
        for (y = centre - size; y <= centre + size; y++) {
            ...
            mp +=
                strlen((map[x][y] < 0) ? compact_door_info[NUM_DOOR_TYPES + map[x][y]].disp : map_info[map[x][y]].disp);
        }
        strcpy(mp, "\r\n");
        mp += 2;
    }
...
static void perform_map(struct char_data *ch, char *argument, bool worldmap)
{
    ...
    char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH], buf1[MAX_STRING_LENGTH],
    ...
    if (*arg2) {
        if (is_abbrev(arg2, "normal"))
            worldmap = FALSE;
        else if (is_abbrev(arg2, "mundi"))
            worldmap = TRUE;
        else {
            send_to_char(ch, "Uso: \tymap <distance> [ normal | mundi ]\tn");
            return;
        }
    }
    ...
    count += sprintf(buf + count, "\tn%s Mergulhando\\\\", map_info[SECT_UNDERWATER].disp);
    count += sprintf(buf + count, "\tn%s Escalando\\\\", map_info[SECT_CLIMBING].disp);
    count += sprintf(buf + count, "\tn%s Corrente Ar\\\\", map_info[SECT_AIR_FLOW].disp);
...
{
    if (!can_see_map(ch)) {
        send_to_char(ch, "O mapa está desativado!\r\n");
        return;
    }
