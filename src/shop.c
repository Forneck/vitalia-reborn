...
const char *trade_letters[] = {"Good",                            /* First, the alignment based ones */
                               ...
                               "Cleric", "Thief",   "Warrior",    "Ranger", "Druid", "Bard", "\n"};
...
static char *list_object(struct obj_data *obj, int cnt, int oindex, int shop_nr, struct char_data *keeper,
...
static struct obj_data *get_selling_obj(struct char_data *ch, char *name, struct char_data *keeper, int shop_nr,
                                        int msg);
...
static struct obj_data *get_purchase_obj(struct char_data *ch, char *arg, struct char_data *keeper, int shop_nr,
                                         int msg);
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
    ...
    if ((IS_MAGIC_USER(ch) && NOTRADE_MAGIC_USER(shop_nr)) || (IS_CLERIC(ch) && NOTRADE_CLERIC(shop_nr)) ||
        ...
        (IS_RANGER(ch) && NOTRADE_RANGER(shop_nr)) || (IS_DRUID(ch) && NOTRADE_DRUID(shop_nr)) ||
...
static void push(struct stack_data *stack, int pushval) { S_DATA(stack, S_LEN(stack)++) = pushval; }
...
void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if ((buynum = transaction_amt(arg)) < 0) {
        ...
        snprintf(buf, sizeof(buf), "%s Um valor negativo? Tente me vender algo.", GET_NAME(ch));
        do_tell(keeper, buf, cmd_tell, 0);
        return;
    }
    if (!*arg || !buynum) {
    ...
    if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
        if (GET_OBJ_COST(obj) > GET_QUESTPOINTS(ch) && !IS_GOD(ch)) {
            ...
            snprintf(actbuf, sizeof(actbuf), "%s Voce nao tem pontos de aventura suficientes.", GET_NAME(ch));
        ...
        if (buy_price(obj, shop_nr, keeper, ch) > GET_GOLD(ch) && !IS_GOD(ch)) {
            ...
            switch (SHOP_BROKE_TEMPER(shop_nr)) {
                ...
                case 1:
                    do_echo(
                        keeper, strcpy(actbuf, "fuma um cigarro."), cmd_emote,
                        SCMD_EMOTE); /* strcpy:
                                                                                                                        OK
                                                                                                                      */
                    return;
    ...
    if (OBJ_FLAGGED(obj, ITEM_QUEST)) {
        while (obj && (GET_QUESTPOINTS(ch) >= GET_OBJ_COST(obj) || IS_GOD(ch)) && IS_CARRYING_N(ch) < CAN_CARRY_N(ch) &&
    ...
    if (bought < buynum) {
        ...
        if (!obj || !same_obj(last_obj, obj))
        ...
        else if (!OBJ_FLAGGED(obj, ITEM_QUEST) && GET_GOLD(ch) < buy_price(obj, shop_nr, keeper, ch))
            ...
            snprintf(buf, sizeof(buf), "%s Voce so' tem pontos de aventura para %d.", GET_NAME(ch), bought);
                                        ...
                                        int msg)
{
    ...
    switch (result) {
        case OBJECT_NOVAL:
            snprintf(buf, sizeof(buf), "%s Voce deve estar brincando! Isto nao vale nada!", GET_NAME(ch));
            break;
        case OBJECT_NOTOK:
        ...
        case OBJECT_DEAD:
            ...
            break;
        default:
            log1("SYSERR: Illegal return value of %d from trade_with() (%s)", result,
                 __FILE__); /* Someone
                                                                                                               might
                                                                                                               rename
                                                                                                               it...
...
static void shopping_value(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    if (!IS_SET(SHOP_BITVECTOR(shop_nr), HAS_UNLIMITED_CASH) && GET_GOLD(keeper) + SHOP_BANK(shop_nr) < price) {
...
static char *list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr, struct char_data *keeper,
                         struct char_data *ch)
{
    ...
    switch (GET_OBJ_TYPE(obj)) {
        case ITEM_DRINKCON:
            if (GET_OBJ_VAL(obj, 1))
                snprintf(itemname, sizeof(itemname), "%s de %s", obj->short_description, drinks[GET_OBJ_VAL(obj, 2)]);
            else
    ...
    snprintf(result, sizeof(result), " %2d)  %9s   %-*s %6d%s\r\n", aindex, quantity, count_color_chars(itemname) + 48,
             itemname, buy_price(obj, shop_nr, keeper, ch), OBJ_FLAGGED(obj, ITEM_QUEST) ? " qp" : "");
            ...
            if (CAN_SEE_OBJ(ch, obj) && GET_OBJ_COST(obj) > 0) {
                if (!last_obj) {
                    ...
                    if (!*name || isname(name, last_obj->name)) {
                        strncat(
                            buf, list_object(last_obj, cnt, lindex, shop_nr, keeper, ch),
                            sizeof(buf) - len -
                                1); /* strncat:
                                                                                                                                       OK
                                                                                                                                     */
...
static int add_to_shop_list(struct shop_buy_data *list, int type, int *len, int *val)
{
    if (*val != NOTHING && *val >= 0) { /* necessary after changing to unsigned
                                           v/rnums -- Welcor */
        if (*len < MAX_SHOP_OBJ) {
...
static char *read_shop_message(int mnum, room_vnum shr, FILE *shop_f, const char *why)
{
    ...
    for (cht = 0; tbuf[cht]; cht++) {
        ...
        if (tbuf[cht + 1] == 's')
            ...
            log1("SYSERR: Shop #%d has invalid format '%%%c' in message #%d.", shr, tbuf[cht + 1], mnum);
            err++;
        }
    }

    if (ss > 1 || ds > 1) {
        log1("SYSERR: Shop #%d has too many specifiers for message #%d. %%s=%d %%d=%d", shr, mnum, ss, ds);
        err++;
    }
    ...
    *buf = '\0';
    for (shop_nr = 0; shop_nr <= top_shop && len < sizeof(buf); shop_nr++) {
        ...
        if (SHOP_KEEPER(shop_nr) == NOBODY)
            ...
            sprintf(
                buf1, "%6d",
                mob_index[SHOP_KEEPER(shop_nr)].vnum); /* sprintf:
                                                                                                                  OK
                                                                                                                  (for
                                                                                                                  'buf1
                                                                                                                  >=
...
static void list_detailed_shop(struct char_data *ch, int shop_nr)
{
    ...
    for (sindex = 0; SHOP_PRODUCT(shop_nr, sindex) != NOTHING; sindex++) {
        ...
        linelen = snprintf(buf1, sizeof(buf1), "%s (#%d)", obj_proto[SHOP_PRODUCT(shop_nr, sindex)].short_description,
    ...
    for (sindex = 0; SHOP_BUYTYPE(shop_nr, sindex) != NOTHING; sindex++) {
        ...
        linelen = snprintf(buf1, sizeof(buf1), "%s (#%d) [%s]", item_types[SHOP_BUYTYPE(shop_nr, sindex)],
...
bool shopping_identify(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
    ...
    send_to_char(ch, "Peso: %d, Custo para vender: %s%d%s, Custo para comprar: %s%d%s\r\n", GET_OBJ_WEIGHT(obj), QYEL,
                 sell_price(obj, shop_nr, keeper, ch), QNRM, QYEL, buy_price(obj, shop_nr, keeper, ch), QNRM);
