...
static char *list_object(struct obj_data *obj, int cnt, int oindex, int shop_nr,
						 struct char_data *keeper, struct char_data *seller);
...
static int buy_price(struct obj_data *obj, int shop_nr, struct char_data *keeper,
...
static int is_ok_char(struct char_data *keeper, struct char_data *ch, int shop_nr)
{
	...
	if ((IS_GOOD(ch) && NOTRADE_GOOD(shop_nr)) ||
		(IS_EVIL(ch) && NOTRADE_EVIL(shop_nr)) || (IS_NEUTRAL(ch) && NOTRADE_NEUTRAL(shop_nr)))
	{
	...
	if ((IS_MAGIC_USER(ch) && NOTRADE_MAGIC_USER(shop_nr)) ||
		...
		(IS_WARRIOR(ch) && NOTRADE_WARRIOR(shop_nr)) || (IS_RANGER(ch) && NOTRADE_RANGER(shop_nr)) ||
...
static int pop(struct stack_data *stack)
{
	...
	{
		log1("SYSERR: Illegal expression %d in shop keyword list.", S_LEN(stack));
		return (0);
	}
}
...
static int evaluate_expression(struct obj_data *obj, char *expr)
{
		...
		else
		{
			if ((temp = find_oper_num(*ptr)) == NOTHING)
			{
				...
				while (*ptr && !isspace(*ptr) && find_oper_num(*ptr) == NOTHING)
					ptr++;
				strncpy(name, end, ptr - end);	/* strncpy: OK
												   (name/end:MAX_STRING_LENGTH) 
												 */
				name[ptr - end] = '\0';
			...
			else
			{
				...
				if (temp == OPER_CLOSE_PAREN)
				{
					...
					{
						log1("SYSERR: Illegal parenthesis in shop keyword expression.");
						return (FALSE);
					}
				}
				else
	...
	if (top(&vals) != -1)
	{
		log1("SYSERR: Extra operands left on shop keyword expression stack.");
		return (FALSE);
	}
	return (temp);
}
...
static int transaction_amt(char *arg)
{
	...
	/* If we have two arguments, it means 'buy 5 3', or buy 5 of #3. We don't
	   do that if we only have one argument, like 'buy 5', buy #5. By Andrey
	   Fidrya */
...
/* Shop purchase adjustment, based on charisma-difference from buyer to
   keeper. for i in `seq 15 -15`; do printf " * %3d: %6.4f\n" $i \ `echo
   "scale=4; 1+$i/70" | bc`; done Shopkeeper higher charisma (markup) ^ 15:
   1.2142 14: 1.2000 13: 1.1857 12: 1.1714 11: 1.1571 | 10: 1.1428 9: 1.1285
   8: 1.1142 7: 1.1000 6: 1.0857 | 5: 1.0714 4: 1.0571 3: 1.0428 2: 1.0285 1:
   1.0142 + 0: 1.0000 | -1: 0.9858 -2: 0.9715 -3: 0.9572 -4: 0.9429 -5: 0.9286
   | -6: 0.9143 -7: 0.9000 -8: 0.8858 -9: 0.8715 -10: 0.8572 v -11: 0.8429 -12: 
   0.8286 -13: 0.8143 -14: 0.8000 -15: 0.7858 Player higher charisma (discount)
   Most mobiles have 11 charisma so an 18 charisma player would get a 10%
   discount beyond the basic price.  That assumes they put a lot of points into
...
static int sell_price(struct obj_data *obj, int shop_nr, struct char_data *keeper,
					  struct char_data *seller)
{
	float sell_cost_modifier =
		SHOP_SELLPROFIT(shop_nr) * (1 - (GET_CHA(keeper) - GET_CHA(seller)) / 70.0);
	float buy_cost_modifier =
		SHOP_BUYPROFIT(shop_nr) * (1 + (GET_CHA(keeper) - GET_CHA(seller)) / 70.0);
...
static void shopping_buy(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
	...
	if ((buynum = transaction_amt(arg)) < 0)
	{
		...
		snprintf(buf, sizeof(buf), "%s Um valor negativo? Tente me vender algo.",
				 GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
		return;
	}
	if (!*arg || !buynum)
	{
		...
		snprintf(buf, sizeof(buf), "%s Oque voce quer comprar??", GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
		return;
	}
	...
	if (OBJ_FLAGGED(obj, ITEM_QUEST))
	{
		if (GET_OBJ_COST(obj) > GET_QUESTPOINTS(ch) && !IS_GOD(ch))
		{
			...
			snprintf(actbuf, sizeof(actbuf),
					 "%s Voce nao tem pontos de aventura suficientes.", GET_NAME(ch));
	...
	{							/* has the player got enough gold? */
		if (buy_price(obj, shop_nr, keeper, ch) > GET_GOLD(ch) && !IS_GOD(ch))
		{
			...
			{
			case 0:
				do_action(keeper, strcpy(actbuf, GET_NAME(ch)), cmd_puke, 0);	/* strcpy: 
																				   OK 
																				   (MAX_NAME_LENGTH 
																				   < 
																				   MAX_INPUT_LENGTH) 
				...
				return;
			case 1:
				do_echo(keeper, strcpy(actbuf, "fuma um cigarro."), cmd_emote, SCMD_EMOTE);	/* strcpy: 
																								   OK 
																								 */
				return;
			default:
	...
	if (IS_NPC(ch) || (!IS_NPC(ch) && !PRF_FLAGGED(ch, PRF_NOHASSLE)))
	{
		...
		{
			send_to_char(ch, "%s: Voce nao consegue carregar mais.\r\n", fname(obj->name));
			return;
		}
		if (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch))
		{
			send_to_char(ch, "%s: Voce nao consegue carregar tanto peso.\r\n", fname(obj->name));
			return;
		}
	}
	...
	if (bought < buynum)
	{
		...
		if (!obj || !same_obj(last_obj, obj))
			snprintf(buf, sizeof(buf), "%s Eu so' tenho %d para te vender.", GET_NAME(ch), bought);
		else if (!OBJ_FLAGGED(obj, ITEM_QUEST) &&
				 GET_GOLD(ch) < buy_price(obj, shop_nr, keeper, ch))
			snprintf(buf, sizeof(buf), "%s Voce so' consegue pagar por %d.", GET_NAME(ch), bought);
		else if (OBJ_FLAGGED(obj, ITEM_QUEST) && GET_QUESTPOINTS(ch) < GET_OBJ_COST(obj))
			snprintf(buf, sizeof(buf), "%s Voce so' tem pontos de aventura para %d.",
					 GET_NAME(ch), bought);
		else if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
			snprintf(buf, sizeof(buf), "%s Voce so' consegue segurar %d.", GET_NAME(ch), bought);
		else if (IS_CARRYING_W(ch) + GET_OBJ_WEIGHT(obj) > CAN_CARRY_W(ch))
			snprintf(buf, sizeof(buf), "%s Voce so' consegue carregar %d.", GET_NAME(ch), bought);
		else
			snprintf(buf, sizeof(buf), "%s Algo deu errado e voce so' recebeu %d.", GET_NAME(ch),
					 bought);
		do_tell(keeper, buf, cmd_tell, 0);
	}
	...
	snprintf(tempbuf, sizeof(tempbuf), "$n compra %s.", tempstr);
	act(tempbuf, FALSE, ch, obj, 0, TO_ROOM);

	if (obj && OBJ_FLAGGED(obj, ITEM_QUEST))
		snprintf(tempbuf, sizeof(tempbuf), "%s Isto custou %d pontos de aventura.", GET_NAME(ch),
				 goldamt);
	else
										...
										int shop_nr, int msg)
{
	...
	{
	case OBJECT_NOVAL:
		snprintf(buf, sizeof(buf), "%s Voce deve estar brincando! Isto nao vale nada!",
				 GET_NAME(ch));
		break;
	case OBJECT_NOTOK:
	...
	case OBJECT_DEAD:
		...
		break;
	default:
		log1("SYSERR: Illegal return value of %d from trade_with() (%s)", result, __FILE__);	/* Someone 
																							   might 
																							   rename 
																							   it... 
																							 */
		snprintf(buf, sizeof(buf), "%s Algo deu errado.", GET_NAME(ch));
		break;
	}
	do_tell(keeper, buf, cmd_tell, 0);
	return (NULL);
...
/* This function is a slight hack! To make sure that duplicate items are only
   listed once on the "list", this function groups "identical" objects
   together on the shopkeeper's inventory list.  The hack involves knowing how 
   the list is put together, and manipulating the order of the objects on the
   list. (But since most of DIKU is not encapsulated, and information hiding
   is almost never used, it isn't that big a deal). -JF */
static struct obj_data *slide_obj(struct obj_data *obj, struct char_data *keeper, int shop_nr)
{
...
static void shopping_sell(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
	...
	if ((sellnum = transaction_amt(arg)) < 0)
	{
		...
		snprintf(buf, sizeof(buf), "%s Uma quantidade negativa? Tente comprar algo.", GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
		return;
	}
	if (!*arg || !sellnum)
	{
		...
		snprintf(buf, sizeof(buf), "%s Oque voce quer vender??", GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
		return;
	}
	one_argument(arg, name);
	...
	if (sold < sellnum)
	{
		...
		if (!obj)
			...
			snprintf(buf, sizeof(buf), "%s Eu so' posso pagar por %d disto.", GET_NAME(ch),
					 sold);
		else
			snprintf(buf, sizeof(buf), "%s Algo esquisito me fez comprar %d.", GET_NAME(ch),
					 sold);
	...
	snprintf(tempbuf, sizeof(tempbuf), "$n vende %s.", tempstr);
	act(tempbuf, FALSE, ch, obj, 0, TO_ROOM);
...
static void shopping_value(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
	...
	if (!*arg)
	{
		snprintf(buf, sizeof(buf), "%s O que voce quer avaliar??", GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
		return;
	}
	one_argument(arg, name);
...
static char *list_object(struct obj_data *obj, int cnt, int aindex, int shop_nr,
						 struct char_data *keeper, struct char_data *ch)
{
	static char result[256];
	char itemname[128], quantity[16];	/* "Unlimited" or "%d" */

	if (shop_producing(obj, shop_nr))
		strcpy(quantity, "Ilimitado");	/* strcpy: OK (for 'quantity >= 10') */
	else
		sprintf(quantity, "%d", cnt);	/* sprintf: OK (for 'quantity >= 11',
										   32-bit int) */
	...
	case ITEM_DRINKCON:
		if (GET_OBJ_VAL(obj, 1))
			snprintf(itemname, sizeof(itemname), "%s de %s", obj->short_description,
	...
	case ITEM_STAFF:
		snprintf(itemname, sizeof(itemname), "%s%s", obj->short_description,
				 GET_OBJ_VAL(obj, 2) < GET_OBJ_VAL(obj, 1) ? " (parcialmente)" : "");
		break;
...
static void shopping_list(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
		...
		for (obj = keeper->carrying; obj; obj = obj->next_content)
			if (CAN_SEE_OBJ(ch, obj) && GET_OBJ_COST(obj) > 0)
			{
				...
				else
				{
					...
					{
						strncat(buf, list_object(last_obj, cnt, lindex, shop_nr, keeper, ch), sizeof(buf) - len - 1);	/* strncat: 
																														   OK 
																														 */
						len = strlen(buf);
						if (len + 1 >= sizeof(buf))
	...
	lindex++;
	if (!last_obj)				/* we actually have nothing in our list for
								   sale, period */
		send_to_char(ch, "Atualmente, nosso estoque esta vazio.\r\n");
	else if (*name && !found)	/* nothing the char was looking for was found */
		send_to_char(ch, "Atualmente, nao temos isto.\r\n");
	else
	{
		...
		if (has_quest)
			send_to_char(ch, "Items com \"qp\" precisam de pontos de aventura para comprar.\r\n");
	}
}
...
SPECIAL(shop_keeper)
{
	...
	if (CMD_IS("steal"))
	{
		...
		snprintf(argm, sizeof(argm), "$N grita %s'", MSG_NO_STEAL_HERE);
...
static int add_to_shop_list(struct shop_buy_data *list, int type, int *len, int *val)
{
	...
	{							/* necessary after changing to unsigned
								   v/rnums -- Welcor */
		if (*len < MAX_SHOP_OBJ)
		{
...
{
	if (error)
		log1("SYSERR: Raise MAX_SHOP_OBJ constant in shop.h to %d", len + error);
	BUY_WORD(list[len]) = NULL;
...
static int read_type_list(FILE * shop_f, struct shop_buy_data *list, int new_format, int max)
{
	...
	do
	{
		...
		{
			if (feof(shop_f))
				log1("SYSERR: unexpected end of file reading shop file type list.");
			else if (ferror(shop_f))
				log1("SYSERR: error reading reading shop file type list: %s", strerror(errno));
			else
				log1("SYSERR: error reading reading shop file type list.");
		}
		...
		if (strncmp(buf, "-1", 2) != 0)
			for (tindex = 0; *item_types[tindex] != '\n'; tindex++)
				...
				{
					num = tindex;
					memmove(buf, buf + strlen(item_types[tindex]),
							strlen(buf) - strlen(item_types[tindex]) + 1);
					break;
				}
...
static char *read_shop_message(int mnum, room_vnum shr, FILE * shop_f, const char *why)
{
	...
	for (cht = 0; tbuf[cht]; cht++)
	{
		...
		else if (tbuf[cht + 1] == 'd' && (mnum == 5 || mnum == 6))
		{
			if (ss == 0)
			{
				log1("SYSERR: Shop #%d has %%d before %%s, message #%d.", shr, mnum);
				err++;
			}
			ds++;
		}
		...
		{
			log1("SYSERR: Shop #%d has invalid format '%%%c' in message #%d.", shr, tbuf[cht + 1],
				mnum);
			err++;
		}
	}
	...
	if (ss > 1 || ds > 1)
	{
		log1("SYSERR: Shop #%d has too many specifiers for message #%d. %%s=%d %%d=%d", shr, mnum,
			ss, ds);
		err++;
	}
...
void boot_the_shops(FILE * shop_f, char *filename, int rec_count)
{
	...
	while (!done)
	{
		...
		{						/* New shop */
			...
			shop_index[top_shop].no_such_item1 =
				read_shop_message(0, SHOP_NUM(top_shop), shop_f, buf2);
			shop_index[top_shop].no_such_item2 =
				read_shop_message(1, SHOP_NUM(top_shop), shop_f, buf2);
			shop_index[top_shop].do_not_buy =
				read_shop_message(2, SHOP_NUM(top_shop), shop_f, buf2);
			shop_index[top_shop].missing_cash1 =
				read_shop_message(3, SHOP_NUM(top_shop), shop_f, buf2);
			shop_index[top_shop].missing_cash2 =
				read_shop_message(4, SHOP_NUM(top_shop), shop_f, buf2);
			shop_index[top_shop].message_buy =
				read_shop_message(5, SHOP_NUM(top_shop), shop_f, buf2);
...
void assign_the_shopkeepers(void)
{
	...
	for (cindex = 0; cindex <= top_shop; cindex++)
	{
		...
		{
			log1("shop %d had mob out of bounds", cindex);
			abort();
		}


		/* Having SHOP_FUNC() as 'shop_keeper' will cause infinite recursion. */
		if (mob_index[SHOP_KEEPER(cindex)].func
	...
	for (shop_nr = 0; shop_nr <= top_shop && len < sizeof(buf); shop_nr++)
	{
		...
		if (!(shop_nr % (PAGE_LENGTH - 2)))
		{
			...
			if (len + headerlen + 1 >= sizeof(buf))
				break;
			strcpy(buf + len, list_all_shops_header);	/* strcpy: OK (length
														   checked above) */
			len += headerlen;
		}
		...
		if (SHOP_KEEPER(shop_nr) == NOBODY)
			...
			sprintf(buf1, "%6d", mob_index[SHOP_KEEPER(shop_nr)].vnum);	/* sprintf: 
																		   OK
																		   (for 
																		   'buf1 
																		   >=
...
static void list_detailed_shop(struct char_data *ch, int shop_nr)
{
	...
	for (sindex = 0; SHOP_ROOM(shop_nr, sindex) != NOWHERE; sindex++)
	{
		...
		if ((temp = real_room(SHOP_ROOM(shop_nr, sindex))) != NOWHERE)
			linelen =
				snprintf(buf1, sizeof(buf1), "%s (#%d)", world[temp].name, GET_ROOM_VNUM(temp));
		else
	...
	/* send_to_char(ch, "Customers: %s\r\n", (ptrsave =
	   customer_string(shop_nr, TRUE)) ? ptrsave : "None"); */
	...
	for (sindex = 0; SHOP_PRODUCT(shop_nr, sindex) != NOTHING; sindex++)
	{
		...
		{
			send_to_char(ch, "\r\n            ");
			/* 12 is to line up with "Produces:" printed first, and spaces
			   above. */
			column = 12;
		}
...
bool shopping_identify(char *arg, struct char_data *ch, struct char_data *keeper, int shop_nr)
{
	...
	if (!*arg)
	{
		snprintf(buf, sizeof(buf), "%s O que voce quer identificar?", GET_NAME(ch));
		do_tell(keeper, buf, cmd_tell, 0);
		return TRUE;
	}
	if (!(obj = get_purchase_obj(ch, arg, keeper, shop_nr, TRUE)))
	...
	send_to_char(ch, "Nome: %s\r\n", (obj->short_description) ? obj->short_description : "<Nenhum>");
	sprinttype(GET_OBJ_TYPE(obj), item_types, buf, sizeof(buf));
	send_to_char(ch, "Tipo: %s\r\n", buf);
	send_to_char(ch, "Peso: %d, Custo para vender: %s%d%s, Custo para comprar: %s%d%s\r\n",
				 GET_OBJ_WEIGHT(obj),
	...
	case ITEM_LIGHT:
		if (GET_OBJ_VAL(obj, 2) == -1)
			...
			send_to_char(ch, "Horas Restantes: Nenhuma!\r\n");
		else
			send_to_char(ch, "Horas Restantes: %d\r\n", GET_OBJ_VAL(obj, 2));
		break;
	case ITEM_SCROLL:
	case ITEM_POTION:
		send_to_char(ch, "Magias: %s, %s, %s\r\n",
					 skill_name(GET_OBJ_VAL(obj, 1)),
					 skill_name(GET_OBJ_VAL(obj, 2)), skill_name(GET_OBJ_VAL(obj, 3)));
		break;
	case ITEM_WAND:
	case ITEM_STAFF:
		send_to_char(ch, "Magia: %s\r\n", skill_name(GET_OBJ_VAL(obj, 3)));
		send_to_char(ch, "Cargas: %d/%d\r\n", GET_OBJ_VAL(obj, 2), GET_OBJ_VAL(obj, 1));
		break;
	case ITEM_WEAPON:
		send_to_char(ch, "Dado de Dano e' '%dD%d' com uma media por rodada de %.1f.\r\n",
	...
	case ITEM_ARMOR:
		...
		{
			send_to_char(ch, "AC-aplicado: [%d]\r\n", GET_OBJ_VAL(obj, 0));
		}
		else
		{
			send_to_char(ch, "AC-aplicado: [%d] - Este item foi afetado magicame nte\r\n",
						 GET_OBJ_VAL(obj, 0));
		}
		break;
	case ITEM_CONTAINER:
		send_to_char(ch, "Capacidade: %d/%d\r\n", GET_OBJ_WEIGHT(obj), GET_OBJ_VAL(obj, 0));
		break;
	case ITEM_DRINKCON:
	case ITEM_FOUNTAIN:
		send_to_char(ch, "Bebidas: %d/%d\r\n", GET_OBJ_VAL(obj, 1), GET_OBJ_VAL(obj, 0));
		break;
	case ITEM_NOTE:
		send_to_char(ch, "\r\n");
		break;
	...
	case ITEM_WORN:
		if (GET_OBJ_VAL(obj, 1) > 0)
			send_to_char(ch, "Este item foi afetado magicamente.\r\n");
		else
			send_to_char(ch, "\r\n");
		break;
	default:
	...
	/* Function to take a shop and mob and save any non-unique items to it It
	   saves any object vnums which aren't native producing, good for crashs */
void save_shop_nonnative(shop_vnum shop_num, struct char_data *keeper)
{
	...
	/* Loops through the keeper's item list, checks if its native or not if it 
