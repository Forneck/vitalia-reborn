    ...
    if (!*buf1)
        ...
        if (str_cmp("save", buf1) == 0) {
            ...
            if (str_cmp("new", buf1) || !stop || !*stop)
                send_to_char(ch,
                             "Format: zedit new <zone number> <bottom-room> "
                             "<upper-room>\r\n");
            else {
...
static void zedit_save_internally(struct descriptor_data *d)
{
    ...
    if (room_num == NOWHERE) {
        log1("SYSERR: zedit_save_internally: OLC_NUM(d) room %d not found.", OLC_NUM(d));
        return;
    }
...
static void zedit_save_to_disk(int zone) { save_zone(zone); }
...
{
    clear_screen(d);
    column_list(d->character, 0, climate_types, NUM_CLIM_FLAGS, TRUE);

    write_to_output(d,
                    "\r\nClima: \tc%s\tn\r\n"
                    "Entre com o Clima, 0 to quit : ",
                    nrm);
...
static void zedit_disp_menu(struct descriptor_data *d)
{
    ...
    send_to_char(d->character,
                 ...
                 OLC_ZONE(d)->bot, grn, nrm, yel, OLC_ZONE(d)->top, grn, nrm, yel,
                 OLC_ZONE(d)->reset_mode
                     ? ((OLC_ZONE(d)->reset_mode == 1) ? "Reset when no players are in zone." : "Normal reset.")
                     : "Never reset",
                 grn, nrm, cyn, buf1, grn, nrm, levels_set ? cyn : yel, lev_string, grn, nrm, cyn, OLC_ZONE(d)->climate,
                 nrm);
        ...
        switch (MYCMD.command) {
            ...
            case 'C':
                write_to_output(d, "%sCheck %s [%s%d%s], Max : %d", MYCMD.if_flag ? " then " : "",
                                mob_proto[MYCMD.arg2].player.short_descr, cyn, mob_index[MYCMD.arg1].vnum, yel,
                                MYCMD.arg1);
                break;
            case 'G':
            ...
            case 'R':
                ...
                break;
            case 'D':
                write_to_output(d, "%sSet door %s as %s.", MYCMD.if_flag ? " then " : "", dirs[MYCMD.arg2],
                                MYCMD.arg3 ? ((MYCMD.arg3 == 1) ? "closed" : "locked") : "open");
                break;
            case 'T':
                write_to_output(
                    d, "%sAttach trigger %s%s%s [%s%d%s] to %s", MYCMD.if_flag ? " then " : "", cyn,
                    trig_index[MYCMD.arg2]->proto->name, yel, cyn, trig_index[MYCMD.arg2]->vnum, yel,
                    ((MYCMD.arg1 == MOB_TRIGGER)
                         ? "mobile"
                         : ((MYCMD.arg1 == OBJ_TRIGGER) ? "object" : ((MYCMD.arg1 == WLD_TRIGGER) ? "room" : "????"))));
                break;
            case 'V':
                write_to_output(
                    d, "%sAssign global %s:%d to %s = %s", MYCMD.if_flag ? " then " : "", MYCMD.sarg1, MYCMD.arg2,
                    ((MYCMD.arg1 == MOB_TRIGGER)
                         ? "mobile"
                         : ((MYCMD.arg1 == OBJ_TRIGGER) ? "object" : ((MYCMD.arg1 == WLD_TRIGGER) ? "room" : "????"))),
                    MYCMD.sarg2);
...
void zedit_parse(struct descriptor_data *d, char *arg)
{
    ...
    switch (OLC_MODE(d)) {
        ...
        case ZEDIT_LEVELS:
            switch (*arg) {
                case '1':
                    write_to_output(d, "Enter the min level for this zone (0-%d, -1 = none): ", (LVL_IMMORT - 1));
                    OLC_MODE(d) = ZEDIT_LEV_MIN;
                    break;

                case '2':
                    write_to_output(d, "Enter the max level for this zone (0-%d, -1 = none): ", (LVL_IMMORT - 1));
                    OLC_MODE(d) = ZEDIT_LEV_MAX;
                    break;

                case '3':
                    OLC_ZONE(d)->min_level = -1;
        ...
        case ZEDIT_ZONE_NAME:
            ...
            if (genolc_checkstring(d, arg)) {
                if (OLC_ZONE(d)->name)
                    ...
                    log1("SYSERR: OLC: ZEDIT_ZONE_NAME: no name to free!");
        ...
        case ZEDIT_ZONE_BUILDERS:
            ...
            if (genolc_checkstring(d, arg)) {
                if (OLC_ZONE(d)->builders)
                    ...
                    log1("SYSERR: OLC: ZEDIT_ZONE_BUILDERS: no builders list to free!");
        ...
        case ZEDIT_ZONE_TOP:
            ...
            if (OLC_ZNUM(d) == top_of_zone_table)
                ...
                OLC_ZONE(d)->top =
                    LIMIT(atoi(arg), genolc_zonep_bottom(OLC_ZONE(d)), genolc_zone_bottom(OLC_ZNUM(d) + 1) - 1);
            OLC_ZONE(d)->number = 1;
            zedit_disp_menu(d);
            break;
