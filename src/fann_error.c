 ...
 */
FANN_EXTERNAL void FANN_API fann_reset_errno(struct fann_error *errdat) { errdat->errno_f = FANN_E_NO_ERROR; }
 ...
 */
FANN_EXTERNAL enum fann_errno_enum FANN_API fann_get_errno(struct fann_error *errdat) { return errdat->errno_f; }
 ...
 */
void fann_error(struct fann_error *errdat, int errno_f, ...)
{
    va_list ap;
    ...
    switch (errno_f) {
        ...
        case FANN_E_WRONG_PARAMETERS_FOR_CREATE:
            strcpy(errstr,
                   "The parameters for create_standard are wrong, either too few parameters provided or a "
                   "negative/very high value provided.\n");
            break;
    }
    va_end(ap);
