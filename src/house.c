...
static void House_delete_file(room_vnum vnum)
{
    ...
    if (!(fl = fopen(filename, "rb"))) {
        if (errno != ENOENT)
            log1("SYSERR: Error deleting house file #%d. (1): %s", vnum, strerror(errno));
        return;
    }
    fclose(fl);
    if (remove(filename) < 0)
        log1("SYSERR: Error deleting house file #%d. (2): %s", vnum, strerror(errno));
}
...
static void House_listrent(struct char_data *ch, room_vnum vnum)
{
    ...
    if (!House_get_filename(vnum, filename, sizeof(filename)))
        return;
    if (!(fl = fopen(filename, "rb"))) {
        send_to_char(ch, " Sem objetos para a casa #%d.\r\n", vnum);
        return;
    }
    *buf = '\0';
...
void House_boot(void)
{
    ...
    if (!(fl = fopen(HCONTROL_FILE, "rb"))) {
        if (errno == ENOENT)
            log1("   No houses to load. File '%s' does not exist.", HCONTROL_FILE);
        else
...
static const char *HCONTROL_FORMAT =
    "Uso: hcontrol build <house vnum> <exit direction> <player name>\r\n"
...
void hcontrol_list_houses(struct char_data *ch, char *arg)
{
    ...
    if (arg && *arg) {
        ...
        if ((i = find_house(toshow)) == NOWHERE) {
            send_to_char(ch, "Casa desconhecida, \"%s\".\r\n", arg);
            return;
        }
    ...
    if (!num_of_houses) {
        send_to_char(ch, "Nenhuma casa foi definida.\r\n");
        return;
    }
    send_to_char(ch,
    ...
    for (i = 0; i < num_of_houses; i++) {
        ...
        send_to_char(ch, "%8d %7d  %-15s    %2d    %-12s %s\r\n", house_control[i].vnum, house_control[i].atrium,
...
static void hcontrol_build_house(struct char_data *ch, char *arg)
{
    ...
    if (num_of_houses >= MAX_HOUSES) {
        send_to_char(ch, "Maximo de casas definidas\r\n");
        return;
    }
    ...
    if ((real_house = real_room(virt_house)) == NOWHERE) {
        send_to_char(ch, "Não existe esta sala.\r\n");
        return;
    }
    if ((find_house(virt_house)) != NOWHERE) {
        send_to_char(ch, "Casa já existente.\r\n");
        return;
    }
    ...
    if ((exit_num = search_block(arg1, dirs, FALSE)) < 0) {
        send_to_char(ch, "'%s não é uma direção válida.\r\n", arg1);
        return;
    }
    if (TOROOM(real_house, exit_num) == NOWHERE) {
        send_to_char(ch, "Não tem saída %s da sala %d.\r\n", dirs_pt[exit_num], virt_house);
        return;
    }
    ...
    if (TOROOM(real_atrium, rev_dir[exit_num]) != real_house) {
        send_to_char(ch, "A saída de uma casa precisa ser bidirecional!\r\n");
        return;
    }
    ...
    if ((owner = get_id_by_name(arg1)) < 0) {
        send_to_char(ch, "Jogador desconhecido '%s'.\r\n", arg1);
        return;
    }
    ...
    send_to_char(ch, "Casa construída.  Parabéns!\r\n");
    House_save_control();
}
...
static void hcontrol_destroy_house(struct char_data *ch, char *arg)
{
    ...
    if ((real_atrium = real_room(house_control[i].atrium)) == NOWHERE)
        log1("SYSERR: House %d had invalid atrium %d!", atoi(arg), house_control[i].atrium);
    else
        REMOVE_BIT_AR(ROOM_FLAGS(real_atrium), ROOM_ATRIUM);

    if ((real_house = real_room(house_control[i].vnum)) == NOWHERE)
        log1("SYSERR: House %d had invalid vnum %d!", atoi(arg), house_control[i].vnum);
    else {
...
static void hcontrol_pay_house(struct char_data *ch, char *arg)
{
    ...
    if (!*arg)
        ...
        House_save_control();
        send_to_char(ch, "Pagamento realizado.\r\n");
    }
}
...
ACMD(do_house)
{
    ...
    if (!ROOM_FLAGGED(IN_ROOM(ch), ROOM_HOUSE))
        ...
        send_to_char(ch, "Mas é a sua própria casa!\r\n");
    else {
        for (j = 0; j < house_control[i].num_of_guests; j++)
            if (house_control[i].guests[j] == id) {
                for (; j < house_control[i].num_of_guests - 1; j++)
...
void House_list_guests(struct char_data *ch, int i, int quiet)
{
    ...
    if (house_control[i].num_of_guests == 0) {
        if (!quiet)
            send_to_char(ch, "  Hóspedes: Ninguém\r\n");
        return;
    }

    send_to_char(ch, "  Hóspedes: ");
