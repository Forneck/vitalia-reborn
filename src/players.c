      ...
      case 'H':
	     if (!strcmp(tag, "Hit "))	load_HMVS(ch, line, LOAD_HIT);
          ...
          GET_HOST(ch) = strdup(line);
        	...
        	else if (!strcmp(tag, "Htwn"))	GET_HOMETOWN(ch)		= atoi(line);
...
void save_char(struct char_data * ch)
{
  ...
  if (GET_HOMETOWN(ch)	   != PFDEF_HOMETOWN)	fprintf(fl, "Htwn: %d\n", GET_HOMETOWN(ch));
 ...
 if (GET_SAVE(ch, 0)	   != PFDEF_SAVETHROW)	fprintf(fl, "Thr1: %d\n", GET_SAVE(ch, 0));
  ...
  if (GET_MOVE(ch)	   != PFDEF_MOVE || GET_MAX_MOVE(ch) != PFDEF_MAXMOVE) fprintf(fl, "Move: %d/%d\n", GET_MOVE(ch), GET_MAX_MOVE(ch));
    if (GET_BREATH(ch)	   != PFDEF_BREATH  || GET_MAX_BREATH(ch)  != PFDEF_MAX_BREATH)  fprintf(fl, "Breath : %d/%d\n", GET_BREATH(ch),  GET_MAX_BREATH(ch));
