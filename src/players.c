...
void build_player_index(void)
{
    ...
    if (!(plr_index = fopen(index_name, "r"))) {
        top_of_p_table = -1;
        log1("No player index file!  First new char will be IMP!");
        return;
    }
    ...
    for (i = 0; i < rec_count; i++) {
        ...
        sscanf(line, "%ld %s %d %s %ld", &player_table[i].id, arg2, &player_table[i].level, bits,
...
void save_player_index(void)
{
    ...
    if (!(index_file = fopen(index_name, "w"))) {
        log1("SYSERR: Could not write player index file");
        return;
    }
...
int load_char(const char *name, struct char_data *ch)
{
    ...
    if ((id = get_ptable_by_name(name)) < 0)
        ...
        GET_HOMETOWN(ch) = PFDEF_HOMETOWN;
        ...
        while (get_line(fl, line)) {
            ...
            switch (*tag) {
                ...
                case 'C':
                    ...
                    else if (!strcmp(tag, "ClHs"))
                        load_class_history(fl, ch);
                ...
                case 'F':
                    ...
                    else if (!strcmp(tag, "Fit"))
                        GET_FIT(ch) = atoi(line);
                    break;
...
void save_char(struct char_data *ch)
{
    ...
    if ((i >= MAX_AFFECT) && aff && aff->next)
        log1("SYSERR: WARNING: OUT OF STORE ROOM FOR AFFECTED TYPES!!!");
    ...
    if (GET_HOMETOWN(ch) != PFDEF_HOMETOWN)
        fprintf(fl, "Htwn: %d\n", GET_HOMETOWN(ch));
    ...
    if (GET_BREATH(ch) != PFDEF_BREATH || GET_MAX_BREATH(ch) != PFDEF_MAX_BREATH)
        fprintf(fl, "Breath : %d/%d\n", GET_BREATH(ch), GET_MAX_BREATH(ch));
    ...
    if (tmp_aff[0].spell > 0) {
        ...
        for (i = 0; i < MAX_AFFECT; i++) {
            aff = &tmp_aff[i];
            if (aff->spell)
                fprintf(fl, "%d %d %d %d %d %d %d %d\n", aff->spell, aff->duration, aff->modifier, aff->location,
...
void remove_player(int pfilepos)
{
    ...
    log1("PCLEAN: %s Lev: %d Last: %s", player_table[pfilepos].name, player_table[pfilepos].level, timestr);
    ...
    do {
        ...
        if (num > 0) {
            ...
            if (n_vars == 8) { /* New 128-bit version */
                ...
                log1("SYSERR: Invalid affects in pfile (%s), expecting 5 or 8 values", GET_NAME(ch));
            }
