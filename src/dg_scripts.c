...
static void eval_op(char *op, char *lhs, char *rhs, char *result, void *go, struct script_data *sc, trig_data *trig);
static char *matching_paren(char *p);
static void eval_expr(char *line, char *result, void *go, struct script_data *sc, trig_data *trig, int type);
static int eval_lhs_op_rhs(char *expr, char *result, void *go, struct script_data *sc, trig_data *trig, int type);
...
static void process_wait(void *go, trig_data *trig, int type, char *cmd, struct cmdlist_element *cl);
...
static struct cmdlist_element *find_case(struct trig_data *trig, struct cmdlist_element *cl, void *go,
...
int find_eq_pos_script(char *arg)
{
    ...
    struct eq_pos_list {
        const char *pos;
        int where;
    } eq_pos[] = {{"hold", WEAR_HOLD},       {"held", WEAR_HOLD},        {"light", WEAR_LIGHT},
                  {"wield", WEAR_WIELD},     {"rfinger", WEAR_FINGER_R}, {"lfinger", WEAR_FINGER_L},
                  ...
                  {"lwrist", WEAR_WRIST_L},  {"wings", WEAR_WINGS},      {"rear", WEAR_EAR_R},
                  {"lear", WEAR_EAR_L},      {"face", WEAR_FACE},        {"nose", WEAR_NOSE},
                  {"insigne", WEAR_INSIGNE}, {"quiver", WEAR_QUIVER},    {"none", -1}};
...
int can_wear_on_pos(struct obj_data *obj, int pos)
{
    switch (pos) {
        ...
        case WEAR_WINGS:
            return CAN_WEAR(obj, ITEM_WEAR_WINGS);
        case WEAR_EAR_R:
        case WEAR_EAR_L:
        ...
        case WEAR_NOSE:
            return CAN_WEAR(obj, ITEM_WEAR_NOSE);
        case WEAR_INSIGNE:
            return CAN_WEAR(obj, ITEM_WEAR_INSIGNE);
        case WEAR_QUIVER:
            return CAN_WEAR(obj, ITEM_WEAR_QUIVER);
        default:
            return FALSE;
    }
}
...
static EVENTFUNC(trig_wait_event)
{
...
#if 1 /* debugging */
    {
        ...
        if (!found) {
            log1("Trigger restarted on unknown entity. Vnum: %d", GET_TRIG_VNUM(trig));
            log1("Type: %s trigger", type == MOB_TRIGGER ? "Mob" : type == OBJ_TRIGGER ? "Obj" : "Room");
            log1("attached %d places", trig_index[trig->nr]->number);
...
static void do_stat_trigger(struct char_data *ch, trig_data *trig)
{
    ...
    if (!trig) {
        log1("SYSERR: NULL trigger passed to do_stat_trigger.");
        return;
    }

    len += snprintf(sb, sizeof(sb), "Name: '%s%s%s',  VNum: [%s%5d%s], RNum: [%5d]\r\n", CCYEL(ch, C_NRM),
                    GET_TRIG_NAME(trig), CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), GET_TRIG_VNUM(trig), CCNRM(ch, C_NRM),
                    GET_TRIG_RNUM(trig));
    ...
    len += snprintf(sb + len, sizeof(sb) - len, "Trigger Type: %s, Numeric Arg: %d, Arg list: %s\r\n", buf,
                    GET_TRIG_NARG(trig), ((GET_TRIG_ARG(trig) && *GET_TRIG_ARG(trig)) ? GET_TRIG_ARG(trig) : "None"));
...
static void script_stat(char_data *ch, struct script_data *sc)
{
    ...
    for (t = TRIGGERS(sc); t; t = t->next) {
        send_to_char(ch, "\r\n  Trigger: %s%s%s, VNum: [%s%5d%s], RNum: [%5d]\r\n", CCYEL(ch, C_NRM), GET_TRIG_NAME(t),
                     CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), GET_TRIG_VNUM(t), CCNRM(ch, C_NRM), GET_TRIG_RNUM(t));
...
ACMD(do_attach)
{
    ...
    if (is_abbrev(arg, "mobile") || is_abbrev(arg, "mtr")) {
        ...
        send_to_char(ch, "Trigger %d (%s) attached to %s [%d].\r\n", tn, GET_TRIG_NAME(trig),
                     (object->short_description ? object->short_description : object->name), GET_OBJ_VNUM(object));
    }
...
ACMD(do_detach)
{
    ...
    if (!*arg1 || !*arg2) {
        send_to_char(ch,
                     "Usage: detach [ mob | object | room ] { target } { trigger |"
                     " 'all' }\r\n");
        return;
    }
...
static void process_attach(void *go, struct script_data *sc, trig_data *trig, int type, char *cmd)
{
    ...
    if (trignum == NOTHING || !(newtrig = read_trigger(trignum))) {
        script_log("Trigger: %s, VNum %d. attach invalid trigger: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                   trignum_s);
        return;
    }
    ...
    if (c) {
        if (!IS_NPC(c) && !CONFIG_SCRIPT_PLAYERS) {
            script_log("Trigger: %s, VNum %d. attach invalid target: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                       GET_NAME(c));
            return;
        }
        if (!SCRIPT(c))
    ...
    *uid = '\0';
    ...
    if (atoi(arg) != 0) { /* easy, if you pass an id number */
        ...
        if (is_abbrev(arg, "mob")) {
            ...
            script_log("Trigger: %s, VNum %d. makeuid syntax error: '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                       cmd);
...
static void process_remote(struct script_data *sc, trig_data *trig, char *cmd)
{
    ...
    if (!*buf || !*buf2) {
        script_log("Trigger: %s, VNum %d. remote: invalid arguments '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                   cmd);
        return;
    }
    ...
    if ((room = find_room(uid))) {
        ...
        script_log("Trigger: %s, VNum %d. remote: uid '%ld' invalid", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig), uid);
        return;
    }
...
static void process_rdelete(struct script_data *sc, trig_data *trig, char *cmd)
{
    ...
    if (!*buf || !*buf2) {
        script_log("Trigger: %s, VNum %d. rdelete: invalid arguments '%s'", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                   cmd);
        return;
    }
...
int script_driver(void *go_adress, trig_data *trig, int type, int mode)
{
    ...
    if (depth > MAX_SCRIPT_DEPTH) {
        ...
        switch (type) {
            case MOB_TRIGGER:
                script_log("It was attached to %s [%d]", GET_NAME((char_data *)go), GET_MOB_VNUM((char_data *)go));
                break;
            case OBJ_TRIGGER:
...
void read_saved_vars(struct char_data *ch)
{
    ...
    if (!file) {
        log1("%s had no variable file", GET_NAME(ch));
        return;
    }
...
void save_char_vars(struct char_data *ch)
{
    ...
    file = fopen(fn, "wt");
    if (!file) {
        mudlog(NRM, LVL_GOD, TRUE, "SYSERR: Could not open player variable file %s for writing.:%s", fn,
               strerror(errno));
        return;
    }
...
static struct char_data *find_char_by_uid_in_lookup_table(long uid)
{
    ...
    log1("find_char_by_uid_in_lookup_table : No entity with number %ld in lookup table", uid);
    return NULL;
}
...
static struct obj_data *find_obj_by_uid_in_lookup_table(long uid)
{
    ...
    log1("find_obj_by_uid_in_lookup_table : No entity with number %ld in lookup table", uid);
    return NULL;
}
...
void add_to_lookup_table(long uid, void *c)
{
    ...
    if (lt && lt->uid == uid) {
        log1("add_to_lookup updating existing value for uid=%ld (%p -> %p)", uid, lt->c, c);
        lt->c = c;
        return;
    }
    ...
    for (; lt && lt->next; lt = lt->next)
        if (lt->next->uid == uid) {
            log1("add_to_lookup updating existing value for uid=%ld (%p -> %p)", uid, lt->next->c, c);
            return;
        }
...
void remove_from_lookup_table(long uid)
{
    ...
    log1("remove_from_lookup. UID %ld not found.", uid);
}
...
long obj_script_id(obj_data *obj)
{
    if (obj->script_id == 0) {
        ...
        if (max_obj_id > x && reboot_control.time > 16) {
          mudlog(CMP, LVL_BUILDER, TRUE, "SYSERR: Script IDs for objects have exceeded the limit -- reboot to fix
        this");
        }
        */
    }
