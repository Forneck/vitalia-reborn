...
ACMD(do_oasis_oedit)
{
    ...
    if (save) {
        ...
        mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE, "OLC: %s saves object info for zone %d.", GET_NAME(ch),
               zone_table[OLC_ZNUM(d)].number);
...
void oedit_save_internally(struct descriptor_data *d)
{
    ...
    if ((robj_num = add_object(OLC_OBJ(d), OLC_NUM(d))) == NOTHING) {
        log1("oedit_save_internally: add_object failed.");
        return;
    }
...
static void oedit_disp_extradesc_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    grn, nrm, yel, (extra_desc->keyword && *extra_desc->keyword) ? extra_desc->keyword : "<NONE>", grn,
                    nrm, yel,
                    (extra_desc->description && *extra_desc->description) ? extra_desc->description : "<NONE>", grn,
                    nrm, !extra_desc->next ? "Not set." : "Set.", grn, nrm);
...
static void oedit_disp_prompt_apply_menu(struct descriptor_data *d)
{
    ...
    for (counter = 0; counter < MAX_OBJ_AFFECT; counter++) {
        if (OLC_OBJ(d)->affected[counter].modifier) {
            ...
            write_to_output(d, " %s%d%s) %+d to %s\r\n", grn, counter + 1, nrm, OLC_OBJ(d)->affected[counter].modifier,
                            apply_buf);
        } else {
...
static void oedit_disp_weapon_menu(struct descriptor_data *d)
{
    ...
    for (counter = 0; counter < NUM_ATTACK_TYPES; counter++) {
        write_to_output(d, "%s%2d%s) %-20.20s %s", grn, counter, nrm, attack_hit_text[counter].singular,
                        !(++columns % 2) ? "\r\n" : "");
    }
    write_to_output(d, "\r\nEnter weapon type : ");
}
...
static void oedit_disp_spells_menu(struct descriptor_data *d)
{
    ...
    for (counter = 1; counter <= MAX_SKILLS; counter++) {
        if ((spell = get_spell_by_vnum(counter)))
            write_to_output(d, "%s%2d%s) %s%-20.20s %s", grn, counter, nrm, yel, spell->name,
                            !(++columns % 3) ? "\r\n" : "");
    }
...
static void oedit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    cyn, OLC_NUM(d), nrm, grn, nrm, yel, (obj->name && *obj->name) ? obj->name : "undefined", grn, nrm,
                    yel, (obj->short_description && *obj->short_description) ? obj->short_description : "undefined",
                    grn, nrm, yel, (obj->description && *obj->description) ? obj->description : "undefined", grn, nrm,
                    yel,
    ...
    write_to_output(d,
                    ...
                    cyn, GET_OBJ_RENT(obj), grn, nrm, cyn, GET_OBJ_TIMER(obj), grn, nrm, cyn, GET_OBJ_VAL(obj, 0),
                    GET_OBJ_VAL(obj, 1), GET_OBJ_VAL(obj, 2), GET_OBJ_VAL(obj, 3), grn, nrm, grn, nrm, cyn,
                    obj->ex_description ? "Set." : "Not Set.", grn, grn, nrm, cyn, GET_OBJ_LEVEL(obj), grn, nrm, cyn,
                    buf2, grn, nrm, cyn, OLC_SCRIPT(d) ? "Set." : "Not Set.", grn, nrm, grn, nrm, grn, nrm);
