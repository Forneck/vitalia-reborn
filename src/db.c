...
static void boot_social_messages(void)
{
	...
	if (CONFIG_NEW_SOCIALS == TRUE)
	{
		...
		*next_soc = '\0';
		...
		if (ferror(fl))
		{
			log1("SYSERR: error encountered reading socials file %s: %s", SOCMESS_FILE_NEW,
				strerror(errno));
			exit(1);
		}
	}
	...
	for (line_number = 0;; ++line_number)
	{
		...
		{
			if (feof(fl))
				log1("SYSERR: unexpected end of file encountered in socials file %s",
					SOCMESS_FILE_NEW);
			else if (ferror(fl))
				log1("SYSERR: error reading socials file %s: %s", SOCMESS_FILE_NEW,
					strerror(errno));
			else
...
void free_text_files(void)
{
	char **textfiles[] = {
		...
		&policies, &handbook, &background, &GREETINGS, &bugs, &typos, &ideas,&menu,&clanpolicies,&rebegin, NULL
	};
	int rf;
...
log1("Sorting command list.");
...
#if 1
	{
		...
		for (j = 0; j < top_of_objt; j++)
		{
			...
			{
				log1("Item %d (%s) had shopkeeper trouble.", obj_index[j].vnum,
...
static void reset_time(void)
{
	...
	else
	{
		if (fscanf(bgtime, "%ld\n", (long *)&beginning_of_time) == EOF)
		{
			if (feof(bgtime))
			{
				log1("SYSERR: reset_time: unexpected end of file encountered reading %s.",
					TIME_FILE);
			}
			else if (ferror(bgtime))
			{
				log1("SYSERR: reset_time: unexpected end of file encountered reading %s: %s.",
...
void index_boot(int mode)
{
	...
	for (line_number = 0;; ++line_number)
	{
		...
		{
			if (feof(db_index))
				log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s. " "Ensure that the last line of the file starts with the character '$'.", prefix, index_filename);
			else if (ferror(db_index))
				log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s: %s", prefix, index_filename, strerror(errno));
			else
				log1("SYSERR: boot error -- error parsing index file %s%s on line %d",
		...
		{
			log1("SYSERR: File '%s' listed in '%s/%s': %s", buf2, prefix,
	...
	for (line_number = 1;; ++line_number)
	{
		...
		{
			if (feof(db_index))
				log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s", prefix, index_filename);
			else if (ferror(db_index))
				log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s: %s", prefix, index_filename, strerror(errno));
			else
				log1("SYSERR: boot error -- error parsing index file ./%s%s on line %d",
	...
	for (;;)
	{
		...
		if (mode != DB_BOOT_OBJ || nr < 0)
			if (!get_line(fl, line))
			{
				...
				{
					log1("SYSERR: Format error in %s after %s #%d\n"
...
void parse_room(FILE * fl, int virtual_nr)
{
	...
	if (virtual_nr < zone_table[zone].bot)
	{
		log1("SYSERR: Room #%d is below zone %d (bot=%d, top=%d).", virtual_nr,
			zone_table[zone].number, zone_table[zone].bot, zone_table[zone].top);
		exit(1);
	}
	...
	if (((retval =
		  sscanf(line, "%d %s %s %s %s %d ", t, flags, flags2, flags3, flags4, t + 2)) == 3)
		&& (bitwarning == TRUE))
	{
	...
	else if ((retval == 3) && (bitwarning == FALSE))
	{
		...
		if (bitsavetodisk)
		{						/* Maybe the implementor just wants to look at 
								   the 128bit files */
...
static void parse_simple_mob(FILE * mob_f, int i, int nr)
{
	...
	{
		log1("SYSERR: Format error in mob #%d, first line after S flag\n"
	...
	if (!get_line(mob_f, line))
	{
		log1("SYSERR: Format error in mob #%d, second line after S flag\n"
	...
	if (!get_line(mob_f, line))
	{
		log1("SYSERR: Format error in last line of mob #%d\n"
			"...expecting line of form '# # #', but file ended!", nr);
		exit(1);
	}
...
void parse_mobile(FILE * mob_f, int nr)
{
	...
	if (((retval =
		  sscanf(line, "%s %s %s %s %s %s %s %s %d %c", f1, f2, f3, f4, f5, f6, f7, f8, t + 2,
				 &letter)) != 10) && (bitwarning == TRUE))
	{
	...
	else if ((retval == 4) && (bitwarning == FALSE))
	{
		...
		if (MOB_FLAGGED(mob_proto + i, MOB_AGGRESSIVE)
			&& MOB_FLAGGED(mob_proto + i, MOB_AGGR_NEUTRAL))
			REMOVE_BIT_AR(MOB_FLAGS(mob_proto + i), MOB_AGGR_NEUTRAL);
		if (MOB_FLAGGED(mob_proto + i, MOB_AGGRESSIVE)
			&& MOB_FLAGGED(mob_proto + i, MOB_AGGR_EVIL))
			REMOVE_BIT_AR(MOB_FLAGS(mob_proto + i), MOB_AGGR_EVIL);

		check_bitvector_names(AFF_FLAGS(mob_proto + i)[0], affected_bits_count, buf2,
							  "mobile affect");
	...
	for (taeller = 0; taeller < AF_ARRAY_MAX; taeller++)
		...
		AFF_FLAGS(mob_proto + i)[0] = asciiflag_conv_aff(f5)+1;
		AFF_FLAGS(mob_proto + i)[1] = asciiflag_conv_aff(f6)+1;
		AFF_FLAGS(mob_proto + i)[2] = asciiflag_conv_aff(f7)+1;
		AFF_FLAGS(mob_proto + i)[3] = asciiflag_conv_aff(f8)+1;
...
char *parse_object(FILE * obj_f, int nr)
{
	...
	{
		log1("SYSERR: Format error in first numeric line (expecting 13 args, got %d), %s",
			retval, buf2);
		exit(1);
	}
	...
	{
		log1("SYSERR: Format error in second numeric line (expecting 4 args, got %d), %s",
			retval, buf2);
		exit(1);
	}
	...
	if ((retval = sscanf(line, "%d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4)) != 5)
	{
		...
		{
			log1("SYSERR: Format error in third numeric line (expecting 5 args, got %d), %s",
				retval, buf2);
			exit(1);
		}
	}
	...
	for (;;)
	{
		...
		case 'A':
			...
			if (!get_line(obj_f, line))
			{
				log1("SYSERR: Format error in 'A' field, %s\n"
...
static void load_zones(FILE * fl, char *zonename)
{
	...
	for (;;)
	{
		...
		error = 0;
		if (strchr("MOGEPDTVC", ZCMD.command) == NULL)
		{						/* a 3-arg command */
	...
	{
		log1("SYSERR: Zone command count mismatch for %s. Estimated: %d, Actual: %d", zname,
			num_of_cmds, cmd_no + 1);
		exit(1);
	}
...
void load_help(FILE * fl, char *name)
{
	...
	while (*key != '$')
	{
		...
		if (entrylen >= sizeof(entry) - 1)
		{
			...
			strcpy(entry + sizeof(entry) - strlen(truncmsg) - 1, truncmsg);	/* strcpy: 
																			   OK 
																			   (assuming 
																			   sane 
																			   'entry' 
		...
		case 'G':				/* obj_to_char */
			if (!mob)
			{
				...
				snprintf(error, sizeof(error),
						 "attempt to give obj #%d to non-existant mob, command disabled",
      ...
      break;
		...
		case 'E':				/* object to equipment list */
			if (!mob)
			{
				char error[MAX_INPUT_LENGTH];
				snprintf(error, sizeof(error),
						 "trying to equip non-existant mob with obj #%d, command disabled",
						 obj_index[ZCMD.arg1].vnum);
				ZONE_ERROR(error);
				ZCMD.command = '*';
				break;
			...
			if (obj_index[ZCMD.arg1].number < ZCMD.arg2)
			{
				if (ZCMD.arg3 < 0 || ZCMD.arg3 >= NUM_WEARS)
				{
					char error[MAX_INPUT_LENGTH];
					snprintf(error, sizeof(error),
							 "invalid equipment pos number (mob %s, obj %d, pos %d)",
		...
		case 'D':				/* set state of door */
				...
				(world[ZCMD.arg1].dir_option[ZCMD.arg2] == NULL))
			{
				char error[MAX_INPUT_LENGTH];
				snprintf(error, sizeof(error),
						 "door does not exist in room %d - dir %d, command disabled",
...
void free_char(struct char_data *ch)
{
	...
	if (ch->player_specials != NULL && ch->player_specials != &dummy_mob)
	{
		...
		if (GET_HOST(ch))
			free(GET_HOST(ch));
		if (IS_NPC(ch))
			log1("SYSERR: Mob %s (#%d) had player_specials allocated!", GET_NAME(ch),
				GET_MOB_VNUM(ch));
	}
...
static int check_object(struct obj_data *obj)
{
	...
	if (GET_OBJ_WEIGHT(obj) < 0 && (error = TRUE))
		log1("SYSERR: Object #%d (%s) has negative weight (%d).",
			GET_OBJ_VNUM(obj), obj->short_description, GET_OBJ_WEIGHT(obj));

	if (GET_OBJ_RENT(obj) < 0 && (error = TRUE))
		log1("SYSERR: Object #%d (%s) has negative cost/day (%d).",
	...
	{
		error |=
			check_bitvector_names(GET_OBJ_WEAR(obj)[y], wear_bits_count, objname, "object wear");
		error |=
			check_bitvector_names(GET_OBJ_EXTRA(obj)[y], extra_bits_count, objname,
								  "object extra");
		error |=
			check_bitvector_names(GET_OBJ_AFFECT(obj)[y], affected_bits_count, objname,
								  "object affect");
	}
	...
	case ITEM_DRINKCON:
		{
			...
			if (search_block(onealias, drinknames, TRUE) < 0 && (error = TRUE))
				log1("SYSERR: Object #%d (%s) doesn't have drink type as last keyword. (%s)",
	...
	case ITEM_FOUNTAIN:
		if ((GET_OBJ_VAL(obj, 0) > 0) && (GET_OBJ_VAL(obj, 1) > GET_OBJ_VAL(obj, 0)
										  && (error = TRUE)))
			log1("SYSERR: Object #%d (%s) contains (%d) more than maximum (%d).",
	...
	case ITEM_STAFF:
		...
		if (GET_OBJ_VAL(obj, 2) > GET_OBJ_VAL(obj, 1) && (error = TRUE))
			log1("SYSERR: Object #%d (%s) has more charges (%d) than maximum (%d).",
				GET_OBJ_VNUM(obj), obj->short_description,
	...
	case ITEM_NOTE:
		if (obj->ex_description)
		{
			...
			do
			{
				...
				{
					log1("SYSERR: Object #%d (%s) is type NOTE and has extra description with same name. (%s)", GET_OBJ_VNUM(obj), obj->short_description, obj->name);
				}
...
static int check_object_spell_number(struct obj_data *obj, int val)
{
 ...
 if ((spellnum == -1) || (spellnum == 0)) /* no spell */
  ...
  if ((spellnum < 0) || (spellnum > TOP_SPELL_DEFINE)) {
		log1("SYSERR: Object #%d (%s) has out of range spell #%d.",
...
spell = get_spell_by_vnum(spellnum);
 if (!spell) {
    log1("SYSERR: Object #%d (%s) has non-existent spell #%d.",
	GET_OBJ_VNUM(obj), obj->short_description, spellnum);
    error = TRUE;
  } else {
     spellname = skill_name(spellnum);
      ...
      if (spell->status == unavailable) {
        log1("SYSERR: Object #%d (%s) uses disabled '%s' spell #%d.",
			GET_OBJ_VNUM(obj), obj->short_description, spellname, +		spellnum);
       error = TRUE;
      } else // no needs to check for "Undefined" named, if it's disabled.

      /* Now check for unnamed spells. */
   if (!str_cmp("Undefined", spellname)) {
        log1("SYSERR: Object #%d (%s) uses '%s' spell #%d.",
...
static int check_object_level(struct obj_data *obj, int val)
{
	...
	if ((GET_OBJ_VAL(obj, val) < 0 || GET_OBJ_VAL(obj, val) > LVL_IMPL) && (error = TRUE))
		log1("SYSERR: Object #%d (%s) has out of range level #%d.",
								 ...
								 const char *whatbits)
{
	...
	for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 8; flagnum++)
		if ((1 << flagnum) & bits)
		{
			log1("SYSERR: %s has unknown %s flag, bit %d (0 through %d known).", whatami,
				whatbits, flagnum, (int)namecount - 1);
			error = TRUE;
		}
