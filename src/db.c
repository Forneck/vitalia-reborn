...
char *clanpolicies = NULL; /* policies page */
char *rebegin = NULL;
...
char *fread_action(FILE *fl, int nr)
{
    ...
    if (fgets(buf, MAX_STRING_LENGTH, fl) == NULL) {
        if (feof(fl)) {
            log1("SYSERR: fread_action: unexpected EOF near action #%d", nr);
            /* SYSERR_DESC: fread_action() will fail if it discovers an end of
               file * marker before it is able to read in the expected string.
               This can be * caused by a truncated socials file. */
        } else {
            log1("SYSERR: fread_action: read error near action #%d: %s", nr, strerror(errno));
        }
        exit(1);
    }
    ...
    /* Some clients interpret '\r' the same as { '\r' '\n' }, so the original
       way of just replacing '\n' with '\0' would appear as 2 new lines
...
static void boot_social_messages(void)
{
    ...
    if (CONFIG_NEW_SOCIALS == TRUE) {
        ...
        if (!(fl = fopen(SOCMESS_FILE_NEW, "r"))) {
            log1("SYSERR: can't open socials file '%s': %s", SOCMESS_FILE_NEW, strerror(errno));
            /* SYSERR_DESC: This error, from boot_social_messages(), occurs
               when the server fails to open the file containing the social
               messages.  The error at the end will indicate the reason why. */
            exit(1);
        }
        /* count socials */
        *next_soc = '\0';
        ...
        if (ferror(fl)) {
            log1("SYSERR: error encountered reading socials file %s: %s", SOCMESS_FILE_NEW, strerror(errno));
            exit(1);
        }
    } else { /* old style */
        ...
        if (!(fl = fopen(SOCMESS_FILE, "r"))) {
            log1("SYSERR: can't open socials file '%s': %s", SOCMESS_FILE, strerror(errno));
            /* SYSERR_DESC: This error, from boot_social_messages(), occurs
               when the server fails to open the file containing the social
               messages.  The error at the end will indicate the reason why. */
            exit(1);
        ...
        while (fgets(next_soc, MAX_STRING_LENGTH, fl))
            if (*next_soc == '\n' || *next_soc == '\r')
                top_of_socialt++; /* all socials are followed by a blank
                                                     line */
    ...
    log1("Social table contains %d socials.", top_of_socialt);
    rewind(fl);
    ...
    for (line_number = 0;; ++line_number) {
        if (fscanf(fl, " %s ", next_soc) != 1) {
            if (feof(fl))
                log1("SYSERR: unexpected end of file encountered in socials file %s", SOCMESS_FILE_NEW);
            else if (ferror(fl))
                log1("SYSERR: error reading socials file %s: %s", SOCMESS_FILE_NEW, strerror(errno));
            else
                log1("SYSERR: format error in social file near line %d", line_number);
            exit(1);
        }
        if (*next_soc == '$')
            break;
        if (CONFIG_NEW_SOCIALS == TRUE) {
            if (fscanf(fl, " %s %d %d %d %d \n", sorted, &hide, &min_char_pos, &min_pos, &min_lvl) != 5) {
                log1("SYSERR: format error in social file near social '%s'", next_soc);
                /* SYSERR_DESC: From boot_social_messages(), this error is
                   output when the server is expecting to find the remainder
                   of the first line of the social ('hide' and 'minimum
                   position').  These must follow the name of the social with
                   a single space such as: 'accuse 0 5\n'. This error often
                   occurs when one of the numbers is missing or the social
                   name has a space in it (i.e., 'bend over'). */
                exit(1);
            }
            curr_soc++;
...
#ifdef CIRCLE_ACORN
        if (fgetc(fl) != '\n')
            log1("SYSERR: Acorn bug workaround failed.");
            /* SYSERR_DESC: The only time that this error should ever arise is if
               you are running your MUD on the Acorn platform.  The error arises
               when the server cannot properly read a '\n' out of the file at the
               end of the first line of the social (that with 'hide' and 'min
               position').  This is in boot_social_messages(). */
#endif
...
{
    char **textfiles[] = {&wizlist, &immlist, &news,     &credits,      &motd,       &imotd,     &help,
                          &ihelp,   &info,    &policies, &handbook,     &background, &GREETINGS, &bugs,
                          &typos,   &ideas,   &menu,     &clanpolicies, &rebegin,    NULL};
    int rf;
...
ACMD(do_reboot)
{
    ...
    if (!str_cmp(arg, "all") || *arg == '*') {
        ...
        if (file_to_string_alloc(MENU_FILE, &menu) < 0)
            send_to_char(ch, "Cannot read menu\r\n");
        if (file_to_string_alloc(CLANPOL_FILE, &clanpolicies) < 0)
            send_to_char(ch, "Cannot read clan policies\r\n");
        if (file_to_string_alloc(REBEGIN_FILE, &news) < 0)
            send_to_char(ch, "Cannot read rebegin\r\n");
    ...
    } else if (!str_cmp(arg, "clan-policies")) {
        if (file_to_string_alloc(CLANPOL_FILE, &clanpolicies) < 0)
            send_to_char(ch, "Cannot read clan policies\r\n");
    } else if (!str_cmp(arg, "rebegin")) {
        if (file_to_string_alloc(REBEGIN_FILE, &rebegin) < 0)
            send_to_char(ch, "Cannot read rebegin\r\n");
...
{
    log1("Loading zone table.");
    index_boot(DB_BOOT_ZON);

    log1("Loading triggers and generating index.");
    ...
    log1("Renumbering rooms.");
    renum_world();

    log1("Checking start rooms.");
    check_start_rooms();

    log1("Loading mobs and generating index.");
    index_boot(DB_BOOT_MOB);

    log1("Loading objs and generating index.");
    index_boot(DB_BOOT_OBJ);

    log1("Renumbering zone table.");
    renum_zone_table();

    if (converting) {
        log1("Saving 128bit world files to disk.");
        save_all();
    }
...
void boot_db(void)
{
    ...
    log1("Resetting the game time:");
    reset_time();

    log1("Initialize Global Lists");
    global_lists = create_list();
    group_list = create_list();

    log1("Initializing Events");
    init_events();

    log1("Reading news, credits, help, ihelp, bground, info & motds.");
    ...
    log1("Reading menu, rebegin & clanpolicies");
    file_to_string_alloc(CLANPOL_FILE, &clanpolicies);
    file_to_string_alloc(MENU_FILE, &menu);
    file_to_string_alloc(REBEGIN_FILE, &rebegin);
    ...
    log1("Loading fight messages.");
    load_messages();

    log1("Loading social messages.");
    boot_social_messages();

    log1("Building command list.");
    ...
    log1("Reading banned site and invalid-name list.");
    load_banned();
    read_invalid_list();
    ...
    if (!no_rent_check) {
        log1("Deleting timed-out crash and rent files:");
        update_obj_file();
        log1("   Done.");
    }
...
#if 1
    {
        ...
        for (j = 0; j < top_of_objt; j++) {
            if (obj_proto[j].script == (struct script_data *)&shop_keeper) {
                log1("Item %d (%s) had shopkeeper trouble.", obj_index[j].vnum, obj_proto[j].short_description);
    ...
    for (i = 0; i <= top_of_zone_table; i++) {
        log1("Resetting #%d: %s (rooms %d-%d).", zone_table[i].number, zone_table[i].name, zone_table[i].bot,
             zone_table[i].top);
        reset_zone(i);
...
static void reset_time(void)
{
    ...
    if ((bgtime = fopen(TIME_FILE, "r")) == NULL)
        log1("No time file '%s' starting from the beginning.", TIME_FILE);
    else {
        if (fscanf(bgtime, "%ld\n", (long *)&beginning_of_time) == EOF) {
            if (feof(bgtime)) {
                log1("SYSERR: reset_time: unexpected end of file encountered reading %s.", TIME_FILE);
            } else if (ferror(bgtime)) {
                log1("SYSERR: reset_time: unexpected end of file encountered reading %s: %s.", TIME_FILE,
                     strerror(errno));
            }
        }
        fclose(bgtime);
    ...
    log1("   Current Gametime: %dH %dD %dM %dY.", time_info.hours, time_info.day, time_info.month, time_info.year);
}
...
void save_mud_time(struct time_info_data *when)
{
    ...
    if ((bgtime = fopen(TIME_FILE, "w")) == NULL)
        log1("SYSERR: Can't write to '%s' time file.", TIME_FILE);
    else {
...
ackeof:
    log1("SYSERR: Unexpected end of help file.");
    exit(1); /* Some day we hope to handle these things
                        better... */
}
...
void index_boot(int mode)
{
    ...
    char buf2[PATH_MAX], buf1[PATH_MAX - 100];   // - 100 to make room for
    // prefix
    ...
    switch (mode) {
        ...
        case DB_BOOT_QST:
            ...
            break;
        default:
            log1("SYSERR: Unknown subcommand %d to index_boot!", mode);
            exit(1);
    }
    ...
    if (!(db_index = fopen(buf2, "r"))) {
        log1("SYSERR: opening index file '%s': %s", buf2, strerror(errno));
        exit(1);
    }
    ...
    for (line_number = 0;; ++line_number) {
        ...
        if (fscanf(db_index, "%s\n", buf1) != 1) {
            if (feof(db_index))
                log1(
                    "SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s. "
                    "Ensure that the last line of the file starts with the character '$'.",
                    prefix, index_filename);
            else if (ferror(db_index))
                log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s: %s", prefix,
                     index_filename, strerror(errno));
            else
                log1("SYSERR: boot error -- error parsing index file %s%s on line %d", prefix, index_filename,
                     line_number);
            exit(1);
        }
        ...
        if (!(db_file = fopen(buf2, "r"))) {
            log1("SYSERR: File '%s' listed in '%s/%s': %s", buf2, prefix, index_filename, strerror(errno));
        } else {
            if (mode == DB_BOOT_ZON)
                rec_count++;
    ...
    if (!rec_count) {
        if (mode == DB_BOOT_SHP || mode == DB_BOOT_QST)
            return;
        log1("SYSERR: boot error - 0 records counted in %s/%s.", prefix, index_filename);
        exit(1);
    }
    ...
    switch (mode) {
        ...
        case DB_BOOT_WLD:
            ...
            log1("   %d rooms, %d bytes.", rec_count, size[0]);
            break;
        case DB_BOOT_MOB:
        ...
        case DB_BOOT_OBJ:
            ...
            log1("   %d objs, %d bytes in index, %d bytes in prototypes.", rec_count, size[0], size[1]);
            break;
        case DB_BOOT_ZON:
            CREATE(zone_table, struct zone_data, rec_count);
            size[0] = sizeof(struct zone_data) * rec_count;
            log1("   %d zones, %d bytes.", rec_count, size[0]);
            break;
        case DB_BOOT_HLP:
            CREATE(help_table, struct help_index_element, rec_count);
            size[0] = sizeof(struct help_index_element) * rec_count;
            log1("   %d entries, %d bytes.", rec_count, size[0]);
            break;
        case DB_BOOT_QST:
    ...
    for (line_number = 1;; ++line_number) {
        if (fscanf(db_index, "%s\n", buf1) != 1) {
            if (feof(db_index))
                log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s", prefix,
                     index_filename);
            else if (ferror(db_index))
                log1("SYSERR: boot error -- unexpected end of file encountered in index file ./%s%s: %s", prefix,
                     index_filename, strerror(errno));
        ...
        snprintf(buf2, sizeof(buf2), "%s%s", prefix, buf1);
        if (!(db_file = fopen(buf2, "r"))) {
            log1("SYSERR: %s: %s", buf2, strerror(errno));
            exit(1);
        }
        switch (mode) {
            case DB_BOOT_WLD:
...
void discrete_load(FILE *fl, int mode, char *filename)
{
    ...
    for (;;) {
        ...
        if (mode != DB_BOOT_OBJ || nr < 0)
            if (!get_line(fl, line)) {
                if (nr == -1) {
                    log1("SYSERR: %s file %s is empty!", modes[mode], filename);
                } else {
                    log1(
                        "SYSERR: Format error in %s after %s #%d\n"
        ...
        if (*line == '#') {
            ...
            if (sscanf(line, "#%d", &nr) != 1) {
                log1("SYSERR: Format error after %s #%d", modes[mode], last);
                exit(1);
            }
            if (nr >= 99999)
                return;
            ...
            log1("SYSERR: Format error in %s file %s near %s #%d", modes[mode], filename, modes[mode], nr);
...
/* Fix for crashes in the editor when formatting. E-descs are assumed to * end
   with a \r\n. -Welcor */
void ensure_newline_terminated(struct extra_descr_data *new_descr)
{
    ...
    if (end > new_descr->description && *(end - 1) != '\n') {
        ...
        sprintf(with_term, "%s\r\n",
                new_descr->description); /* snprintf ok
                                                                                                    : size
                                                                                                    checked
                                                                                                    above */
...
void parse_room(FILE *fl, int virtual_nr)
{
    ...
    if (virtual_nr < zone_table[zone].bot) {
        log1("SYSERR: Room #%d is below zone %d (bot=%d, top=%d).", virtual_nr, zone_table[zone].number,
             zone_table[zone].bot, zone_table[zone].top);
        exit(1);
    }
    while (virtual_nr > zone_table[zone].top)
        if (++zone > top_of_zone_table) {
            log1("SYSERR: Room %d is outside of any zone.", virtual_nr);
            exit(1);
        }
    ...
    if (((retval = sscanf(line, "%d %s %s %s %s %d ", t, flags, flags2, flags3, flags4, t + 2)) == 3) &&
        (bitwarning == TRUE)) {
        log1("WARNING: Conventional world files detected. See config.c.");
        exit(1);
    } else if ((retval == 3) && (bitwarning == FALSE)) {
        /* Looks like the implementor is ready, so let's load the world files.
           We load the extra three flags as 0, since they won't be anything
           anyway. We will save the entire world later on, when every room,
           mobile, and object is converted. */
        log1("Converting room #%d to 128bits..", virtual_nr);
        ...
        sprintf(flags, "room #%d", virtual_nr); /* sprintf: OK (until 399-bit
                                                                                           integers) */
        ...
        if (bitsavetodisk) { /* Maybe the implementor just wants to look at
                                the 128bit files */
        ...
        sprintf(flags, "object #%d", virtual_nr); /* sprintf: OK (until
                                                                                             399-bit integers) */
        ...
        log1("%d %s %s %s", t[0], flags, flags2, flags3);
        log1("Vitalia room with subzone");

        log1("Converting room #%d to 128bits..", virtual_nr);

        world[room_nr].room_flags[0] = asciiflag_conv(flags2);
        world[room_nr].room_flags[1] = 0;
        world[room_nr].room_flags[2] = 0;
        world[room_nr].room_flags[3] = 0;

        /* In the old-style files, the 3rd item was the sector-type */
        world[room_nr].sector_type = atoi(flags3);

        sprintf(flags, "room #%d", virtual_nr); /* sprintf: OK (until 399-bit
                                                                                           integers) */

        /* No need to scan the other three sections; they're 0 anyway. */
        check_bitvector_names(world[room_nr].room_flags[0], room_bits_count, flags2, "room");
        ...
        log1("SYSERR: Format error in roomflags/sector type of room #%d", virtual_nr);
        exit(1);
    }
    ...
    for (;;) {
        ...
        switch (*line) {
            ...
            case 'S': /* end of room */
                ...
                return;
            case 'O':
                // ignorar as opcoes de sala de vitalia
                log1("Vitalia room with options");
                letter = fread_letter(fl);
                ungetc(letter, fl);
                switch (letter) {
                    case 'N':
...
void setup_dir(FILE *fl, int room, int dir)
{
    ...
    if (!CONFIG_DIAGONAL_DIRS && IS_DIAGONAL(dir)) {
        log1("Warning: Diagonal direction disabled: %s", buf2);
        return;
    }
    ...
    if (!get_line(fl, line)) {
        log1("SYSERR: Format error, %s", buf2);
        exit(1);
    }
    ...
    /* convert dirs with flags in letter format to numbers*/
    t[0] = asciiflag_conv(flags);
    ...
    if (t[0] == 1)
        ...
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_GHOSTPROOF;
    else if (t[0] == 4)
        world[room].dir_option[dir]->exit_info = EX_ISDOOR | EX_HIDDEN;
    else if (t[0] == 5)
        world[room].dir_option[dir]->exit_info = EX_CLIMBUP;
    else if (t[0] == 6)
        world[room].dir_option[dir]->exit_info = EX_CLIMBDOWN;
    else if (t[0] == 7)
...
static void check_start_rooms(void)
{
    if ((r_newbie_start_room = real_room(CONFIG_NEWBIE_START)) == NOWHERE) {
        log1("SYSERR:  Newbie start room does not exist.  Change in config.c.");
        r_newbie_start_room = CONFIG_NEWBIE_START;
    }
    if ((r_dead_start_room = real_room(CONFIG_DEAD_START)) == NOWHERE) {
        log1("SYSERR:  Dead start room does not exist.  Change in config.c.");
        r_dead_start_room = CONFIG_NEWBIE_START;
    }
    if ((r_immort_start_room = real_room(CONFIG_IMMORTAL_START)) == NOWHERE) {
        if (!mini_mud)
            log1("SYSERR:  Warning: Immort start room does not exist.  Change in config.c.");
        r_immort_start_room = CONFIG_NEWBIE_START;
    }
    if ((r_frozen_start_room = real_room(CONFIG_FROZEN_START)) == NOWHERE) {
        if (!mini_mud)
            log1("SYSERR:  Warning: Frozen start room does not exist.  Change in config.c.");
        r_frozen_start_room = CONFIG_NEWBIE_START;
    }
    if ((r_hometown_1 = real_room(CONFIG_HOMETOWN_1)) == NOWHERE) {
        log1("SYSERR:  Hometown 1 does not exist.  Change in config.c.");
        r_hometown_1 = CONFIG_NEWBIE_START;
    }
    if ((r_hometown_2 = real_room(CONFIG_HOMETOWN_2)) == NOWHERE) {
...
/* Resolve vnums into rnums in the zone reset tables. In English: Once all
   of the zone reset tables have been loaded, we resolve the virtual
   numbers into real numbers all at once so we don't have to do it
   repeatedly while the game is running.  This does make adding any room,
   mobile, or object a little more difficult while the game is running.
   Assumes NOWHERE == NOBODY == NOTHING. Assumes sizeof(room_rnum) >=
   (sizeof(mob_rnum) and sizeof(obj_rnum)) */
static void renum_zone_table(void)
{
    int cmd_no;
...
static void parse_simple_mob(FILE *mob_f, int i, int nr)
{
    ...
    if (!get_line(mob_f, line)) {
        log1("SYSERR: Format error in mob #%d, file ended after S flag!", nr);
        exit(1);
    }

    if (sscanf(line, " %d %d %d %dd%d+%d %dd%d+%d ", t, t + 1, t + 2, t + 3, t + 4, t + 5, t + 6, t + 7, t + 8) != 9) {
        log1(
            "SYSERR: Format error in mob #%d, first line after S flag\n"
    ...
    if (!get_line(mob_f, line)) {
        log1(
            "SYSERR: Format error in mob #%d, second line after S flag\n"
    ...
    if (!get_line(mob_f, line)) {
        log1(
            "SYSERR: Format error in last line of mob #%d\n"
            "...expecting line of form '# # #', but file ended!",
            nr);
        exit(1);
    }
...
/* interpret_espec is the function that takes espec keywords and values
   ...
   function! No other changes need to be made anywhere in the code. CASE :
   Requires a parameter through 'value'. */
...
static void interpret_espec(const char *keyword, const char *value, int i, int nr)
{
    ...
    {
        if (mob_proto[i].ai_data) {
            mob_proto[i].ai_data->genetics.trade_tendency = num_arg;
        }
    }
    CASE("GenQuest")
    {
        if (mob_proto[i].ai_data) {
            mob_proto[i].ai_data->genetics.quest_tendency = num_arg;
        }
    }
    CASE("GenAdventurer")
    {
    ...
    if (!matched) {
        log1("SYSERR: Warning: unrecognized espec keyword %s in mob #%d", keyword, nr);
    }
}
...
static void parse_enhanced_mob(FILE *mob_f, int i, int nr)
{
    ...
    while (get_line(mob_f, line)) {
        if (!strcmp(line, "E")) /* end of the enhanced section */
            ...
            log1("SYSERR: Unterminated E section in mob #%d", nr);
            exit(1);
        } else
            parse_espec(line, i, nr);
    }

    log1("SYSERR: Unexpected end of file reached after mob #%d", nr);
    exit(1);
}
...
void parse_mobile(FILE *mob_f, int nr)
{
    ...
    if (tmpptr && *tmpptr)
        if (!str_cmp(fname(tmpptr), "um") || !str_cmp(fname(tmpptr), "uma") || !str_cmp(fname(tmpptr), "o") ||
    ...
    if (!get_line(mob_f, line)) {
        log1(
            "SYSERR: Format error after string section of mob #%d\n"
    ...
    if (((retval = sscanf(line, "%s %s %s %s %s %s %s %s %d %c", f1, f2, f3, f4, f5, f6, f7, f8, t + 2, &letter)) !=
         10) &&
        (bitwarning == TRUE)) {
        /* Let's make the implementor read some, before converting his world
           files. */
        log1("WARNING: Conventional mobile files detected. See config.c.");
        exit(1);
    } else if ((retval == 4) && (bitwarning == FALSE)) {
        log1("Converting mobile #%d to 128bits..", nr);
        ...
        /* This is necessary, since if we have conventional world files,
           &letter is loaded into f4 instead of the letter characters. So what
           we do, is copy f4 into letter. Disadvantage is that &letter cannot
           be longer then 128 characters, but this shouldn't occur anyway. */
        letter = *f4;
        ...
        log1("SYSERR: Format error after string section of mob #%d\n ...expecting line of form '# # # {S | E}'", nr);
        exit(1);
    }
    ...
    if (MOB_FLAGGED(mob_proto + i, MOB_NOTDEADYET)) {
        ...
        log1("SYSERR: Mob #%d has reserved bit MOB_NOTDEADYET set.", nr);
    ...
    switch (UPPER(letter)) {
        ...
        default:
            log1("SYSERR: Unsupported mob type '%c' in mob #%d", letter, nr);
            exit(1);
    }
...
char *parse_object(FILE *obj_f, int nr)
{
    ...
    if ((obj_proto[i].name = fread_string(obj_f, buf2)) == NULL) {
        log1("SYSERR: Null obj name or format error at or near %s", buf2);
        exit(1);
    }
    tmpptr = obj_proto[i].short_description = fread_string(obj_f, buf2);
    if (tmpptr && *tmpptr)
        if (!str_cmp(fname(tmpptr), "a") || !str_cmp(fname(tmpptr), "o") || !str_cmp(fname(tmpptr), "um") ||
    ...
    if (!get_line(obj_f, line)) {
        log1("SYSERR: Expecting first numeric line of %s, but file ended!", buf2);
        exit(1);
    }
    ...
    if (((retval = sscanf(line, " %d %s %s %s %s %s %s %s %s %s %s %s %s", t, f1, f2, f3, f4, f5, f6, f7, f8, f9, f10,
        ...
        /* Let's make the implementor read some, before converting his world
           files. */
        log1("WARNING: Conventional object files detected. Please see config.c.");
        exit(1);
        ...
        log1("Converting object #%d to 128bits..", nr);
        ...
        log1("SYSERR: Format error in first numeric line (expecting 13 args, got %d), %s", retval, buf2);
        exit(1);
    }
    ...
    if (!get_line(obj_f, line)) {
        log1("SYSERR: Expecting second numeric line of %s, but file ended!", buf2);
        exit(1);
    }
    if ((retval = sscanf(line, "%d %d %d %d", t, t + 1, t + 2, t + 3)) != 4) {
        log1("SYSERR: Format error in second numeric line (expecting 4 args, got %d), %s", retval, buf2);
        exit(1);
    }
    ...
    if (!get_line(obj_f, line)) {
        log1("SYSERR: Expecting third numeric line of %s, but file ended!", buf2);
        exit(1);
    }
    if ((retval = sscanf(line, "%d %d %d %d %d", t, t + 1, t + 2, t + 3, t + 4)) != 5) {
        if (retval == 3) {
            ...
            log1("SYSERR: Format error in third numeric line (expecting 5 args, got %d), %s", retval, buf2);
            exit(1);
        }
    }
    ...
    for (;;) {
        if (!get_line(obj_f, line)) {
            log1("SYSERR: Format error in %s", buf2);
            exit(1);
        }
        switch (*line) {
            case 'E':
            ...
            case 'A':
                if (j >= MAX_OBJ_AFFECT) {
                    log1("SYSERR: Too many A fields (%d max), %s", MAX_OBJ_AFFECT, buf2);
                    exit(1);
                }
                if (!get_line(obj_f, line)) {
                    log1(
                        "SYSERR: Format error in 'A' field, %s\n"
            ...
            case '#':
                ...
                return (line);
            default:
                log1("SYSERR: Format error in (%c): %s", *line, buf2);
                exit(1);
        }
    }
}
...
static void load_zones(FILE *fl, char *zonename)
{
    ...
    /* More accurate count. Previous was always 4 or 5 too high. -gg Note that
       if a new zone command is added to reset_zone(), this string will need
       to be updated to suit. - ae. */
    while (get_line(fl, buf))
        if ((strchr("MOPGERDTVC", buf[0]) && buf[1] == ' ') || (buf[0] == 'S' && buf[1] == '\0'))
            num_of_cmds++;

    rewind(fl);

    if (num_of_cmds == 0) {
        log1("SYSERR: %s is empty!", zname);
        exit(1);
    } else
    ...
    if (sscanf(buf, "#%hd", &Z.number) != 1) {
        log1("SYSERR: Format error in %s, line %d", zname, line_num);
        exit(1);
    }
    ...
    if (sscanf(buf, " %hd %hd %d %d %s %s %s %s %d %d %d", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode, zbuf1, zbuf2,
               zbuf3, zbuf4, &Z.min_level, &Z.max_level, &Z.climate) != 11) {
        if (sscanf(buf, " %hd %hd %d %d ", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode) != 4) {
            /* This may be due to the fact that the zone has no builder. So,
               we just attempt to fix this by copying the previous 2 last
               reads into this variable and the last one. */
            log1("SYSERR: Format error in numeric constant line of %s, attempting to fix.", zname);
            if (sscanf(Z.name, " %hd %hd %d %d ", &Z.bot, &Z.top, &Z.lifespan, &Z.reset_mode) != 4) {
                log1("SYSERR: Could not fix previous error, aborting game.");
                exit(1);
            } else {
                free(Z.name);
    ...
    for (;;) {
        ...
        if (zone_fix != TRUE) {
            if ((tmp = get_line(fl, buf)) == 0) {
                log1("SYSERR: Format error in %s - premature end of file", zname);
                exit(1);
            }
        } else
            zone_fix = FALSE;
        ...
        error = 0;
        if (strchr("MOGEPDTVC", ZCMD.command) == NULL) { /* a 3-arg command */
            if (sscanf(ptr, " %d %d %d ", &tmp, &ZCMD.arg1, &ZCMD.arg2) != 3)
                error = 1;
        } else if (ZCMD.command == 'V') { /* a string-arg command */
            if (sscanf(ptr, " %d %d %d %d %79s %79[^\f\n\r\t\v]", &tmp, &ZCMD.arg1, &ZCMD.arg2, &ZCMD.arg3, t1, t2) !=
                6)
                error = 1;
            else {
        ...
        if (error) {
            log1("SYSERR: Format error in %s, line %d: '%s'", zname, line_num, buf);
            exit(1);
        }
        ZCMD.line = line_num;
        cmd_no++;
    ...
    if (num_of_cmds != cmd_no + 1) {
        log1("SYSERR: Zone command count mismatch for %s. Estimated: %d, Actual: %d", zname, num_of_cmds, cmd_no + 1);
        exit(1);
    }
...
static void get_one_line(FILE *fl, char *buf)
{
    if (fgets(buf, READ_SIZE, fl) == NULL) {
        log1("SYSERR: error reading help file: not terminated with $?");
        exit(1);
    }
...
void load_help(FILE *fl, char *name)
{
    ...
    while (*key != '$') {
        ...
        if (entrylen >= sizeof(entry) - 1) {
            ...
            log1("SYSERR: Help entry exceeded buffer space: %.*s", keysize, key);
        ...
        if (*line == '#') {
            if (sscanf(line, "#%d", &el.min_level) != 1) {
                log1("SYSERR: Help entry does not have a min level. %s", key);
                el.min_level = 0;
            }
        }
...
struct char_data *read_mobile(mob_vnum nr, int type) /* and mob_rnum */
{
    ...
    if (type == VIRTUAL) {
        if ((i = real_mobile(nr)) == NOBODY) {
            log1("WARNING: Mobile vnum %d does not exist in database.", nr);
            return (NULL);
        }
    } else
        i = nr;
...
struct obj_data *read_object(obj_vnum nr, int type) /* and obj_rnum */
{
    ...
    if (i == NOTHING || i > top_of_objt) {
        log1("Object (%c) %d does not exist in database.", type == VIRTUAL ? 'V' : 'R', nr);
        return (NULL);
    }
...
void zone_update(void)
{
    ...
    if (((++timer * PULSE_ZONE) / PASSES_PER_SEC) >= 60) {
        /* one minute has passed NOT accurate unless PULSE_ZONE is a multiple
           of PASSES_PER_SEC or a factor of 60 */
...
void reset_zone(zone_rnum zone)
{
    ...
    for (cmd_no = 0; ZCMD.command != 'S'; cmd_no++) {
        ...
        /* This is the list of actual zone commands. If any new zone commands
           are added to the game, be certain to update the list of commands in
           load_zone () so that the counting will still be correct. - ae. */
        switch (ZCMD.command) {
            case '*': /* ignore command */
                last_cmd = 0;
                break;
            ...
            case 'G': /* obj_to_char */
                if (!mob) {
                    ...
                    snprintf(error, sizeof(error), "attempt to give obj #%d to non-existant mob, command disabled",
            ...
            case 'C': /* Check mob */
                if (mob_index[ZCMD.arg2].number < ZCMD.arg1)
                    last_cmd = 1;
                else
                    last_cmd = 0;
                break;
            ...
            case 'E': /* object to equipment list */
                if (!mob) {
                    char error[MAX_INPUT_LENGTH];
                    snprintf(error, sizeof(error), "trying to equip non-existant mob with obj #%d, command disabled",
                             obj_index[ZCMD.arg1].vnum);
                    ZONE_ERROR(error);
                    ZCMD.command = '*';
                    break;
            ...
            case 'D': /* set state of door */
                if (ZCMD.arg2 < 0 || ZCMD.arg2 >= DIR_COUNT || (world[ZCMD.arg1].dir_option[ZCMD.arg2] == NULL)) {
                    char error[MAX_INPUT_LENGTH];
                    snprintf(error, sizeof(error), "door does not exist in room %d - dir %d, command disabled",
...
int is_empty(zone_rnum zone_nr)
{
    ...
    for (i = descriptor_list; i; i = i->next) {
        ...
        if (world[IN_ROOM(i->character)].zone != zone_nr)
            continue;
        /* If an immortal has nohassle off, he counts as present. Added for
           testing zone reset triggers -Welcor */
        if ((!IS_NPC(i->character)) && (GET_LEVEL(i->character) >= LVL_IMMORT) &&
...
char *fread_string(FILE *fl, const char *error)
{
    ...
    *buf = '\0';
    ...
    do {
        if (!fgets(tmp, 512, fl)) {
            log1("SYSERR: fread_string: format error at or near %s", error);
            exit(1);
        }
        ...
        if (length + templength >= MAX_STRING_LENGTH) {
            log1("SYSERR: fread_string: string too large (db.c)");
            log1("%s", error);
            exit(1);
        } else {
...
/* fread_clean_string is the same as fread_string, but skips preceding
   spaces */
char *fread_clean_string(FILE *fl, const char *error)
{
    ...
    *buf = '\0';
    ...
    do {
        if (feof(fl)) {
            log1("%s", "fread_clean_string: EOF encountered on read.");
            return 0;
        }
        c = getc(fl);
    } while (isspace(c));
    ...
    do {
        if (!fgets(tmp, 512, fl)) {
            log1("SYSERR: fread_clean_string: format error at or near %s", error);
            exit(1);
        }
        /* If there is a '~', end the string; else put an "\r\n" over the
           '\n'. */
        ...
        if (length + templength >= MAX_STRING_LENGTH) {
            log1("SYSERR: fread_clean_string: string too large (db.c)");
            log1("%s", error);
            exit(1);
        } else {
            strcat(buf + length, tmp); /* strcat: OK (size checked above) */
...
int fread_number(FILE *fp)
{
    ...
    do {
        if (feof(fp)) {
            log1("%s", "fread_number: EOF encountered on read.");
            return 0;
        }
        c = getc(fp);
    } while (isspace(c));
    ...
    if (!isdigit(c)) {
        log1("fread_number: bad format. (%c)", c);
        return 0;
    }
...
char *fread_line(FILE *fp)
{
    ...
    do {
        if (feof(fp)) {
            log1("fread_line: EOF encountered on read.");
            *pline = '\0';
            return (line);
        }
        c = getc(fp);
    ...
    *pline = '\0';
    ...
    /* Since tildes generally aren't found at the end of lines, this seems
       workable. Will enable reading old configs. */
...
/* Read to end of line from a given file and convert to flag values, then
...
int fread_flags(FILE *fp, int *fg, int fg_size)
{
    ...
    do {
        if (feof(fp)) {
            log1("fread_flags: EOF encountered on read.");
            *pline = '\0';
            return (0);
        }
        c = getc(fp);
    ...
    do {
        ...
        *pline++ = c;
        ln++;
        if (ln >= (MAX_STRING_LENGTH - 1)) {
            log1("fread_flags: line too long");
            break;
        }
    } while ((c != '\n') && (c != '\r'));
...
char *fread_word(FILE *fp)
{
    ...
    do {
        if (feof(fp)) {
            log1("fread_word: EOF encountered on read.");
            word[0] = '\0';
            return word;
        }
        cEnd = getc(fp);
...
void fread_to_eol(FILE *fp)
{
    ...
    do {
        if (feof(fp)) {
            log1("%s", "fread_to_eol: EOF encountered on read.");
            return;
        }
        c = getc(fp);
    } while (c != '\n' && c != '\r');
...
void free_char(struct char_data *ch)
{
    ...
    if (ch->player_specials != NULL && ch->player_specials != &dummy_mob) {
        ...
        if (GET_HOST(ch))
            free(GET_HOST(ch));
        if (IS_NPC(ch))
            log1("SYSERR: Mob %s (#%d) had player_specials allocated!", GET_NAME(ch), GET_MOB_VNUM(ch));
    }
    ...
    /* find_char helper, when free_char is called with a blank character
       struct, ID is set to 0, and has not yet been added to the lookup table.
     */
    if (ch->script_id != 0) {
...
/* Steps: 1: Read contents of a text file. 2: Make sure no one is using
   the pointer in paging. 3: Allocate space. 4: Point 'buf' to it. We
   don't want to free() the string that someone may be viewing in the
   pager. page_string() keeps the internal strdup()'d copy on
   ->showstr_head and it won't care if we delete the original.  Otherwise,
   strings are kept on ->showstr_vector but we'll only match if the
   pointer is to the string we're interested in and not a copy. If someone
   is reading a global copy we're trying to replace, give everybody using
   it a different copy so as to avoid special cases. */
static int file_to_string_alloc(const char *name, char **buf)
{
    int temppage;
...
static int file_to_string(const char *name, char *buf)
{
    ...
    *buf = '\0';
    ...
    if (!(fl = fopen(name, "r"))) {
        log1("SYSERR: reading %s: %s", name, strerror(errno));
        return (-1);
    }
    ...
    for (;;) {
        ...
        if (strlen(buf) + strlen(tmp) + 1 > MAX_STRING_LENGTH) {
            log1("SYSERR: %s: string too big (%d max)", name, MAX_STRING_LENGTH);
            *buf = '\0';
            fclose(fl);
            return (-1);
        }
...
/* Called during character creation after picking character class (and
   then never again for that character). */
void init_char(struct char_data *ch)
{
    ...
    /* Bias the height and weight of the character depending on what gender
       they have chosen. While it is possible to have a tall, heavy female
       it's not as likely as a male. Height is in centimeters. Weight is in
       pounds. The only place they're ever printed (in stock code) is
       SPELL_IDENTIFY. */
    ...
    if ((i = get_ptable_by_name(GET_NAME(ch))) != -1)
        ...
        log1("SYSERR: init_char: Character '%s' not found in player table.", GET_NAME(ch));
    ...
    GET_LOADROOM(ch) = CONFIG_NEWBIE_START;
...
/* Extend later to include more checks and add checks for unknown
   bitvectors. */
static int check_object(struct obj_data *obj)
{
    ...
    if (GET_OBJ_WEIGHT(obj) < 0 && (error = TRUE))
        log1("SYSERR: Object #%d (%s) has negative weight (%d).", GET_OBJ_VNUM(obj), obj->short_description,
             GET_OBJ_WEIGHT(obj));

    if (GET_OBJ_RENT(obj) < 0 && (error = TRUE))
        log1("SYSERR: Object #%d (%s) has negative cost/day (%d).", GET_OBJ_VNUM(obj), obj->short_description,
             GET_OBJ_RENT(obj));
    ...
    switch (GET_OBJ_TYPE(obj)) {
        case ITEM_DRINKCON: {
            ...
            if (search_block(onealias, drinknames, TRUE) < 0 && (error = TRUE))
                log1("SYSERR: Object #%d (%s) doesn't have drink type as last keyword. (%s)", GET_OBJ_VNUM(obj),
                     obj->short_description, obj->name);
        }
            /* Fall through. */
        case ITEM_FOUNTAIN:
            if ((GET_OBJ_VAL(obj, 0) > 0) && (GET_OBJ_VAL(obj, 1) > GET_OBJ_VAL(obj, 0) && (error = TRUE)))
                log1("SYSERR: Object #%d (%s) contains (%d) more than maximum (%d).", GET_OBJ_VNUM(obj),
        ...
        case ITEM_STAFF:
            ...
            if (GET_OBJ_VAL(obj, 2) > GET_OBJ_VAL(obj, 1) && (error = TRUE))
                log1("SYSERR: Object #%d (%s) has more charges (%d) than maximum (%d).", GET_OBJ_VNUM(obj),
            ...
            if (obj->ex_description) {
                ...
                do {
                    if (find_exdesc(onealias, obj->ex_description) && (error = TRUE)) {
                        log1("SYSERR: Object #%d (%s) is type NOTE and has extra description with same name. (%s)",
...
static int check_object_spell_number(struct obj_data *obj, int val)
{
    ...
    if ((spellnum < 0) || (spellnum > TOP_SPELL_DEFINE)) {
        log1("SYSERR: Object #%d (%s) has out of range spell #%d.", GET_OBJ_VNUM(obj), obj->short_description,
             spellnum);
        error = TRUE;
    }
    ...
    if (!spell) {
        log1("SYSERR: Object #%d (%s) has non-existent spell #%d.", GET_OBJ_VNUM(obj), obj->short_description,
             spellnum);
        error = TRUE;
    } else {
        ...
        if (spell->status == unavailable) {
            log1("SYSERR: Object #%d (%s) uses disabled '%s' spell #%d.", GET_OBJ_VNUM(obj), obj->short_description,
                 spellname, +spellnum);
            error = TRUE;
            ...
            if (!str_cmp("Undefined", spellname)) {
                log1("SYSERR: Object #%d (%s) uses '%s' spell #%d.", GET_OBJ_VNUM(obj), obj->short_description,
                     spellname, spellnum);
                error = TRUE;
            }
    }
...
static int check_object_level(struct obj_data *obj, int val)
{
    ...
    if ((GET_OBJ_VAL(obj, val) < 0 || GET_OBJ_VAL(obj, val) > LVL_IMPL) && (error = TRUE))
        log1("SYSERR: Object #%d (%s) has out of range level #%d.", GET_OBJ_VNUM(obj), obj->short_description,
             GET_OBJ_VAL(obj, val));
...
static int check_bitvector_names(bitvector_t bits, size_t namecount, const char *whatami, const char *whatbits)
{
    ...
    for (flagnum = namecount; flagnum < sizeof(bitvector_t) * 8; flagnum++)
        if ((1 << flagnum) & bits) {
            log1("SYSERR: %s has unknown %s flag, bit %d (0 through %d known).", whatami, whatbits, flagnum,
                 (int)namecount - 1);
            error = TRUE;
        }
...
static void load_default_config(void)
{
    /* This function is called only once, at boot-time. We assume config_info
       is empty. -Welcor */
    /* Game play options. */
    ...
    /* Room numbers. */
    CONFIG_NEWBIE_START = newbie_start_room;
    ...
    CONFIG_EMOTION_PAIN_MODERATE_MIN = 5;
    CONFIG_EMOTION_PAIN_MODERATE_MAX = 15;
    CONFIG_EMOTION_PAIN_HEAVY_MIN = 15;
    CONFIG_EMOTION_PAIN_HEAVY_MAX = 30;
    CONFIG_EMOTION_PAIN_MASSIVE_MIN = 30;
    ...
    while (get_line(fl, line)) {
        ...
        switch (LOWER(*tag)) {
            ...
            case 'd':
                ...
                else if (!str_cmp(tag, "dead_start_room"))
                    if (num == -1)
                        CONFIG_DEAD_START = NOWHERE;
                    else
                        CONFIG_DEAD_START = num;
                else if (!str_cmp(tag, "disp_closed_doors"))
