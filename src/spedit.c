...
char *SPELL_OLCING_BY(int vnum)
{
    ...
    for (q = descriptor_list; q; q = q->next)
        if ((q->connected == CON_SPEDIT) && OLC_NUM(q) == vnum)
...
int is_summon_set(struct str_spells *spell) { return spell->summon_mob ? 1 : 0; }
...
{
    if (spell->messages.wear_off)
        return 1;
    if (spell->messages.to_self)
        return 1;
    if (spell->messages.to_vict)
        return 1;
    if (spell->messages.to_room)
        return 1;
...
void spedit_show_warnings(struct descriptor_data *d)
{
    ...
    if ((len < BUFSIZE) && is_apply_set(OLC_SPELL(d)) && !(OLC_SPELL(d)->mag_flags & MAG_AFFECTS))
        len += snprintf(buf + len, BUFSIZE - len,
                        "Magic flags: MAG_AFFECTS is required. (Affects and applies are set).\r\n");
    ...
    if ((len < BUFSIZE) && is_objects_set(OLC_SPELL(d)) && !(OLC_SPELL(d)->mag_flags & MAG_CREATIONS))
        len +=
            snprintf(buf + len, BUFSIZE - len, "Magic flags: MAG_CREATIONS is required. (Create objects is set).\r\n");
...
void spedit_show_messages(struct descriptor_data *d)
{
    ...
    snprintf(buf, sizeof(buf),
             ...
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->messages.wear_off), grn, nrm, cyn,
             EMPTY_STR(OLC_SPELL(d)->messages.to_self), grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->messages.to_vict), grn,
             nrm, cyn, EMPTY_STR(OLC_SPELL(d)->messages.to_room), nrm);
...
void spedit_show_objects(struct descriptor_data *d)
{
    ...
    snprintf(buf, BUFSIZE,
             ...
             "%s3%s) Object : %s%s\r\n"
             "\r\n%sEnter choice (0 to quit) : ",
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->objects[0]), grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->objects[1]),
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->objects[2]), nrm);
...
void spedit_show_dispel(struct descriptor_data *d)
{
    ...
    snprintf(buf, BUFSIZE,
             ...
             "%s3%s) Spell : %s%s\r\n"
             "\r\n%sEnter choice (0 to quit) : ",
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->dispel[0]), grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->dispel[1]), grn,
             nrm, cyn, EMPTY_STR(OLC_SPELL(d)->dispel[2]), nrm);
...
void spedit_show_points(struct descriptor_data *d)
{
    ...
    snprintf(buf, BUFSIZE,
             ...
             "%s5%s) Breath      : %s%s\r\n"
             "\r\n%sEnter choice (0 to quit) : ",
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->points.hp), grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->points.mana),
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->points.move), grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->points.gold),
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->points.breath), nrm);
...
void spedit_show_mobile(struct descriptor_data *d)
{
    ...
    snprintf(buf, BUFSIZE,
             ...
             "\r\n%sEnter choice (0 to quit) : ",
             grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->summon_mob), grn, nrm, cyn, EMPTY_STR(OLC_SPELL(d)->summon_req),
             nrm);
...
void spedit_element_menu(struct descriptor_data *d)
{
    ...
    for (i = 0; i < NUM_ELEMENTS; i++) {
        len = snprintf(buf + total_len, BUFSIZE - total_len, "%s%2d%s) %-15s", grn, i, nrm, get_spell_element_name(i));
        if (len < 0 || total_len + len >= BUFSIZE)
            break;
        total_len += len;
    ...
    snprintf(buf + total_len, BUFSIZE - total_len, "\r\n%sEnter choice (current: %s%s%s) : ", nrm, cyn,
             get_spell_element_name(OLC_SPELL(d)->element), nrm);
    send_to_char(d->character, "%s", buf);
...
void spedit_main_menu(struct descriptor_data *d)
{
    ...
    snprintf(buf, BUFSIZE,
             ...
             prog ? red : grn, nrm, cyn,
             (Q->type == SPELL)   ? "SPELL"
             : (Q->type == SKILL) ? "SKILL"
                                  : "CHANSON",
             nrm, grn, nrm, (Q->status == available) ? nrm : YEL, yel,
             (Q->status == available) ? "Available" : "Unavailable", prog ? red : grn, nrm, yel,
             Q->name ? Q->name : UNDEF_SPELL, prog ? red : grn, nrm, cyn,
             ((Q->min_pos >= 0) && (Q->min_pos < NUM_CHAR_POSITION)) ? position_types[Q->min_pos] : "<ILLEGAL>",
...
void spedit_string_cleanup(struct descriptor_data *d, int action) { spedit_main_menu(d); }
    ...
    while (!feof(fp)) {
        ...
        switch (fct) {
            case DB_CODE_INIT_VARS:
                ...
                ret = fscanf(fp, "%c %d %d %d %d %d %d\n", &Q->type, &Q->vnum, &Q->status, &Q->targ_flags,
                             &Q->mag_flags, &Q->min_pos, &Q->max_dam);
            ...
            case DB_CODE_PROT_6:
                ...
                if (fgets(buf, MAX_STRING_LENGTH, fp)) {
                    ...
                    if (ret == 2)
                        Q->protfrom[x].duration = strdup(buf1);
                }
                if (fgets(buf, MAX_STRING_LENGTH, fp)) {
                    ret = sscanf(buf, "%d %[^\n]", &d1, buf1);
                    ...
                    if (ret == 2)
                        Q->protfrom[x].resist = strdup(buf1);
                }
                break;
            ...
            case DB_CODE_AFFECTS_6:
                ...
                if (fgets(buf, MAX_STRING_LENGTH, fp)) {
                    ...
                    if (ret == 2)
                        Q->applies[x].modifier = strdup(buf1);
                }
                if (fgets(buf, MAX_STRING_LENGTH, fp)) {
                    ret = sscanf(buf, "%d %[^\n]", &d1, buf1);
                    if (d1 != DB_CODE_MARKER)
                    ...
                    if (ret == 2)
                        Q->applies[x].duration = strdup(buf1);
                }
                break;
            ...
            case DB_CODE_CLASS_RA:
                ...
                if (fgets(buf, MAX_STRING_LENGTH, fp)) {
                    ...
                    if (ret == 3)
                        Q->assign[x].prac_gain = strdup(buf1);
                    if (fgets(buf, MAX_STRING_LENGTH, fp)) {
                        ret = sscanf(buf, "%d %[^\n]", &d1, buf1);
                        if (d1 != DB_CODE_MARKER)
                            log1("SYSERR: boot spells: Invalid marker in DB_CODE_CLASS: %d", x + 1);
                        if (ret == 2)
                            Q->assign[x].num_mana = strdup(buf1);
                    }
                }
                break;
...
void spedit_save_to_disk(void)
{
    ...
    for (r = list_spells; r; r = r->next) {
        snprintf(buf, BUFSIZE, "%2d %c %d %d %d %d %d %d\n", DB_CODE_INIT_VARS, r->type, r->vnum, r->status,
                 r->targ_flags, r->mag_flags, r->min_pos, r->max_dam);
        fprintf(fp, "%s", buf);
        ...
        if (r->protfrom[0].prot_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_PROT_1, r->protfrom[0].prot_num,
                     r->protfrom[0].duration, DB_CODE_MARKER, r->protfrom[0].resist);
            fprintf(fp, "%s", buf);
        }

        if (r->protfrom[1].prot_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_PROT_2, r->protfrom[1].prot_num,
                     r->protfrom[1].duration, DB_CODE_MARKER, r->protfrom[1].resist);
            fprintf(fp, "%s", buf);
        }

        if (r->protfrom[2].prot_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_PROT_3, r->protfrom[2].prot_num,
                     r->protfrom[2].duration, DB_CODE_MARKER, r->protfrom[2].resist);
            fprintf(fp, "%s", buf);
        }

        if (r->protfrom[3].prot_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_PROT_4, r->protfrom[3].prot_num,
                     r->protfrom[3].duration, DB_CODE_MARKER, r->protfrom[3].resist);
            fprintf(fp, "%s", buf);
        }

        if (r->protfrom[4].prot_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_PROT_5, r->protfrom[4].prot_num,
                     r->protfrom[4].duration, DB_CODE_MARKER, r->protfrom[4].resist);
            fprintf(fp, "%s", buf);
        }

        if (r->protfrom[5].prot_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_PROT_6, r->protfrom[5].prot_num,
                     r->protfrom[5].duration, DB_CODE_MARKER, r->protfrom[5].resist);
            fprintf(fp, "%s", buf);
        }

        if (r->applies[0].appl_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_AFFECTS_1, r->applies[0].appl_num,
                     NULL_STR(r->applies[0].modifier), DB_CODE_MARKER, r->applies[0].duration);
            fprintf(fp, "%s", buf);
        }

        if (r->applies[1].appl_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_AFFECTS_2, r->applies[1].appl_num,
                     NULL_STR(r->applies[1].modifier), DB_CODE_MARKER, r->applies[1].duration);
            fprintf(fp, "%s", buf);
        }

        if (r->applies[2].appl_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_AFFECTS_3, r->applies[2].appl_num,
                     NULL_STR(r->applies[2].modifier), DB_CODE_MARKER, r->applies[2].duration);
            fprintf(fp, "%s", buf);
        }

        if (r->applies[3].appl_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_AFFECTS_4, r->applies[3].appl_num,
                     NULL_STR(r->applies[3].modifier), DB_CODE_MARKER, r->applies[3].duration);
            fprintf(fp, "%s", buf);
        }

        if (r->applies[4].appl_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_AFFECTS_5, r->applies[4].appl_num,
                     NULL_STR(r->applies[4].modifier), DB_CODE_MARKER, r->applies[4].duration);
            fprintf(fp, "%s", buf);
        }

        if (r->applies[5].appl_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %s\n%2d %s\n", DB_CODE_AFFECTS_6, r->applies[5].appl_num,
                     NULL_STR(r->applies[5].modifier), DB_CODE_MARKER, r->applies[5].duration);
            fprintf(fp, "%s", buf);
        }

        if (r->assign[0].class_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %d %s\n%2d %s\n", DB_CODE_CLASS_MU, r->assign[0].class_num,
                     r->assign[0].level, NULL_STR(r->assign[0].prac_gain), DB_CODE_MARKER,
                     NULL_STR(r->assign[0].num_mana));
            fprintf(fp, "%s", buf);
        }

        if (r->assign[1].class_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %d %s\n%2d %s\n", DB_CODE_CLASS_CL, r->assign[1].class_num,
                     r->assign[1].level, NULL_STR(r->assign[1].prac_gain), DB_CODE_MARKER,
                     NULL_STR(r->assign[1].num_mana));
            fprintf(fp, "%s", buf);
        }

        if (r->assign[2].class_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %d %s\n%2d %s\n", DB_CODE_CLASS_TH, r->assign[2].class_num,
                     r->assign[2].level, NULL_STR(r->assign[2].prac_gain), DB_CODE_MARKER,
                     NULL_STR(r->assign[2].num_mana));
            fprintf(fp, "%s", buf);
        }

        if (r->assign[3].class_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %d %s\n%2d %s\n", DB_CODE_CLASS_WA, r->assign[3].class_num,
                     r->assign[4].level, NULL_STR(r->assign[3].prac_gain), DB_CODE_MARKER,
                     NULL_STR(r->assign[3].num_mana));
            fprintf(fp, "%s", buf);
        }
        if (r->assign[4].class_num != -1) {
            snprintf(buf, BUFSIZE, "%2d %d %d %s\n%2d %s\n", DB_CODE_CLASS_RA, r->assign[4].class_num,
                     r->assign[4].level, NULL_STR(r->assign[4].prac_gain), DB_CODE_MARKER,
                     NULL_STR(r->assign[4].num_mana));
            fprintf(fp, "%s", buf);
        }
    ...
    if (OLC_STORAGE(d)) {
        ...
        if (!vnum) {
            ...
            if (vnum > last_spell_vnum) {
                send_to_char(d->character,
                             "This spell (vnum: %d) is out of bound. Try 'spedit' to create a new spell.\r\n", vnum);
                cleanup_olc(d, CLEANUP_ALL);
                return 0;
            }
            ...
            if (name != UNDEF_SPELL) {
                sprintf(buf, "Do you want to edit '%s' (vnum: %d)? (y/n%s): ", name, vnum, OLC_SEARCH(d) ? ", q" : "");
                send_to_char(d->character, "%s", buf);
...
{
    const char *points[] = {"Hit points += ", "Mana points += ", "Move points += ", "Gold += ", "Breath += "};
    char buf[BUFSIZE];
    char *oldtext;
    ...
    switch (OLC_MODE(d)) {
        case SPEDIT_CONFIRM_SAVESTRING:
            switch (*arg) {
                case 'y':
                case 'Y':
                    to = get_spell_by_vnum(OLC_NUM(d));

                    if (!to) {
                        spedit_save_internally(OLC_SPELL(d));
                        OLC_SPELL(d) =
                            NULL;   // now, it's not a copy, it's saved in memory.
            ...
            return;
        case SPEDIT_GET_TYPE:
            OLC_SPELL(d)->type = (atoi(arg) == 1) ? SKILL : (atoi(arg) == 2) ? CHANSON : SPELL;
            ...
            break;
        case SPEDIT_GET_NUMMANA:
            value = formula_interpreter(d->character, d->character, OLC_NUM(d), FALSE, arg, GET_LEVEL(d->character),
                                        &rts_code);
            if (!rts_code) {
            ...
            return;
        case SPEDIT_GET_NAME:
            SAFE_FREE(OLC_SPELL(d)->name);
            OLC_SPELL(d)->name = (arg && *arg) ? strdup(arg) : strdup(UNDEF_SPELL);
            break;
        case SPEDIT_GET_PROTDUR:
            value = formula_interpreter(d->character, d->character, OLC_NUM(d), FALSE, arg, GET_LEVEL(d->character),
                                        &rts_code);
            if (!rts_code) {
        ...
        case SPEDIT_GET_NUMPRAC:
            ...
            return;
        case SPEDIT_GET_LEVEL:
            OLC_SPELL(d)->assign[OLC_VAL(d)].level = atoi(arg);
            send_to_char(d->character, "Practice gain %% : ");
            OLC_MODE(d) = SPEDIT_GET_NUMPRAC;
            return;
        case SPEDIT_GET_MODIF:
            value = formula_interpreter(d->character, d->character, OLC_NUM(d), FALSE, arg, GET_LEVEL(d->character),
                                        &rts_code);
            if (!rts_code) {
        ...
        case SPEDIT_GET_APPLDUR:
            ...
            if (!rts_code) {
                ...
                send_to_char(d->character, "Duration : ");
            return;
        case SPEDIT_GET_MAXDAM:
            OLC_SPELL(d)->max_dam = atoi(arg);
            break;
        case SPEDIT_GET_DAMAGES:
            value = formula_interpreter(d->character, d->character, OLC_NUM(d), FALSE, arg, GET_LEVEL(d->character),
                                        &rts_code);
            if (!rts_code) {
        ...
        case SPEDIT_GET_DELAY:
            ...
            return;
        case SPEDIT_GET_STATUS:
            if ((x = atoi(arg)) == available)
        ...
        case SPEDIT_SHOW_APPLY:
            ...
            if (!(x = atoi(arg)))
                spedit_apply_menu(d);
            else if ((x < 0) || (x > NUM_APPLIES + NUM_AFF_FLAGS)) {
                send_to_char(d->character, "Invalid choice!\r\n");
                spedit_choose_apply(d);
            } else {
                if (x <= NUM_APPLIES) {
        ...
        case SPEDIT_SHOW_MESSAGES:
            ...
            switch (x) {
                case 0:
                    break;
                case 1:
                    send_to_char(d->character, "Wear off : ");
                    OLC_MODE(d) = SPEDIT_GET_MSG_WEAR_OFF;
                    return;
                case 2:
                    send_to_char(d->character, "To self: ");
                    OLC_MODE(d) = SPEDIT_GET_MSG_TO_SELF;
                    return;
                case 3:
                    send_to_char(d->character, "To victim: ");
                    OLC_MODE(d) = SPEDIT_GET_MSG_TO_VICT;
                    return;
                case 4:
                    send_to_char(d->character, "To room: ");
                    OLC_MODE(d) = SPEDIT_GET_MSG_TO_ROOM;
                    return;
                default:
                    send_to_char(d->character, "Invalid choice!\r\n");
                    spedit_show_messages(d);
        ...
        case SPEDIT_GET_DISPEL:
            ...
            return;
        case SPEDIT_GET_POINTS:
            value = formula_interpreter(d->character, d->character, OLC_NUM(d), FALSE, arg, GET_LEVEL(d->character),
                                        &rts_code);
            if (!rts_code) {
                switch (OLC_VAL(d)) {
                    case 0:
                        SAFE_FREE(OLC_SPELL(d)->points.hp);
                        OLC_SPELL(d)->points.hp = STRDUP(arg);
                        break;
                    case 1:
                        SAFE_FREE(OLC_SPELL(d)->points.mana);
                        OLC_SPELL(d)->points.mana = STRDUP(arg);
                        break;
                    case 2:
                        SAFE_FREE(OLC_SPELL(d)->points.move);
                        OLC_SPELL(d)->points.move = STRDUP(arg);
                        break;
                    case 3:
                        SAFE_FREE(OLC_SPELL(d)->points.gold);
        ...
        case SPEDIT_GET_MOBILE:
            value = formula_interpreter(d->character, d->character, OLC_NUM(d), FALSE, arg, GET_LEVEL(d->character),
                                        &rts_code);
            if (!rts_code) {
                switch (OLC_VAL(d)) {
                    case 0:
                        SAFE_FREE(OLC_SPELL(d)->summon_mob);
                        OLC_SPELL(d)->summon_mob = STRDUP(arg);
                        break;
                    case 1:
                        SAFE_FREE(OLC_SPELL(d)->summon_req);
        ...
        case SPEDIT_SHOW_MOBILE:
            ...
            if (x == 1)
                send_to_char(d->character, "Mobile : ");
            else
                send_to_char(d->character, "Item   : ");
            OLC_VAL(d) = x - 1;
        ...
        case SPEDIT_CONFIRM_EDIT:
            if ((*arg == 'y') || (*arg == 'Y')) {
                send_to_char(d->character, "\r\n");
                spedit_setup2(d);
            } else if (OLC_SEARCH(d) && (*arg != 'q') && (*arg != 'Q')) {
                spedit_setup(d);
            } else
                cleanup_olc(d, CLEANUP_ALL);
            return;
        case SPEDIT_MAIN_MENU:
            ...
            switch (*arg) {
                ...
                case '1': /* if (GET_LEVEL(d->character) < LVL_IMPL) {
                    ...
                    /*  } */
                case '2':
                    send_to_char(d->character, "Spell name : ");
                ...
                case 's':
                case 'S':
                    page_string(d, OLC_SPELL(d)->script, 1);
                    send_to_char(d->character, "\r\n");
                    send_editor_help(d);
...
{
    {
        ...
        else if (!strcmp(argument, "cl"))
            search_by_class = CLASS_CLERIC;
        else if (!strcmp(argument, "th"))
            search_by_class = CLASS_THIEF;
        else if (!strcmp(argument, "wa"))
            search_by_class = CLASS_WARRIOR;
        else if (!strcmp(argument, "ra"))
            search_by_class = CLASS_RANGER;
        else if (!strcmp(argument, "ba"))
            search_by_class = CLASS_BARD;
        else if (!strcmp(argument, "dru"))
            search_by_class = CLASS_DRUID;
        else if (!strcmp(argument, "sk"))
            search_by_skill = 1;
        else if (!strcmp(argument, "off"))
            search_disabled = 1;
        else if (!strcmp(argument, "not"))
            search_not_assigned = 1;
        else if (!strcmp(argument, "spec"))
        ...
        for (ptr = list_spells; ptr; ptr = ptr->next) {
            ...
            if ((mu = (get_spell_class(ptr, CLASS_MAGIC_USER) != -1)))
                strcat(classes, "Mu ");
            if ((search_by_class == CLASS_MAGIC_USER) && !mu)
                continue;

            if ((cl = (get_spell_class(ptr, CLASS_CLERIC) != -1)))
                strcat(classes, "Cl ");
            if ((search_by_class == CLASS_CLERIC) && !cl)
                continue;

            if ((th = (get_spell_class(ptr, CLASS_THIEF) != -1)))
                strcat(classes, "Th ");
            if ((search_by_class == CLASS_THIEF) && !th)
                continue;

            if ((wa = (get_spell_class(ptr, CLASS_WARRIOR) != -1)))
                strcat(classes, "Wa ");
            if ((search_by_class == CLASS_WARRIOR) && !wa)
                continue;
            if ((dru = (get_spell_class(ptr, CLASS_DRUID) != -1)))
                strcat(classes, "Dru ");
            if ((search_by_class == CLASS_DRUID) && !dru)
                continue;
            if ((ba = (get_spell_class(ptr, CLASS_BARD) != -1)))
                strcat(classes, "Ba ");
            if ((search_by_class == CLASS_BARD) && !ba)
                continue;

            if ((ra = (get_spell_class(ptr, CLASS_RANGER) != -1)))
                strcat(classes, "Ra ");
            if ((search_by_class == CLASS_RANGER) && !ra)
                continue;

            if (search_disabled && (ptr->status == available))
                continue;

            if ((mu || cl || th || wa || dru || ba || ra) && search_not_assigned)
                continue;

            if (search_by_skill && (ptr->type == SPELL))
                continue;

            if (search_by_function && !ptr->function)
                continue;

            if (search_by_part_name && !stristr3(ptr->name, argument))
                continue;

            tmp_len = snprintf(buf + len, sizeof(buf) - len,
                               "%s%4d%s) [%s%5d%s]%s %-20s%s %-5s    %s%-3s          %s%-3s         %s%s%s\r\n", QGRN,
                               ++cpt, QNRM, QGRN, ptr->vnum, QNRM, QCYN, ptr->name, QYEL,
                               ptr->type == SPELL   ? "SPELL"
                               : ptr->type == SKILL ? "SKILL"
                                                    : "CHANSON",
                               QNRM, ptr->function ? "Yes" : "No", ptr->status == available ? QGRN : QRED,
                               ptr->status == available ? "Yes" : "No", QCYN, classes, QNRM);
            len += tmp_len;
            if (len > sizeof(buf))
                break;
        }
