...
ACMD(do_oasis_copy)
{
    ...
    struct {
        ...
        const char *command;
        const char *text;
    } oasis_copy_info[] = {{CON_REDIT, real_room, redit_save_internally, redit_setup_existing, "rcopy", "room"},
...
ACMD(do_dig)
{
    ...
    if (!*sdir || !*sroom) {
        send_to_char(ch,
                     "Formato: dig <dir> <sala> - para criar saida\r\n"
