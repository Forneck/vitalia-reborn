...
void look_at_sky(struct char_data *ch)
{
    ...
    for (linenum = 0; linenum < MAP_HEIGHT; linenum++) {
        if ((time_info.hours >= 6 && time_info.hours <= 18) && (linenum < 3 || linenum >= 6))
            continue;
        ...
        for (i = 1; i <= MAP_WIDTH; i++) {
            ...
            if ((time_info.hours >= 6 && time_info.hours <= 18) &&                      /* daytime? */
                ...
                if ((moonphase < 0 && i - 2 - moonpos >= moonphase) || (moonphase > 0 && i + 2 - moonpos <= moonphase))
                    snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%s@", CCWHT(ch, C_SPR));
                else
                    strcat(buf, " ");
                ...
                if (time_info.hours >= 6 && time_info.hours <= 18) { /* daytime */
                    if (i >= sunpos - 2 && i <= sunpos + 2) {
                        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%s%c", CCYEL(ch, C_SPR),
                                 sun_map[linenum - 3][i + 2 - sunpos]);
                    } else
                        strcat(buf, " ");
                } else {
