...
static void prefedit_disp_auto_menu(struct descriptor_data *d);
...
static void prefedit_disp_main_menu(struct descriptor_data *d)
{
    ...
    const char *multi_types[] = {"Desligado", "Breve", "Normal", "Completo", "\n"};
    ...
    sprintf(color_string, "%s",
            multi_types[(PREFEDIT_FLAGGED(PRF_COLOR_1) ? 1 : 0) + (PREFEDIT_FLAGGED(PRF_COLOR_2) ? 2 : 0)]);

    send_to_char(d->character, "\r\n%sPreferencias para %s%s\r\n", CCYEL(d->character, C_NRM), GET_NAME(vict),
    ...
    send_to_char(
        ...
        "\r\n"
        "%sPreferencias\r\n"
        "%sP%s) Prompt : %s[%s%-3s%s]         %sL%s) Tam.Pagina : %s[%s%-3d%s]\r\n"
        "%sC%s) Cor    : %s[%s%-8s%s]    %sS%s) Larg.Pagina : %s[%s%-3d%s]\r\n"
    ...
    if (GET_LEVEL(PREFEDIT_GET_CHAR) >= LVL_IMMORT) {
        ...
        send_to_char(d->character,
                     ...
                     CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM), syslog_string, CCCYN(d->character, C_NRM), CBYEL(d->character, C_NRM),
                     CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM), CCYEL(d->character, C_NRM),
                     ONOFF(PREFEDIT_FLAGGED(PRF_CLS)), CCCYN(d->character, C_NRM),
                     /* Line 2 - show vnums and nowiz */
                     CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM), ONOFF(PREFEDIT_FLAGGED(PRF_SHOWVNUMS)), CCCYN(d->character, C_NRM),
                     CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM), ONOFF(PREFEDIT_FLAGGED(PRF_NOWIZ)), CCCYN(d->character, C_NRM),
                     /* Line 3 - nohassle and holylight */
                     CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM), ONOFF(PREFEDIT_FLAGGED(PRF_NOHASSLE)), CCCYN(d->character, C_NRM),
                     CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM), ONOFF(PREFEDIT_FLAGGED(PRF_HOLYLIGHT)), CCCYN(d->character, C_NRM),
                     /* Line 4 - Verbose */
                     CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
                     CCYEL(d->character, C_NRM), ONOFF(PREFEDIT_FLAGGED(PRF_VERBOSE)), CCCYN(d->character, C_NRM));
...
static void prefedit_disp_toggles_menu(struct descriptor_data *d)
{
    ...
    /* Top of the menu */
    send_to_char(d->character, "Preferencias do Toggle para %s%-20s\r\n", CBGRN(d->character, C_NRM), GET_NAME(vict));

    send_to_char(d->character,
                 "\r\n"
                 "%sMenus\r\n"
                 "%s1%s) Configuracoes Automaticas...\r\n"
                 "\r\n"
                 "%sCanais\r\n",
    ...
    /* Channel toggles */
    send_to_char(d->character,
                 "%sA%s) Gossip   %s[%s%3s%s]      %sD%s) Auction  %s[%s%3s%s]\r\n"
                 "%sB%s) Shout    %s[%s%3s%s]      %sE%s) Gratz    %s[%s%3s%s]\r\n"
                 "%sC%s) Tell     %s[%s%3s%s]\r\n",
                 /* Gossip */
                 CBYEL(d->character, C_NRM), CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
    ...
    send_to_char(
        ...
        ONOFF(PREFEDIT_FLAGGED(PRF_MCCP)), CCCYN(d->character, C_NRM), CBYEL(d->character, C_NRM),
        CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
        PREFEDIT_FLAGGED(PRF_AUTOSIZE) ? CBGRN(d->character, C_NRM) : CBRED(d->character, C_NRM),
...
static void prefedit_disp_auto_menu(struct descriptor_data *d)
{
    ...
    /* Top of the menu */
    send_to_char(d->character, "Configuracoes Automaticas para %s%-20s\r\n", CBGRN(d->character, C_NRM),
                 GET_NAME(vict));
    ...
    send_to_char(d->character,
                 ...
                 ONOFF(PREFEDIT_FLAGGED(PRF_AUTOASSIST)), CCCYN(d->character, C_NRM), CBYEL(d->character, C_NRM),
                 CCNRM(d->character, C_NRM), CCCYN(d->character, C_NRM),
...
static void prefedit_disp_prompt_menu(struct descriptor_data *d)
{
    ...
    if (PREFEDIT_FLAGGED(PRF_DISPAUTO))
        ...
        sprintf(prompt_string, "%s%s%s", PREFEDIT_FLAGGED(PRF_DISPHP) ? "H" : "",
                PREFEDIT_FLAGGED(PRF_DISPMANA) ? "M" : "", PREFEDIT_FLAGGED(PRF_DISPMOVE) ? "V" : "");

    send_to_char(d->character,
                 "%sPreferencias de Prompt\r\n"
                 "%s1%s) Toggle HP\r\n"
                 "%s2%s) Toggle Mana\r\n"
                 "%s3%s) Toggle Moves\r\n"
