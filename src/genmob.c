...
int add_mobile(struct char_data *mob, mob_vnum vnum)
{
    ...
    if ((rnum = real_mobile(vnum)) != NOBODY) {
        ...
        log1("GenOLC: add_mobile: Updated existing mobile #%d.", vnum);
        return rnum;
    }
    ...
    log1("GenOLC: add_mobile: Added mobile %d at index #%d.", vnum, found);
...
#endif
        log1("SYSERR: GenOLC: delete_mobile: Invalid rnum %d.", refpt);
        return NOBODY;
    }
...
int save_mobiles(zone_rnum rznum)
{
...
#endif
        log1("SYSERR: GenOLC: save_mobiles: Invalid real zone number %d. (0-%d)", rznum, top_of_zone_table);
        return FALSE;
    }
    ...
    for (i = genolc_zone_bottom(rznum); i <= zone_table[rznum].top; i++) {
        ...
        if (write_mobile_record(i, &mob_proto[rmob], mobfd) < 0)
            log1("SYSERR: GenOLC: Error writing mobile #%d.", i);
    }
    fputs("$\n", mobfd);
    written = ftell(mobfd);
    fclose(mobfd);
    ...
    log1("GenOLC: '%s' saved, %d bytes written.", usedfname, written);
    return written;
}
...
int write_mobile_espec(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
    ...
    if (mob->ai_data) {
        ...
        if (GET_GENEMOTIONAL_IQ(mob) != 0) /* Only save if initialized (not 0) */
...
int write_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
    ...
    if (n < MAX_STRING_LENGTH) {
        ...
        if (write_mobile_mobprog(mvnum, mob, fd) < 0)
            log1("SYSERR: GenOLC: Error writing MobProgs for mobile #%d.", mvnum);
#endif
    } else {
        mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Could not save mobile #%d due to size (%d > maximum of %d)", mvnum, n,
               MAX_STRING_LENGTH);
    }
