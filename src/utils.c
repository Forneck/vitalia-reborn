...
int rand_number(int from, int to)
{
  ...
  if (from > to) {
    ...
    from = to;
    to = tmp;
    log1("SYSERR: rand_number() should be called with lowest, then highest. (%d, %d), not (%d, %d).", from, to, to, from);
  }
...
int get_filename(char *filename, size_t fbufsize, int mode, const char *orig_name)
{
  ...
  if (orig_name == NULL || *orig_name == '\0' || filename == NULL) {
    log1("SYSERR: NULL pointer or empty string passed to get_filename(), %p or %p.",
...
const char *gauge(char *buf, int type, float pos, float total)
{
  ...
  sprintf(buf, "\tW[\t%c%s\tW]\tn",
	  type & 1 ? 'g' :
	  type & 2 ? gauge_table[i].clrmx : gauge_table[i].clr,
...
{
  if (align >=  450) return ("\tW[\tR-----\tG|\tB>>>>>\tW]\tn");
  if (align >=  350) return ("\tW[\tR-----\tG|\tB>>>>-\tW]\tn");
  if (align >=  250) return ("\tW[\tR-----\tG|>>>\tB--\tW]\tn");
  if (align >=  150) return ("\tW[\tR-----\tG|>>\tB---\tW]\tn");
  if (align >=   50) return ("\tW[\tR-----\tG|>\tB----\tW]\tn");
  if (align <= -450) return ("\tW[\tR<<<<<\tG|\tB-----\tW]\tn");
  if (align <= -350) return ("\tW[\tR-<<<<\tG|\tB-----\tW]\tn");
  if (align <= -250) return ("\tW[\tR--\tG<<<|\tB-----\tW]\tn");
  if (align <= -150) return ("\tW[\tR---\tG<<|\tB-----\tW]\tn");
