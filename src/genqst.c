...
int copy_quest_strings(struct aq_data *to, struct aq_data *from)
{
    if (from == NULL || to == NULL) {
        log1("SYSERR: GenQST: copy_quest_strings: Null values passed.");
        return FALSE;
    }
...
int delete_quest(qst_rnum rnum)
{
    ...
    log1("GenOLC: delete_quest: Deleting quest #%d (%s).", QST_NUM(rnum), QST_NAME(rnum));
...
#endif
        log1("SYSERR: GenOLC: save_quests: Invalid zone number %d passed! (0-%d)", zone_num, top_of_zone_table);
        return FALSE;
    }
    ...
    for (i = genolc_zone_bottom(zone_num); i <= zone_table[zone_num].top; i++) {
        ...
        if ((rnum = real_quest(i)) != NOTHING) {
            ...
            int n = snprintf(
                ...
                "%d %d %d\n"
                "S\n",
                QST_NUM(rnum), QST_NAME(rnum) ? QST_NAME(rnum) : "Untitled", STRING_TERMINATOR, quest_desc,
                STRING_TERMINATOR, quest_info, STRING_TERMINATOR, quest_done, STRING_TERMINATOR, quest_quit,
                STRING_TERMINATOR, QST_TYPE(rnum), QST_MASTER(rnum) == NOBODY ? -1 : QST_MASTER(rnum), quest_flags,
                QST_TARGET(rnum) == NOTHING ? -1 : QST_TARGET(rnum), QST_PREV(rnum) == NOTHING ? -1 : QST_PREV(rnum),
                QST_NEXT(rnum) == NOTHING ? -1 : QST_NEXT(rnum), QST_PREREQ(rnum) == NOTHING ? -1 : QST_PREREQ(rnum),
                QST_POINTS(rnum), QST_PENALTY(rnum), QST_MINLEVEL(rnum), QST_MAXLEVEL(rnum), QST_TIME(rnum),
                QST_RETURNMOB(rnum) == NOBODY ? -1 : QST_RETURNMOB(rnum), QST_QUANTITY(rnum), QST_GOLD(rnum),
                QST_EXP(rnum), QST_OBJ(rnum));
