...
void load_messages(void)
{
    ...
    if (!(fl = fopen(MESS_FILE, "r"))) {
        log1("SYSERR: Error reading combat message file %s: %s", MESS_FILE, strerror(errno));
        exit(1);
    }
    ...
    while (fgets(chk, 128, fl)) {
        while (*chk == '\n' || *chk == '*') {
            if (fgets(chk, 128, fl) == NULL) {
                if (feof(fl))
                    ...
                    log1("SYSERR: Error reading combat message file %s", MESS_FILE);
                exit(1);
            }
        }
        ...
        while (*chk == 'M') {
            if (fgets(chk, 128, fl) == NULL) {
                if (feof(fl)) {
                    log1("SYSERR: Unexpected end of file reading combat message file %s", MESS_FILE);
                    break;
                } else if (ferror(fl))
                    log1("SYSERR: Error reading combat message file %s: %s", MESS_FILE, strerror(errno));
                else
            ...
            for (i = 0; (i < MAX_MESSAGES) && (fight_messages[i].a_type != type) && (fight_messages[i].a_type); i++)
                ;
            if (i >= MAX_MESSAGES) {
                log1("SYSERR: Too many combat messages.  Increase MAX_MESSAGES and recompile.");
                exit(1);
            }
    ...
    fclose(fl);
    log1("Loaded %d Combat Messages...", i);
}
...
static void show_messages(struct char_data *ch)
{
    ...
    for (i = 0; i < MAX_MESSAGES / 2; i++, half++)
        if (fight_messages[i].msg != NULL && len < sizeof(buf)) {
            ...
            len += snprintf(buf + len, sizeof(buf) - len, "%-2d) [%-3d] %d, %-18s%s", i, fight_messages[i].a_type,
                            fight_messages[i].number_of_attacks, get_spell_name(fight_messages[i].a_type),
            ...
            if (half < MAX_MESSAGES && fight_messages[half].msg)
                len += snprintf(buf + len, sizeof(buf) - len, "%-2d) [%-3d] %d, %-18s\r\n", half,
                                ...
                                get_spell_name(fight_messages[half].a_type));
        }
...
void save_messages_to_disk(void)
{
    ...
    if (!(fp = fopen(MESS_FILE, "w"))) {
        log1("SYSERR: Error writing combat message file %s: %s", MESS_FILE, strerror(errno));
        exit(1);
    }
    ...
    for (i = 0; i < MAX_MESSAGES; i++) {
        ...
        for (msg = fight_messages[i].msg; msg; msg = msg->next) {
            fprintf(fp,
                    ...
                    "%s\n"
                    "\n",
                    fight_messages[i].a_type, PRINT_MSG(msg->die_msg.attacker_msg), PRINT_MSG(msg->die_msg.victim_msg),
                    PRINT_MSG(msg->die_msg.room_msg), PRINT_MSG(msg->miss_msg.attacker_msg),
                    PRINT_MSG(msg->miss_msg.victim_msg), PRINT_MSG(msg->miss_msg.room_msg),
                    PRINT_MSG(msg->hit_msg.attacker_msg), PRINT_MSG(msg->hit_msg.victim_msg),
                    PRINT_MSG(msg->hit_msg.room_msg), PRINT_MSG(msg->god_msg.attacker_msg),
                    PRINT_MSG(msg->god_msg.victim_msg), PRINT_MSG(msg->god_msg.room_msg));
        }
    }
...
static void msgedit_main_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d, "%sMsg Edit: %s[%s%dx%d%s] [%s$n: Attacker | $N: Victim%s]%s\r\n", cyn, grn, yel, OLC_NUM(d),
                    OLC_MSG_LIST(d)->number_of_attacks, grn, yel, grn, nrm);
    write_to_output(d, "%s1%s) %sAction Type: %s%d %s[%s%s%s]%s\r\n", grn, yel, cyn, yel, OLC_MSG_LIST(d)->a_type, grn,
                    yel, get_spell_name(OLC_MSG_LIST(d)->a_type), grn, nrm);
    ...
    write_to_output(d,
                    ...
                    PRINT_MSG(OLC_MSG(d)->die_msg.victim_msg), grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->die_msg.room_msg));
    ...
    write_to_output(d,
                    ...
                    PRINT_MSG(OLC_MSG(d)->hit_msg.victim_msg), grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->hit_msg.room_msg));
    ...
    write_to_output(d,
                    ...
                    PRINT_MSG(OLC_MSG(d)->god_msg.victim_msg), grn, yel, nrm, PRINT_MSG(OLC_MSG(d)->god_msg.room_msg));
