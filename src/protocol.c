...
static void ReportBug(const char *apText) { log1("%s", apText); }
...
void MSDPSend(descriptor_t *apDescriptor, variable_t aMSDP)
{
    ...
    if (aMSDP > eMSDP_NONE && aMSDP < eMSDP_MAX) {
        ...
        if (VariableNameTable[aMSDP].bString) {
            ...
            int RequiredBuffer =
                strlen(VariableNameTable[aMSDP].pName) + strlen(pProtocol->pVariables[aMSDP]->pValueString) + 12;
            ...
            if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
                ...
                sprintf(MSDPBuffer, "%c%c%c%c%s%c%s%c%c", IAC, SB, TELOPT_MSDP, MSDP_VAR,
                        VariableNameTable[aMSDP].pName, MSDP_VAL, pProtocol->pVariables[aMSDP]->pValueString, IAC, SE);
            } else if (pProtocol->bATCP) {
                sprintf(MSDPBuffer, "%c%c%cMSDP.%s %s%c%c", IAC, SB, TELOPT_ATCP, VariableNameTable[aMSDP].pName,
        ...
        } else /* It's an integer, not a string */
        {
            if (pProtocol->bMSDP) {
                sprintf(MSDPBuffer, "%c%c%c%c%s%c%d%c%c", IAC, SB, TELOPT_MSDP, MSDP_VAR,
                        VariableNameTable[aMSDP].pName, MSDP_VAL, pProtocol->pVariables[aMSDP]->ValueInt, IAC, SE);
            } else if (pProtocol->bATCP) {
                sprintf(MSDPBuffer, "%c%c%cMSDP.%s %d%c%c", IAC, SB, TELOPT_ATCP, VariableNameTable[aMSDP].pName,
...
void MSDPSendPair(descriptor_t *apDescriptor, const char *apVariable, const char *apValue)
{
    ...
    if (apVariable != NULL && apValue != NULL) {
        ...
        if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
            ...
            sprintf(MSDPBuffer, "%c%c%c%c%s%c%s%c%c", IAC, SB, TELOPT_MSDP, MSDP_VAR, apVariable, MSDP_VAL, apValue,
                    IAC, SE);
        } else if (pProtocol->bATCP) {
...
void MSDPSendList(descriptor_t *apDescriptor, const char *apVariable, const char *apValue)
{
    ...
    if (apVariable != NULL && apValue != NULL) {
        ...
        if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
            ...
            int i; /* Loop counter */
            sprintf(MSDPBuffer, "%c%c%c%c%s%c%c%c%s%c%c%c", IAC, SB, TELOPT_MSDP, MSDP_VAR, apVariable, MSDP_VAL,
...
void GMCPSend(descriptor_t *apDescriptor, const char *apMessage)
{
    ...
    if (pProtocol != NULL && pProtocol->bGMCP && apMessage != NULL) {
        ...
        if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
            ...
            sprintf(ErrorBuffer, "GMCPSend: Message length %d bytes (exceeds MAX_VARIABLE_LENGTH of %d).\n",
                    RequiredBuffer, MAX_VARIABLE_LENGTH);
            ReportBug(ErrorBuffer);
            return;
        }

        sprintf(GMCPBuffer, "%c%c%c%s%c%c", IAC, SB, TELOPT_GMCP, apMessage, IAC, SE);
...
void GMCPSendData(descriptor_t *apDescriptor, const char *apPackage, const char *apData)
{
    if (apPackage != NULL && apData != NULL) {
        ...
        if (RequiredBuffer >= MAX_VARIABLE_LENGTH) {
            char ErrorBuffer[256];
            sprintf(ErrorBuffer, "GMCPSendData: Combined length %d bytes (exceeds MAX_VARIABLE_LENGTH of %d).\n",
                    RequiredBuffer, MAX_VARIABLE_LENGTH);
            ReportBug(ErrorBuffer);
            return;
        }
...
#ifdef USING_MCCP
        const char WillMCCP2[] = {(char)IAC, (char)WILL, TELOPT_MCCP2, '\0'};
        const char WillMCCP3[] = {(char)IAC, (char)WILL, TELOPT_MCCP3, '\0'};
#endif   // USING_MCCP
...
#ifdef USING_MCCP
        Write(apDescriptor, WillMCCP2);
        Write(apDescriptor, WillMCCP3);
#endif   // USING_MCCP
    }
}
...
static void PerformHandshake(descriptor_t *apDescriptor, char aCmd, char aProtocol)
{
    ...
    switch (aProtocol) {
        ...
        case (char)TELOPT_CHARSET:
            if (aCmd == (char)WILL) {
                char charset_utf8[] = {(char)IAC, (char)SB, TELOPT_CHARSET, 1,        ' ', 'U', 'T', 'F',
        ...
        case (char)TELOPT_GMCP:
            if (aCmd == (char)DO) {
                ...
                GMCPSend(apDescriptor,
                         "Core.Supports.Set [\"Char 1\", \"Char.Items 1\", \"Char.Skills 1\", \"Room 1\", "
                         "\"Comm.Channel 1\"]");
            } else if (aCmd == (char)DONT)
                pProtocol->bGMCP = false;
            break;
    ...
    switch (aCmd) {
        case (char)TELOPT_TTYPE:
            if (pProtocol->bTTYPE) {
                ...
                if (MatchString(pClientName, "GMud") ||
                    MatchString(pClientName,
                                "Pueblo")) { /* We know for certain that this client does not have support */
...
void *z_alloc(void *opaque, uint items, uint size) { return calloc(items, size); }

void z_free(void *opaque, void *address) { return free(address); }
