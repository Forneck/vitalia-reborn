...
void add_var(struct trig_var_data **var_list, const char *name, const char *value, long id)
{
    ...
    if (strchr(name, '.')) {
        log1("add_var() : Attempt to add illegal var: %s", name);
        return;
    }
                      ...
                      char *subfield, char *str, size_t slen)
{
    ...
    *str = '\0';
    ...
    else if (vd && text_processed(field, subfield, vd, str, slen))
        return;
            ...
            if (!str_cmp(var, "self")) {
                ...
                *str = '\0';
                ...
                if (!str_cmp(field, "qp") && IS_HAPPYHOUR)
                    ...
                    snprintf(str, slen, "%d", HAPPY_TIME);
                return;
                ...
                if (!str_cmp(field, "char")) {
                    ...
                    if (type == MOB_TRIGGER) {
                        ...
                        for (c = world[IN_ROOM(ch)].people; c; c = c->next_in_room)
                            if ((c != ch) && valid_dg_target(c, DG_ALLOW_GODS) && CAN_SEE(ch, c)) {
                ...
                case 'c':
                    if (!str_cmp(field, "canbeseen")) {
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (c->real_abils.cha > max)
                                c->real_abils.cha = max;
                            if (c->real_abils.cha < 3)
                                c->real_abils.cha = 3;
                            affect_total(c);
                        }
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (c->real_abils.con > max)
                                c->real_abils.con = max;
                            if (c->real_abils.con < 3)
                                c->real_abils.con = 3;
                            affect_total(c);
                        }
                ...
                case 'd':
                    if (!str_cmp(field, "damroll")) {
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (c->real_abils.dex > max)
                                c->real_abils.dex = max;
                            if (c->real_abils.dex < 3)
                                c->real_abils.dex = 3;
                            affect_total(c);
                        }
                ...
                case 'i':
                    ...
                    /* new check for pc/npc status */
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (c->real_abils.intel > max)
                                c->real_abils.intel = max;
                            if (c->real_abils.intel < 3)
                                c->real_abils.intel = 3;
                            affect_total(c);
                        }
                    ...
                    if (!str_cmp(field, "saving_breath")) {
                        ...
                        *str = '\0'; /* so the parser know we recognize 'skillset' as a field */
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (c->real_abils.str > max)
                                c->real_abils.str = max;
                            if (c->real_abils.str < 3)
                                c->real_abils.str = 3;
                            affect_total(c);
                        }
                        ...
                        if (GET_STR(c) >= 18) {
                            if (subfield && *subfield) {
                                ...
                                if (c->real_abils.str_add > 100)
                                    c->real_abils.str_add = 100;
                                if (c->real_abils.str_add < 0)
                                    c->real_abils.str_add = 0;
                                affect_total(c);
                            }
                ...
                case 'w':
                    if (!str_cmp(field, "weight"))
                        ...
                        if (subfield && *subfield) {
                            ...
                            if (c->real_abils.wis > max)
                                c->real_abils.wis = max;
                            if (c->real_abils.wis < 3)
                                c->real_abils.wis = 3;
                            affect_total(c);
                        }
            ...
            if (*str == '\x1') { /* no match found in switch */
                if (SCRIPT(c)) {
                    ...
                    if (vd)
                        ...
                        *str = '\0';
                        script_log("Trigger: %s, VNum %d. unknown char field: '%s'", GET_TRIG_NAME(trig),
                                   GET_TRIG_VNUM(trig), field);
                    }
                } else {
                    *str = '\0';
                    script_log("Trigger: %s, VNum %d. unknown char field: '%s'", GET_TRIG_NAME(trig),
            ...
            *str = '\x1';
            ...
            if (*str == '\x1') { /* no match in switch */
                if (SCRIPT(o)) { /* check for global var */
                    ...
                    if (vd)
                        ...
                        *str = '\0';
                        script_log("Trigger: %s, VNum %d, type: %d. unknown object field: '%s'", GET_TRIG_NAME(trig),
                                   GET_TRIG_VNUM(trig), type, field);
                    }
                } else {
                    *str = '\0';
                    script_log("Trigger: %s, VNum %d, type: %d. unknown object field: '%s'", GET_TRIG_NAME(trig),
            ...
            if (r->number == 0) {
                if (!SCRIPT(r)) {
                    *str = '\0';
                    script_log(
                        "Trigger: %s, Vnum %d, type %d. Trying to access Global var list of void. Apparently this has "
                        "not been set up!",
                ...
                const char *sky_look[] = {"sunny", "cloudy", "rainy", "lightning", "snowing"};
                    ...
                    if (subfield && *subfield) {
                        if (!str_cmp(subfield, "vnum"))
                            ...
                            if (R_EXIT(r, NORTH)->to_room != NOWHERE)
                                snprintf(str, slen, "%c%ld", UID_CHAR,
                                         room_script_id(world + R_EXIT(r, NORTH)->to_room));
                            else
                                *str = '\0';
                        }
                    ...
                    if (subfield && *subfield) {
                        if (!str_cmp(subfield, "vnum"))
                            ...
                            if (R_EXIT(r, EAST)->to_room != NOWHERE)
                                snprintf(str, slen, "%c%ld", UID_CHAR,
                                         room_script_id(world + R_EXIT(r, EAST)->to_room));
                            else
                                *str = '\0';
                        }
                    } else /* no subfield - default to bits */
                    ...
                    if (subfield && *subfield) {
                        if (!str_cmp(subfield, "vnum"))
                            ...
                            if (R_EXIT(r, SOUTH)->to_room != NOWHERE)
                                snprintf(str, slen, "%c%ld", UID_CHAR,
                                         room_script_id(world + R_EXIT(r, SOUTH)->to_room));
                            else
                                *str = '\0';
                        }
                    } else /* no subfield - default to bits */
                    ...
                    if (subfield && *subfield) {
                        if (!str_cmp(subfield, "vnum"))
                            ...
                            if (R_EXIT(r, WEST)->to_room != NOWHERE)
                                snprintf(str, slen, "%c%ld", UID_CHAR,
                                         room_script_id(world + R_EXIT(r, WEST)->to_room));
                            else
                                *str = '\0';
                        }
                    } else /* no subfield - default to bits */
                    ...
                    if (subfield && *subfield) {
                        if (!str_cmp(subfield, "vnum"))
                            ...
                            if (R_EXIT(r, DOWN)->to_room != NOWHERE)
                                snprintf(str, slen, "%c%ld", UID_CHAR,
                                         room_script_id(world + R_EXIT(r, DOWN)->to_room));
                            else
                                *str = '\0';
                        }
                    } else /* no subfield - default to bits */
                ...
                if (SCRIPT(r)) { /* check for global var */
                    ...
                    if (vd)
                        ...
                        *str = '\0';
                        script_log("Trigger: %s, VNum %d, type: %d. unknown room field: '%s'", GET_TRIG_NAME(trig),
                                   GET_TRIG_VNUM(trig), type, field);
                    }
                } else {
                    *str = '\0';
