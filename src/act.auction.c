...
ACMD(do_auction)
{
    ...
    if (is_abbrev(subcommand, "criar")) {
        ...
        /* Create the auction */
        auction = create_auction(ch, obj, AUCTION_TYPE_ENGLISH, AUCTION_OPEN, 
                               bid_amount, bid_amount, 1800); /* 30 minutes */
        
        if (auction) {
            auction->state = AUCTION_ACTIVE;
            send_to_char(ch, "Leilão #%d criado para %s com preço inicial de %d moedas.\r\n",
                        auction->auction_id, obj->short_description, bid_amount);
    ...
    if (is_abbrev(subcommand, "dar")) {
        ...
        if (place_bid(ch, auction_id, bid_amount)) {
            ...
            snprintf(buf, sizeof(buf), "%s deu um lance de %d moedas no leilão #%d (%s).",
                   GET_NAME(ch), bid_amount, auction_id, auction->item_name);
    ...
    if (is_abbrev(subcommand, "passe")) {
        /* Only Belchior can give auction passes */
        send_to_char(ch, "Você precisa falar com Belchior para obter um passe de leilão.\r\n");
        return;
    }
