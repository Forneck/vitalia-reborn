...
void trig_data_copy(trig_data *this_data, const trig_data *trg)
{
    ...
    if (trg->name)
        ...
        log1("Trigger with no name! (%d)", trg->nr);
    }
        ...
        switch (type) {
            case MOB_TRIGGER:
                mudlog(BRF, LVL_BUILDER, TRUE,
                       "SYSERR: dg_read_trigger: Trigger vnum #%d asked for but non-existant! (mob: %s - %d)", vnum,
                       GET_NAME((char_data *)proto), GET_MOB_VNUM((char_data *)proto));
                break;
            case WLD_TRIGGER:
                mudlog(BRF, LVL_BUILDER, TRUE,
                       "SYSERR: dg_read_trigger: Trigger vnum #%d asked for but non-existant! (room:%d)", vnum,
                       GET_ROOM_VNUM(((room_data *)proto)->number));
                break;
            default:
                mudlog(BRF, LVL_BUILDER, TRUE,
    ...
    switch (type) {
        ...
        case WLD_TRIGGER:
            ...
            if (rnum != NOTHING) {
                ...
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: non-existant trigger #%d assigned to room #%d", vnum,
                       room->number);
            }
            break;
        default:
...
void dg_obj_trigger(char *line, struct obj_data *obj)
{
    ...
    if (count != 2) {
        mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: dg_obj_trigger() : Error assigning trigger! - Line was:\n  %s", line);
        return;
    }

    rnum = real_trigger(vnum);
    if (rnum == NOTHING) {
        mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Trigger vnum #%d asked for but non-existant! (Object: %s - %d)", vnum,
    ...
    switch (type) {
        case MOB_TRIGGER:
            ...
            while (trg_proto) {
                ...
                if (rnum == NOTHING) {
                    mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: trigger #%d non-existant, for mob #%d", trg_proto->vnum,
        ...
        case OBJ_TRIGGER:
            ...
            while (trg_proto) {
                rnum = real_trigger(trg_proto->vnum);
                if (rnum == NOTHING) {
                    log1("SYSERR: trigger #%d non-existant, for obj #%d", trg_proto->vnum,
        ...
        case WLD_TRIGGER:
            ...
            while (trg_proto) {
                rnum = real_trigger(trg_proto->vnum);
                if (rnum == NOTHING) {
                    mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: trigger #%d non-existant, for room #%d", trg_proto->vnum,
                           room->number);
                } else {
                    if (!SCRIPT(room))
            ...
            break;
        default:
            mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: unknown type for assign_triggers()");
            break;
    }
}