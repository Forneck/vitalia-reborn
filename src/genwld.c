...
room_rnum add_room(struct room_data *room)
{
    ...
    if ((i = real_room(room->number)) != NOWHERE) {
        ...
        log1("GenOLC: add_room: Updated existing room #%d.", room->number);
        return i;
    }
    ...
    log1("GenOLC: add_room: Added room %d at index #%d.", room->number, found);
...
int delete_room(room_rnum rnum)
{
    ...
    log1("GenOLC: delete_room: Deleting room #%d (%s).", room->number, room->name);

    if (r_newbie_start_room == rnum) {
        log1("WARNING: GenOLC: delete_room: Deleting mortal start room!");
        r_newbie_start_room = 0; /* The Void */
    }
    if (r_immort_start_room == rnum) {
        log1("WARNING: GenOLC: delete_room: Deleting immortal start room!");
    ...
    if (r_hometown_4 == rnum) {
        log1("WARNING: GenOLC: delete_room: Deleting hometown 4!");
        r_hometown_4 = 0; /* The Void */
    }
    ...
    if (r_ress_room_4 == rnum) {
        log1("WARNING: GenOLC: delete_room: Deleting ress room 4!");
        r_ress_room_4 = 0; /* The Void */
    }
...
#endif
        log1("SYSERR: GenOLC: save_rooms: Invalid zone number %d passed! (0-%d)", rzone, top_of_zone_table);
        return FALSE;
    }

    log1("GenOLC: save_rooms: Saving rooms in zone #%d (%d-%d).", zone_table[rzone].number, genolc_zone_bottom(rzone),
    ...
    for (i = genolc_zone_bottom(rzone); i <= zone_table[rzone].top; i++) {
        ...
        if ((rnum = real_room(i)) != NOWHERE) {
            ...
            int n = snprintf(buf2, MAX_STRING_LENGTH,
                             ...
                             STRING_TERMINATOR, zone_table[room->zone].number, room->room_flags[0], room->room_flags[1],
                             room->room_flags[2], room->room_flags[3], room->sector_type);

            if (n >= MAX_STRING_LENGTH) {
                mudlog(BRF, LVL_BUILDER, TRUE, "SYSERR: Could not save room #%d due to size (%d > maximum of %d).",
...
int copy_room_strings(struct room_data *dest, struct room_data *source)
{
    ...
    if (dest == NULL || source == NULL) {
        log1("SYSERR: GenOLC: copy_room_strings: NULL values passed.");
        return FALSE;
    }
