...
int save_config(IDXTYPE nowhere)
{
	...
	fprintf(fl, "* Does weather affect spell effectiveness?\n" "weather_affects_spells = %d\n\n",
			CONFIG_WEATHER_AFFECTS_SPELLS);
	fprintf(fl, "* Does weather affect spells based on school?\n" "school_weather_affects = %d\n\n",
	...
	fprintf(fl, "* The virtual number of the room that frozen people should enter at.\n"
			"frozen_start_room = %d\n\n", CONFIG_FROZEN_START);
				fprintf(fl, "* The virtual number of the room that dead people should enter at.\n"
			"dead_start_room = %d\n\n", CONFIG_DEAD_START);
...
static void cedit_disp_game_play_options(struct descriptor_data *d)
{
	...
	write_to_output(d, "\r\n\r\n"
					...
					CHECK_VAR(OLC_CONFIG(d)->play.fit_evolve), grn, nrm, cyn,
					CHECK_VAR(OLC_CONFIG(d)->play.school_weather_affects), grn, nrm, cyn,
...
static void cedit_disp_crash_save_options(struct descriptor_data *d)
{
	...
	write_to_output(d, "\r\n\r\n"
					...
					grn, nrm, cyn, OLC_CONFIG(d)->csd.rent_file_timeout, grn, nrm);
...
static void cedit_disp_operation_options(struct descriptor_data *d)
{
	...
	write_to_output(d, "\r\n\r\n"
					...
					grn, nrm, cyn,
					OLC_CONFIG(d)->operation.DFLT_DIR ? OLC_CONFIG(d)->operation.
					DFLT_DIR : "<None>", grn, nrm, cyn,
					OLC_CONFIG(d)->operation.LOGNAME ? OLC_CONFIG(d)->operation.LOGNAME : "<None>",
					grn, nrm, cyn, OLC_CONFIG(d)->operation.max_playing, grn, nrm, cyn,
					OLC_CONFIG(d)->operation.max_filesize, grn, nrm, cyn,
					OLC_CONFIG(d)->operation.max_bad_pws, grn, nrm, cyn,
					YESNO(OLC_CONFIG(d)->operation.siteok_everyone), grn, nrm, cyn,
					YESNO(OLC_CONFIG(d)->operation.nameserver_is_slow), grn, nrm, cyn,
					YESNO(OLC_CONFIG(d)->operation.use_new_socials), grn, nrm, cyn,
					YESNO(OLC_CONFIG(d)->operation.auto_save_olc), grn, nrm, cyn,
					OLC_CONFIG(d)->operation.MENU ? OLC_CONFIG(d)->operation.MENU : "<None>", grn,
					nrm, cyn,
					OLC_CONFIG(d)->operation.WELC_MESSG ? OLC_CONFIG(d)->operation.
					WELC_MESSG : "<None>", grn, nrm, cyn,
					OLC_CONFIG(d)->operation.START_MESSG ? OLC_CONFIG(d)->operation.
					START_MESSG : "<None>", grn, nrm, cyn,
					OLC_CONFIG(d)->operation.medit_advanced ? "Advanced" : "Standard", grn, nrm,
					cyn, OLC_CONFIG(d)->operation.ibt_autosave ? "Yes" : "No", grn, nrm, cyn,
					OLC_CONFIG(d)->operation.protocol_negotiation ? "Yes" : "No", grn, nrm, cyn,
					OLC_CONFIG(d)->operation.special_in_comm ? "Yes" : "No", grn, nrm, cyn,
					OLC_CONFIG(d)->operation.debug_mode ==
					0 ? "OFF" : (OLC_CONFIG(d)->operation.debug_mode ==
								 1 ? "BRIEF" : (OLC_CONFIG(d)->operation.debug_mode ==
												2 ? "NORMAL" : "COMPLETE")), grn, nrm);
...
static void cedit_disp_autowiz_options(struct descriptor_data *d)
{
	...
	write_to_output(d, "\r\n\r\n"
					...
					grn, nrm, cyn, OLC_CONFIG(d)->autowiz.min_wizlist_lev, grn, nrm);
...
void cedit_parse(struct descriptor_data *d, char *arg)
{
	...
	case CEDIT_OPERATION_OPTIONS_MENU:
		...
		case 'T':
			write_to_output(d,
							"Enter the current debug level (0: Off, 1: Brief, 2: Normal, 3: Complete) : ");
