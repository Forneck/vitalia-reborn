...
static void Crash_report_rent(struct char_data *ch, struct char_data *recep, struct obj_data *obj, long *cost,
...
#define TEST_OBJS(obj1, obj2, field) ((!obj1->field || !obj2->field || strcmp(obj1->field, obj2->field)))
#define TEST_OBJN(field) (obj->obj_flags.field != temp->obj_flags.field)
    ...
    if (TEST_OBJN(wear_flags))
        fprintf(fp, "Wear: %d %d %d %d\n", GET_OBJ_WEAR(obj)[0], GET_OBJ_WEAR(obj)[1], GET_OBJ_WEAR(obj)[2],
                GET_OBJ_WEAR(obj)[3]);
    ...
    if (obj->ex_description || temp->ex_description) {
        /* To be reimplemented.  Need to handle this case in loading as
           well */
        if ((obj->ex_description && temp->ex_description && obj->ex_description != temp->ex_description) ||
...
int Crash_delete_file(char *name)
{
    ...
    if (!(fl = fopen(filename, "r"))) {
        if (errno != ENOENT) /* if it fails but NOT because of no file */
            log1("SYSERR: deleting crash file %s (1): %s", filename, strerror(errno));
        return FALSE;
    }
    fclose(fl);
    ...
    if (remove(filename) < 0 && errno != ENOENT)
        log1("SYSERR: deleting crash file %s (2): %s", filename, strerror(errno));
...
int Crash_delete_crashfile(struct char_data *ch)
{
    ...
    if (!(fl = fopen(filename, "r"))) {
        if (errno != ENOENT) /* if it fails, NOT because of no file */
            log1("SYSERR: checking for crash file %s (3): %s", filename, strerror(errno));
        return FALSE;
    }
...
int Crash_clean_file(char *name)
{
    ...
    if (!(fl = fopen(filename, "r"))) {
        if (errno != ENOENT) /* if it fails, NOT because of no file */
            log1("SYSERR: OPENING OBJECT FILE %s (4): %s", filename, strerror(errno));
        return FALSE;
    }
    ...
    if ((rentcode == RENT_CRASH) || (rentcode == RENT_FORCED) || (rentcode == RENT_TIMEDOUT)) {
        if (timed < time(0) - (CONFIG_CRASH_TIMEOUT * SECS_PER_REAL_DAY)) {
            ...
            log1("    Deleting %s's %s file.", name, filetype);
            return TRUE;
        }
        ...
        if (timed < time(0) - (CONFIG_RENT_TIMEOUT * SECS_PER_REAL_DAY)) {
            Crash_delete_file(name);
            log1("    Deleting %s's rent file.", name);
            return TRUE;
        }
    return FALSE;
}
...
int Crash_load(struct char_data *ch) { return (Crash_load_objs(ch)); }
...
static int Crash_is_unrentable(struct obj_data *obj)
{
    ...
    if (OBJ_FLAGGED(obj, ITEM_NORENT) || GET_OBJ_RENT(obj) < 0 || GET_OBJ_RNUM(obj) == NOTHING ||
        ...
        log1("Crash_is_unrentable: removing object %s", obj->short_description);
        return TRUE;
    }
...
static int objsave_write_rentcode(FILE *fl, int rentcode, int cost_per_day, struct char_data *ch)
{
    if (fprintf(fl, "%d %ld %d %d %d %d\r\n", rentcode, (long)time(0), cost_per_day, GET_GOLD(ch), GET_BANK_GOLD(ch),
                0) < 1) {
    ...
    if (obj) {
        if (!Crash_is_unrentable(obj)) {
            ...
            *cost += MAX(0, (GET_OBJ_RENT(obj) * factor));
            if (display) {
                sprintf(buf, "$n tells you, '%5d coins for %s..'", (GET_OBJ_RENT(obj) * factor), OBJS(obj, ch));
                act(buf, FALSE, recep, 0, ch, TO_VICT);
            }
        }
...
static int Crash_offer_rent(struct char_data *ch, struct char_data *recep, int display, int factor)
{
    ...
    if (!numitems) {
        act("$n tells you, 'But you are not carrying anything!  Just quit!'", FALSE, recep, 0, ch, TO_VICT);
        return FALSE;
    }
    if (numitems > CONFIG_MAX_OBJ_SAVE) {
        sprintf(buf, "$n tells you, 'Sorry, but I cannot store more than %d items.'", CONFIG_MAX_OBJ_SAVE);
        act(buf, FALSE, recep, 0, ch, TO_VICT);
        return FALSE;
    }
    if (display) {
        ...
        act(buf, FALSE, recep, 0, ch, TO_VICT);
        sprintf(buf, "$n tells you, 'For a total of %ld coins%s.'", totalcost,
...
static int gen_receptionist(struct char_data *ch, struct char_data *recep, int cmd, char *arg, int mode)
{
    ...
    if (CONFIG_FREE_RENT) {
        act("$n tells you, 'Rent is free here.  Just quit, and your objects will be saved!'", FALSE, recep, 0, ch,
            TO_VICT);
        return TRUE;
    }
    ...
    if (CMD_IS("rent")) {
        ...
        if (mode == RENT_FACTOR) {
            ...
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s has rented (%d/day, %d tot.)", GET_NAME(ch), cost,
...
SPECIAL(receptionist) { return (gen_receptionist(ch, (struct char_data *)me, cmd, argument, RENT_FACTOR)); }

SPECIAL(cryogenicist) { return (gen_receptionist(ch, (struct char_data *)me, cmd, argument, CRYO_FACTOR)); }
    ...
    while (TRUE) {
        ...
        if (get_line(fl, line) == FALSE || (*line == '$' && line[1] == '~')) {
            if (temp == NULL && current->obj == NULL) {
                ...
                if (temp != current->obj)
                    log1("inconsistent object pointers in objsave_parse_objects: %p/%p", (void *)temp,
                         (void *)current->obj);
            }
...
static int Crash_load_objs(struct char_data *ch)
{
    ...
    if (rentcode == RENT_RENTED || rentcode == RENT_TIMEDOUT) {
        ...
        if (cost > (unsigned int)GET_GOLD(ch) + (unsigned int)GET_BANK_GOLD(ch)) {
            fclose(fl);
            mudlog(BRF, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s entering game, rented equipment lost (no $).",
                   GET_NAME(ch));
            Crash_crashsave(ch);
            return 2;
        } else {
    ...
    switch (orig_rent_code = rentcode) {
        ...
        case RENT_CRASH:
            ...
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s retrieving crash-saved items and entering game.",
                   GET_NAME(ch));
            break;
        case RENT_CRYO:
        ...
        case RENT_FORCED:
        case RENT_TIMEDOUT:
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "%s retrieving force-saved items and entering game.",
                   GET_NAME(ch));
            break;
        default:
            mudlog(NRM, MAX(LVL_IMMORT, GET_INVIS_LEV(ch)), TRUE, "WARNING: %s entering game with undefined rent code.",
                   GET_NAME(ch));
            break;
    }
    ...
    mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "%s (level %d) has %d object%s (max %d).", GET_NAME(ch),
           GET_LEVEL(ch), num_objs, num_objs != 1 ? "s" : "", CONFIG_MAX_OBJ_SAVE);
