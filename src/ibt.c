...
static const char *ibt_types[] = {"Bug", "Idea", "Typo", "\n"};
...
static void free_ibt_list(IBT_DATA *first_ibt, IBT_DATA *last_ibt)
{
    ...
    if ((first_ibt == NULL) || (last_ibt == NULL))
        return;
    ...
    while (first_ibt) {
        ...
        if (this_ibt->body)
            STRFREE(this_ibt->body);
        if (this_ibt->name)
            STRFREE(this_ibt->name);
        if (this_ibt->text)
            STRFREE(this_ibt->text);
        if (this_ibt->notes)
            STRFREE(this_ibt->notes);
...
static IBT_DATA *read_ibt(char *filename, FILE *fp)
{
    ...
    for (;;) {
        ...
        switch (UPPER(word[0])) {
            ...
            case 'N':
                if (!str_cmp(word, "Name") && ibtData->name)
                    STRFREE(ibtData->name);
                if (!str_cmp(word, "Notes") && ibtData->notes)
            ...
            default:
                log1("SYSERR: Invalid keyword (%s) in IBT file", word);
                break;
        }
        if (!fMatch) {
            fread_to_eol(fp);
            log1("read_ibt (%s): no match: %s - aborting line!", filename, word);
        }
    }
    log1("read_ibt: Aborted! Returning NULL");
    if (ibtData->name)
        STRFREE(ibtData->name);
    if (ibtData->text)
        STRFREE(ibtData->text);
...
void load_ibt_file(int mode)
{
    ...
    switch (mode) {
        ...
        case SCMD_TYPO:
            ...
            break;
        default:
            log1("SYSERR: Invalid mode (%d) in load_ibt_file", mode);
            return;
    }
    ...
    while ((ibtData = read_ibt(filename, fp)) != NULL) {
        switch (mode) {
            case SCMD_BUG:
                LINK(ibtData, first_bug, last_bug, next, prev);
                break;
            case SCMD_IDEA:
                LINK(ibtData, first_idea, last_idea, next, prev);
                break;
            case SCMD_TYPO:
...
void save_ibt_file(int mode)
{
    ...
    switch (mode) {
        ...
        case SCMD_TYPO:
            ...
            break;
        default:
            log1("SYSERR: Invalid mode (%d) in save_ibt_file", mode);
            return;
    }

    if ((fp = fopen(filename, "w")) == NULL) {
        log1("SYSERR: Unable to open IBT file for writing in save_ibt_file");
        log1("        IBT File: %s", filename);
        return;
    } else {
        for (ibtData = first_ibt; ibtData; ibtData = ibtData->next) {
            ...
            fprintf(fp, "Level     %d\n", ibtData->level);
            fprintf(fp, "Room      %d\n", ibtData->room);
            fprintf(fp, "Flags     %d %d %d %d\n", ibtData->flags[0], ibtData->flags[1], ibtData->flags[2],
                    ibtData->flags[3]);
            fprintf(fp, "End\n");
        }
        fclose(fp);
...
static IBT_DATA *get_first_ibt(int mode)
{
    ...
    switch (mode) {
        ...
        case SCMD_TYPO:
            ...
            break;
        default:
            log1("SYSERR: Invalid mode (%d) in get_first_ibt", mode);
            break;
    }
    return (first_ibt);
}
...
static IBT_DATA *get_last_ibt(int mode)
{
    ...
    switch (mode) {
        ...
        case SCMD_TYPO:
            ...
            break;
        default:
            log1("SYSERR: Invalid mode (%d) in get_last_ibt", mode);
            break;
    }
    return (last_ibt);
}
...
static IBT_DATA *get_ibt_by_num(int mode, int target_num)
{
    ...
    if ((first_ibt = get_first_ibt(mode)) == NULL)
        return NULL;
...
static bool free_ibt(int mode, IBT_DATA *ibtData)
{
    ...
    if (ibt_in_list(mode, ibtData)) {
        switch (mode) {
            case SCMD_BUG:
                UNLINK(ibtData, first_bug, last_bug, next, prev);
                break;
            case SCMD_IDEA:
                UNLINK(ibtData, first_idea, last_idea, next, prev);
                break;
            case SCMD_TYPO:
    ...
    if ((!*arg)) {
        if (GET_LEVEL(ch) >= LVL_GRGOD) {
            send_to_char(ch,
                         ...
                         "       %s%s edit <num>%s\r\n"
                         "       %s%s resolve <num>%s\r\n",
                         QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL,
                         CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM);
            return;
            ...
            send_to_char(ch,
                         ...
                         "       %s%s list%s\r\n"
                         "       %s%s show <num>%s\r\n",
                         QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM, QYEL, CMD_NAME, QNRM);
            return;
        } else {
            send_to_char(ch,
                         "Usage: %s%s submit <header>%s\r\n"
        ...
        if ((ibtData = get_ibt_by_num(subcmd, ano)) == NULL) {
            ...
            if ((GET_LEVEL(ch) < LVL_IMMORT) && (!is_ibt_logger(ibtData, ch))) {
                ...
                send_to_char(ch, "%s%s Status: %s%s%s%s\r\n", QCYN, ibt_types[subcmd],
                             ...
                             IBT_FLAGGED(ibtData, IBT_INPROGRESS) ? " (In Progress)" : "", QNRM);
            }
        }
        ...
        if (first_ibt) {
            ...
            for (ibtData = first_ibt; ibtData; ibtData = ibtData->next) {
                ...
                if (IBT_FLAGGED(ibtData, IBT_RESOLVED)) {
                    ...
                    if (GET_LEVEL(ch) < LVL_IMMORT) {
                        ...
                        len += snprintf(buf + len, sizeof(buf) - len, "%s%s%3d%s|%s%-12s%s|%s%6d%s|%s%5d%s|%s%s%s\r\n",
                                        imp, QRED, i, QGRN, QRED, ibtData->name, QGRN, QRED, ibtData->room, QGRN, QRED,
            ...
            if ((num_res + num_unres) > 0) {
                len += snprintf(buf + len, sizeof(buf) - len,
                                ...
                                QCYN, i, CMD_NAME, QBGRN, num_res, QCYN, QBRED, num_unres, QCYN, QNRM, QCYN,
                                ibt_types[subcmd], QRED, QCYN, CMD_NAME, QCYN, ibt_types[subcmd], QYEL, QCYN, CMD_NAME,
            ...
            snprintf(buf + len, sizeof(buf) - len,
                     "%sYou may use %s%s show <number>%s to see more indepth about the %s.%s\r\n", QCYN, QYEL, CMD_NAME,
                     QCYN, CMD_NAME, QNRM);
        } else {
        ...
        switch (subcmd) {
            ...
            case SCMD_TYPO:
                ...
                break;
            default:
                log1("Invalid subcmd (%d) in do_ibt", subcmd);
                return;
        }
...
static void ibtedit_save(struct descriptor_data *d)
{
    ...
    if ((ibtData = get_ibt_by_num(OLC_VAL(d), OLC_NUM(d))) != NULL) {
        ...
        log1("SYSERR: ibtedit_save: Invalid IBT vnum (%d) in OLC struct", OLC_NUM(d));
        log1("        IBT possibly removed while being edited");
        return;
    }
...
static void ibtedit_disp_main_menu(struct descriptor_data *d)
{
    ...
    send_to_char(ch,
                 ...
                 "%s6%s) Admin Notes:\r\n%s%s\r\n"
                 "%sQ%s) Quit %s Editor\r\n",
                 QBGRN, IBT_TYPE, cyn, yel, OLC_NUM(d), cyn, yel, nrm, yel, OLC_IBT(d)->name, yel, nrm, cyn, yel,
                 OLC_IBT(d)->room, cyn, nrm, (rr == NOWHERE) ? CCRED(d->character, C_NRM) : cyn,
                 (rr == NOWHERE) ? "<Invalid Room!>" : world[rr].name, yel, nrm, yel, OLC_IBT(d)->text, yel, nrm, cyn,
                 flg_text, yel, nrm, yel, OLC_IBT(d)->body ? OLC_IBT(d)->body : "<Not Set!>", yel, nrm, yel,
                 OLC_IBT(d)->notes ? OLC_IBT(d)->notes : "<Not Set!>", yel, nrm, IBT_TYPE);
