...
void load_ibt_file(int mode)
{
   ...
   switch(mode) {
     ...
     case SCMD_TYPO: sprintf( filename, "%s",TYPOS_FILE );
                     ...
                     last_ibt  = last_typo;
                     break;
     default       : log1("SYSERR: Invalid mode (%d) in load_ibt_file", mode);
                     return;
   }
...
void save_ibt_file(int mode)
{
   ...
   switch(mode) {
     ...
     case SCMD_TYPO: sprintf( filename, "%s",TYPOS_FILE );
                     first_ibt = first_typo;
                     break;
     default       : log1("SYSERR: Invalid mode (%d) in save_ibt_file", mode);
                     return;
   }
...
static IBT_DATA *get_first_ibt(int mode)
{
  ...
  switch(mode) {
    ...
    case SCMD_TYPO: first_ibt = first_typo;
                    break;
    default       : log1("SYSERR: Invalid mode (%d) in get_first_ibt", mode);
                    break;
  }
  return (first_ibt);
}
...
static IBT_DATA *get_last_ibt(int mode)
{
  ...
  switch(mode) {
    ...
    case SCMD_TYPO: last_ibt = last_typo;
                    break;
    default       : log1("SYSERR: Invalid mode (%d) in get_last_ibt", mode);
                    break;
  }
  return (last_ibt);
}
  ...
  else if (is_abbrev(arg,"submit"))
  {
    ...
    switch (subcmd) {
      ...
      case SCMD_TYPO: SET_BIT_AR(PLR_FLAGS(ch), PLR_TYPO);
                      break;
      default       : log1("Invalid subcmd (%d) in do_ibt", subcmd);
                      return;
	}
