...
extern void raise_online(struct char_data *ch, struct char_data *raiser, struct obj_data *corpse, room_rnum targ_room, int restore);
...
ACMD(do_echo)
{
	...
	else
	{
		...
		else
		{
			...
			mudlog(CMP, MAX(LVL_BUILDER, GET_INVIS_LEV(ch)), TRUE, "(GC) %s echoed: %s",
				   GET_NAME(ch), buf);
		}
...
ACMD(do_send)
{
	...
	send_to_char(vict, "%s\r\n", buf);
	mudlog(CMP, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s sent %s: %s", GET_NAME(ch),
		   GET_NAME(vict), buf);
		...
		if (cmd_room == rrnum)
		{
			...
			case 'P':
				send_to_char(ch, "%sPut %s [%s%d%s] in %s [%s%d%s], Max : %d\r\n",
							 ...
							 cyn, obj_index[ZOCMD.arg3].vnum, yel, ZOCMD.arg2);
				break;
			case 'R':
			...
			case 'V':
				send_to_char(ch, "%sAssign global %s:%d to %s = %s\r\n",
							 ...
							 ((ZOCMD.arg1 == MOB_TRIGGER) ? "mobile" :
							  ((ZOCMD.arg1 == OBJ_TRIGGER) ? "object" :
							   ((ZOCMD.arg1 == WLD_TRIGGER) ? "room" : "????"))), ZOCMD.sarg2);
				break;
			case 'C':
				send_to_char(ch, "%sCheck %s [%s%d%s], Max : %d\r\n",
							 ZOCMD.if_flag ? " then " : "",
							 mob_proto[ZOCMD.arg2].player.short_descr, cyn,
							 mob_index[ZOCMD.arg2].vnum, yel, ZOCMD.arg1);
				break;
			default:
...
static void do_stat_room(struct char_data *ch, struct room_data *rm)
{
	...
	for (i = 0; i < DIR_COUNT; i++)
	{
		...
		send_to_char(ch, "Exit %s%-5s%s:  To: [%s], Key: [%5d], Keywords: %s, Type: %s\r\n%s",
					 ...
					 dir_option[i]->general_description : "  No exit description.\r\n");
	}
...
static void do_stat_object(struct char_data *ch, struct obj_data *j)
{
	...
	send_to_char(ch, "A-Desc: '%s%s%s'\r\n", CCYEL(ch, C_NRM),
				 j->action_description ? j->action_description : "<None>", CCNRM(ch, C_NRM));
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
	...
	if (!IS_NPC(k))
	{
		...
		if (GET_LEVEL(k) >= LVL_BUILDER)
		{
			if (GET_OLC_ZONE(k) == AEDIT_PERMISSION)
				...
				send_to_char(ch, ", OLC[%s%d%s]", CCCYN(ch, C_NRM), GET_OLC_ZONE(k),
							 CCNRM(ch, C_NRM));
		}
		send_to_char(ch, "\r\n");
	}
	...
	if (IS_NPC(k)) {
	    ...
	    send_to_char(ch, "Tendência Grupo (Genética): [%s%d%s]\r\n",
			 CCCYN(ch, C_NRM), GET_GENGROUP(k), CCNRM(ch, C_NRM));
	...
	send_to_char(ch,
				 "AC: [%d%+d/10], Hitroll: [%2d], Damroll: [%2d], Saving throws: [%d/%d/%d/%d/%d]\r\n",
				 GET_AC(k), dex_app[GET_DEX(k)].defensive, k->points.hitroll, k->points.damroll,
				 GET_SAVE(k, 0), GET_SAVE(k, 1), GET_SAVE(k, 2), GET_SAVE(k, 3), GET_SAVE(k, 4));
	...
	if (IS_MOB(k))
		send_to_char(ch, "Mob Spec-Proc: %s, NPC Bare Hand Dam: %dd%d\r\n",
					 (mob_index[GET_MOB_RNUM(k)].func ?
					  get_spec_func_name(mob_index[GET_MOB_RNUM(k)].func) : "None"),
					 k->mob_specials.damnodice, k->mob_specials.damsizedice);

	for (i = 0, j = k->carrying; j; j = j->next_content, i++);
	send_to_char(ch, "Carried: weight: %d, items: %d; Items in: inventory: %d, ", IS_CARRYING_W(k),
				 IS_CARRYING_N(k), i);
	...
	column =
		send_to_char(ch, "Master is: %s, Followers are:",
	...
	if (!IS_NPC(k) && (GET_LEVEL(k) >= LVL_IMMORT))
	{
		if (POOFIN(k))
			...
			send_to_char(ch, "%sPOOFIN:  %s%s appears with an ear-splitting bang.%s\r\n", QYEL,
						 QCYN, GET_NAME(k), QNRM);

		if (POOFOUT(k))
			send_to_char(ch, "%sPOOFOUT: %s%s %s%s\r\n", QYEL, QCYN, GET_NAME(k), POOFOUT(k),
						 QNRM);
		else
	...
	else
	{
		...
		if ((object = get_obj_in_equip_vis(ch, name, &number, ch->equipment)) != NULL)
			...
			do_stat_character(ch, victim);
				  ...
				  get_obj_in_list_vis(ch, name, &number, world[IN_ROOM(ch)].contents)) != NULL)
			do_stat_object(ch, object);
...
ACMD(do_switch)
{
	...
	{
		send_to_char(ch, "%s", CONFIG_OK);
		mudlog(CMP, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s Switched into: %s",
...
ACMD(do_purge)
{
	...
	if (*buf)
	{
		...
		if ((vict = get_char_vis(ch, buf, &number, FIND_CHAR_ROOM)) != NULL)
		{
			...
			if (!IS_NPC(vict))
			{
				mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s has purged %s.",
			...
			extract_char(vict);
				  ...
				  get_obj_in_list_vis(ch, buf, &number, world[IN_ROOM(ch)].contents)) != NULL)
		{
...
ACMD(do_advance)
{
	...
	if (newlevel < oldlevel)
		log1("(GC) %s demoted %s from level %d to %d.",
			GET_NAME(ch), GET_NAME(victim), oldlevel, newlevel);
	else
		log1("(GC) %s has advanced %s to level %d (from %d)",
...
ACMD(do_restore)
{
	...
	{
		mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s restored %s", GET_NAME(ch),
			   GET_NAME(vict));
...
void perform_immort_vis(struct char_data *ch)
{
	if ((GET_INVIS_LEV(ch) == 0)
		&& (!AFF_FLAGGED(ch, AFF_HIDE) && !AFF_FLAGGED(ch, AFF_INVISIBLE)))
	{
...
ACMD(do_force)
{
	...
	else if ((GET_LEVEL(ch) < LVL_GRGOD) || (str_cmp("all", arg) && str_cmp("room", arg)))
	{
		...
		else
		{
			...
			mudlog(CMP, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s forced %s to %s",
	...
	{							/* force all */
		send_to_char(ch, "%s", CONFIG_OK);
		mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s forced all to %s",
...
ACMD(do_wiznet)
{
	...
	for (d = descriptor_list; d; d = d->next)
	{
			...
			&& (d != ch->desc || !(PRF_FLAGGED(d->character, PRF_NOREPEAT))))
		{
			...
			{
				parse_at(buf1);
				send_to_char(d->character, "%s%s%s", CCCYN(d->character, C_NRM), buf1,
			...
			{
				parse_at(buf2);
				send_to_char(d->character, "%s%s%s", CCCYN(d->character, C_NRM), buf2,
							 CCNRM(d->character, C_NRM));
...
ACMD(do_zreset)
{
	...
	if (*arg == '*')
	{
		...
		else
		{
			...
			mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s reset entire world.",
				   GET_NAME(ch));
			return;
		}
	}
	...
	if (i <= top_of_zone_table && (can_edit_zone(ch, i) || GET_LEVEL(ch) > LVL_IMMORT))
	{
		...
		mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s reset zone %d (%s)",
...
ACMD(do_wizutil)
{
	...
	else
	{
		...
		case SCMD_PARDON:
			...
			mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s pardoned by %s",
		...
		case SCMD_FREEZE:
			...
			act("A sudden cold wind conjured from nowhere freezes $n!", FALSE, vict, 0, 0,
				TO_ROOM);
...
static size_t print_zone_to_buf(char *bufptr, size_t left, zone_rnum zone, int listall)
{
	...
	if (listall)
	{
		...
		tmp = snprintf(bufptr, left,
					   ...
					   zone_table[zone].number, zone_table[zone].name, KNRM,
					   zone_table[zone].builders, KNRM, zone_table[zone].age,
					   zone_table[zone].lifespan,
					   zone_table[zone].reset_mode ? ((zone_table[zone].reset_mode == 1) ?
		...
		for (i = 0; i < top_of_objt; i++)
			if (obj_index[i].vnum >= zone_table[zone].bot
				&& obj_index[i].vnum <= zone_table[zone].top)
				k++;

		for (i = 0; i < top_of_mobt; i++)
			if (mob_index[i].vnum >= zone_table[zone].bot
				&& mob_index[i].vnum <= zone_table[zone].top)
				l++;
...
ACMD(do_show)
{
	...
	case 2:
		{
			...
			strftime(buf2, sizeof(buf2), "%a %b %d %H:%H:%S %Y",
					 localtime(&(vict->player.time.logon)));
				...
				if (W_EXIT(i, j)->to_room == 0)
				{
					nlen =
						snprintf(buf + len, sizeof(buf) - len,
								 "%2d: (void   ) [%5d] %-*s%s (%s)\r\n", ++k, GET_ROOM_VNUM(i),
								 count_color_chars(world[i].name) + 40, world[i].name, QNRM,
								 dirs[j]);
					if (len + nlen >= sizeof(buf))
						break;
					len += nlen;
	...
	case 6:
		...
		for (i = 0, j = 0; i <= top_of_world; i++)
			...
			{
				nlen =
					snprintf(buf + len, sizeof(buf) - len, "%2d: [%5d] %s%s\r\n", ++j,
	...
	case 11:
		...
		for (j = 1; j < LVL_IMMORT; j++)
		{
			nlen = snprintf(buf + len, sizeof(buf) - len, "%-3d - %-2d %-2d %-2d %-2d\r\n", j,
							...
							thaco(CLASS_THIEF, j), thaco(CLASS_WARRIOR, j));
			if (len + nlen >= sizeof(buf))
				break;
			len += nlen;
		}
				...
				for (b = 0; b < 6; b++)
				{
					...
					nlen =
						snprintf(buf + len, sizeof(buf) - len, "%s%s%s",
								 ColourRGB(ch->desc, colour), colour,
								 ++k % 6 == 0 ? "\tn\r\n" : "    ");
					if (len + nlen >= sizeof(buf))
						break;
					len += nlen;
...
static int perform_set(struct char_data *ch, struct char_data *vict, int mode, char *val_arg)
{
	...
	case 38:					/* password */
		...
		strncpy(GET_PASSWD(vict), CRYPT(val_arg, GET_NAME(vict)), MAX_PWD_LENGTH);	/* strncpy: 
																					   OK 
																					   (G_P:MAX_PWD_LENGTH) 
																					 */
...
static const int offlimit_zones[] = { 0,5, 12, 14, -1 };	/* what zones can
															   ...
															   to (virtual
															   num) */
#define MIN_ROOM_DESC_LENGTH   80	/* at least one line - set to 0 to not
									   care. */
...
ACMD(do_zcheck)
{
	...
	int i = 0, j = 0, k = 0, l = 0, m = 0, found = 0;	/* found is used as a
														   'send now' flag */
	char buf[MAX_STRING_LENGTH];
	float avg_dam;
	size_t len = 0;
	...
	for (i = 0; i < top_of_mobt; i++)
	{
		...
		{						/* is mob in this zone? */
			...
			if (!strncmp(mob->player.long_descr, "An unfinished mob stands here.", 30)
				&& (found = 1))
				len +=
					snprintf(buf + len, sizeof(buf) - len,
			...
			avg_dam =
				(((mob->mob_specials.damsizedice / 2.0) * mob->mob_specials.damnodice) +
				 GET_DAMROLL(mob));
			...
			if (mob->mob_specials.damsizedice == 1 &&
				mob->mob_specials.damnodice == 1 && GET_LEVEL(mob) == 0 && (found = 1))
				len += snprintf(buf + len, sizeof(buf) - len,
			...
			if ((AFF_FLAGGED(mob, AFF_CHARM) || AFF_FLAGGED(mob, AFF_POISON)) && (found = 1))
				len +=
					snprintf(buf + len, sizeof(buf) - len,
							 ...
							 AFF_FLAGGED(mob, AFF_CHARM) ? "CHARM" : "", AFF_FLAGGED(mob,
																					 AFF_POISON)
							 ? "POISON" : "");
	...
	for (i = 0; i < top_of_objt; i++)
	{
		...
		{						/* is object in this zone? */
			...
			{
				if ((GET_OBJ_COST(obj)
					 || (GET_OBJ_WEIGHT(obj) && GET_OBJ_TYPE(obj) != ITEM_FOUNTAIN)
			...
			if (obj->action_description && *obj->action_description &&
				...
				GET_OBJ_TYPE(obj) != ITEM_SCROLL && GET_OBJ_TYPE(obj) != ITEM_NOTE && (found = 1))
				len +=
					snprintf(buf + len, sizeof(buf) - len,
			...
			for (j = 0; j < MAX_OBJ_AFFECT; j++)
				if (zaffs[(int)obj->affected[j].location].max_aff != -99 &&	/* only 
																			   care 
																			   if 
																			   a 
																			   range 
			...
			if (ROOM_FLAGGED(i, ROOM_ATRIUM) || ROOM_FLAGGED(i, ROOM_HOUSE)
				...
				len +=
					snprintf(buf + len, sizeof(buf) - len,
							 ...
							 ROOM_FLAGGED(i, ROOM_ATRIUM) ? "ATRIUM" : "", ROOM_FLAGGED(i,
																						ROOM_HOUSE)
							 ? "HOUSE" : "", ROOM_FLAGGED(i,
														  ROOM_HOUSE_CRASH) ? "HCRSH" : "",
							 ROOM_FLAGGED(i, ROOM_OLC) ? "OLC" : "", ROOM_FLAGGED(i,
																				  ROOM_BFS_MARK)
							 ? "*" : "");

			if ((MIN_ROOM_DESC_LENGTH)
				&& strlen(world[i].description) < MIN_ROOM_DESC_LENGTH && (found = 1))
				len +=
					snprintf(buf + len, sizeof(buf) - len,
		...
		for (cmd_no = 0; ZCMD2.command != 'S'; cmd_no++)
		{
			...
			if (ZCMD2.arg1 == mrnum)
			{
				send_to_char(ch, "  [%5d] %s (%d MAX)\r\n",
							 world[ZCMD2.arg3].number, world[ZCMD2.arg3].name, ZCMD2.arg2);
			}
		}
	}
}
...
static void trg_checkload(struct char_data *ch, trig_vnum tvnum)
{
	...
	send_to_char(ch, "Checking load info for the %s trigger '%s':\r\n",
				 ...
				 (trig_index[trnum]->proto->attach_type ==
				  OBJ_TRIGGER ? "object" : "room"), trig_index[trnum]->proto->name);
...
ACMD(do_zpurge)
{
	...
	if (!purge_all)
	{
		...
		mudlog(NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s purged zone %d (%s)",
	...
	else
	{
		...
		mudlog(NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE,
			   "(GC) %s purged entire world.", GET_NAME(ch));
	}
}
...
bool change_player_name(struct char_data *ch, struct char_data *vict, char *new_name)
{
	...
	if (player_table[i].id != GET_IDNUM(vict))
	{
		...
		log1("SYSERR: Player %s, with ID %ld, could not be found in the player index.",
			GET_NAME(vict), GET_IDNUM(vict));
		return FALSE;
	}
	...
	mudlog(BRF, LVL_IMMORT, TRUE, "(GC) %s changed the name of %s to %s", GET_NAME(ch),
		   old_name, new_name);

	if (vict->desc)				/* Descriptor is set if the victim is logged
								   in */
		send_to_char(vict, "Your login name has changed from %s%s%s to %s%s%s.\r\n",
					 CCYEL(vict, C_NRM), old_name, CCNRM(vict, C_NRM), CCYEL(vict, C_NRM),
					 new_name, CCNRM(vict, C_NRM));
...
ACMD(do_zlock)
{
	...
	if (is_abbrev(arg, "all"))
	{
		...
		mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s has locked ALL zones!",
			   GET_NAME(ch));
		return;
	}
		...
		for (zn = 0; zn <= top_of_zone_table; zn++)
		{
			if (ZONE_FLAGGED(zn, ZONE_NOBUILD))
			{
				...
				send_to_char(ch, "[%s%3d%s] %s%-*s %s%-1s%s\r\n",
							 ...
							 count_color_chars(zone_table[zn].name) + 30,
							 zone_table[zn].name, QYEL,
	...
	if (GET_LEVEL(ch) < LVL_GRGOD && !is_name(GET_NAME(ch), zone_table[zn].builders)
	...
	if (save_zone(zn))
	{
		mudlog(NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s has locked zone %d",
			   GET_NAME(ch), znvnum);
	}
	else
	{
...
ACMD(do_zunlock)
{
	...
	if (is_abbrev(arg, "all"))
	{
		...
		mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE,
			   "(GC) %s has unlocked ALL zones!", GET_NAME(ch));
		return;
	}
	if (is_abbrev(arg, "list"))
	{
	...
	{
		mudlog(NRM, MAX(LVL_GRGOD, GET_INVIS_LEV(ch)), TRUE,
			   "(GC) %s has unlocked zone %d", GET_NAME(ch), znvnum);
	}
	else
	{
		send_to_char(ch, "Unable to save zone changes.  Check syslog!\r\n");
...
ACMD(do_recent)
{
	...
	{							/* If High-Level Imm, then show Host IP */
		send_to_char(ch,
					 " ID | DATE/TIME                | HOST IP                          | Player Name\r\n");
	}
	else
	{
	...
	while (this)
	{
		...
		if ((limit == 0) || (count < limit))
		{
											   ...
											   Host IP */
			{
				...
				{
					send_to_char(ch,
								 "%3d | %-24.24s | %s%-32s%s | %s %s(New Player)%s\r\n",
								 this->vnum, timestr, loc ? QRED : "", this->host, QNRM,
								 this->name, QYEL, QNRM);
				}
				...
				else
				{
					send_to_char(ch, "%3d | %-24.24s | %s%-32s%s | %s\r\n", this->vnum,
								 timestr, loc ? QRED : "", this->host, QNRM, this->name);
				}
			}
			else
			{
...
ACMD(do_oset)
{
	...
	else
	{
		...
		else
		{
			...
			else if (is_abbrev(arg2, "longdesc")
					 && (success = oset_long_description(obj, argument)))
				send_to_char(ch, "Object long description set.\r\n");
			else if (is_abbrev(arg2, "shortdesc")
					 && (success = oset_short_description(obj, argument)))
