...
extern void raise_online(struct char_data *ch, struct char_data *raiser, struct obj_data *corpse, room_rnum targ_room,
                         int restore);
...
static void do_stat_character(struct char_data *ch, struct char_data *k)
{
    ...
    send_to_char(ch,
                 ...
                 CCCYN(ch, C_NRM), GET_STR(k), GET_ADD(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_INT(k),
                 CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_WIS(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_DEX(k),
                 CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_CON(k), CCNRM(ch, C_NRM), CCCYN(ch, C_NRM), GET_CHA(k),
                 CCNRM(ch, C_NRM));
     ...
     **************************************************************************/
    if (IS_NPC(k)) {
        send_to_char(ch, "Tendência Wimpy (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENWIMPY(k),
                     CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Loot (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENLOOT(k), CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Equip (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENEQUIP(k),
                     CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Uso (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENUSE(k), CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Roam (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENROAM(k), CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Grupo (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENGROUP(k),
                     CCNRM(ch, C_NRM));
        send_to_char(ch, "Prevalencia de Bravura (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENBRAVE(k),
                     CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Trade (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENTRADE(k),
                     CCNRM(ch, C_NRM));
        send_to_char(ch, "Tendência Postar Quest (Genética): [%s%d%s]\r\n", CCCYN(ch, C_NRM), GET_GENQUEST(k),
                     CCNRM(ch, C_NRM));
    ...
    send_to_char(ch, "Hit p.:[%s%d/%d+%d%s]  Mana p.:[%s%d/%d+%d%s]  Move p.:[%s%d/%d+%d%s]\r\n", CCGRN(ch, C_NRM),
                 GET_HIT(k), GET_MAX_HIT(k), hit_gain(k), CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), GET_MANA(k),
                 GET_MAX_MANA(k), mana_gain(k), CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), GET_MOVE(k), GET_MAX_MOVE(k),
                 move_gain(k), CCNRM(ch, C_NRM));

    send_to_char(ch, "Gold: [%9d], Bank: [%9d] (Total: %d), ", GET_GOLD(k), GET_BANK_GOLD(k),
                 GET_GOLD(k) + GET_BANK_GOLD(k));

    if (!IS_NPC(k))
        send_to_char(ch, "Screen %s[%s%d%sx%s%d%s]%s\r\n", CCCYN(ch, C_NRM), CCYEL(ch, C_NRM), GET_SCREEN_WIDTH(k),
                     CCNRM(ch, C_NRM), CCYEL(ch, C_NRM), GET_PAGE_LENGTH(k), CCCYN(ch, C_NRM), CCNRM(ch, C_NRM));

    send_to_char(ch, "AC: [%d%+d/10], Hitroll: [%2d], Damroll: [%2d], Saving throws: [%d/%d/%d/%d/%d]\r\n", GET_AC(k),
                 dex_app[GET_DEX(k)].defensive, k->points.hitroll, k->points.damroll, GET_SAVE(k, 0), GET_SAVE(k, 1),
                 GET_SAVE(k, 2), GET_SAVE(k, 3), GET_SAVE(k, 4));
    ...
    for (i = 0, j = k->carrying; j; j = j->next_content, i++)
        ;
    send_to_char(ch, "Carried: weight: %d, items: %d; Items in: inventory: %d, ", IS_CARRYING_W(k), IS_CARRYING_N(k),
                 i);
    ...
    if (k->affected) {
        for (aff = k->affected; aff; aff = aff->next) {
            send_to_char(ch, "SPL: (%3dhr) %s%-21s%s ", aff->duration + 1, CCCYN(ch, C_NRM), get_spell_name(aff->spell),
                         CCNRM(ch, C_NRM));
            ...
            if (memory_count > 0) {
                ...
                for (i = 0; i < EMOTION_MEMORY_SIZE; i++) {
                    ...
                    if (mem->timestamp > 0) {
                        ...
                        const char *interaction_name = (mem->interaction_type >= 0 && mem->interaction_type <= 18)
    ...
    if (mem->field > 0) {                                                                                              \
        ...
        if (_n > 0 && offset + _n < sizeof(emotion_buf)) {                                                             \
            ...
            has_emotions = 1;                                                                                          \
                        ...
                        AME(happiness_level, "Happy") AME(sadness_level, "Sad") AME(friendship_level, "Friend")
                            AME(love_level, "Love") AME(trust_level, "Trust") AME(loyalty_level, "Loyal")
                                AME(curiosity_level, "Curious") AME(greed_level, "Greed") AME(pride_level, "Pride")
                                    AME(compassion_level, "Compassion") AME(envy_level, "Envy")
                                        AME(courage_level, "Courage") AME(excitement_level, "Excited")
                                            AME(disgust_level, "Disgust") AME(shame_level, "Shame")
                                                AME(pain_level, "Pain") AME(horror_level, "Horror")
...
ACMD(do_shutdown)
{
    ...
    if (!*arg) {
        log1("(GC) Shutdown by %s.", GET_NAME(ch));
        send_to_all("Shutting down.\r\n");
        circle_shutdown = 1;
    } else if (!str_cmp(arg, "reboot")) {
        log1("(GC) Reboot by %s.", GET_NAME(ch));
        ...
        log1("(GC) Shutdown NOW by %s.", GET_NAME(ch));
...
{
    /* This short routine is to ensure that characters that happen to be
       snooping (or snooped) and get advanced/demoted will not be
       snooping/snooped someone of a higher/lower level (and thus, not
       entitled to be snooping. */
    if (!ch || !ch->desc)
        return;
...
void return_to_char(struct char_data *ch)
{
    /* If someone switched into your original body, disconnect them. - JE *
       ...
       * we must also NULL his pointer to our character, otherwise
...
ACMD(do_purge)
{
    ...
    if (*buf) {
        ...
        if ((vict = get_char_vis(ch, buf, &number, FIND_CHAR_ROOM)) != NULL) {
            ...
            if (!IS_NPC(vict)) {
                mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s has purged %s.", GET_NAME(ch),
                       GET_NAME(vict));
                if (vict->desc) {
...
ACMD(do_advance)
{
    ...
    if (newlevel < oldlevel)
        log1("(GC) %s demoted %s from level %d to %d.", GET_NAME(ch), GET_NAME(victim), oldlevel, newlevel);
    else
        log1("(GC) %s has advanced %s to level %d (from %d)", GET_NAME(ch), GET_NAME(victim), newlevel, oldlevel);
...
ACMD(do_dc)
{
    ...
    /* We used to just close the socket here using close_socket(), but various
       ...
       below you on the descriptor list.  Just setting to CON_CLOSE leaves
       things in a massively inconsistent state so I had to add this new flag to
       the descriptor. -je It is a much more logical extension for a
       CON_DISCONNECT to be used for in-game socket closes and CON_CLOSE for out
       of game closings. This will retain the stability of the close_me hack
       while being neater in appearance. -gg For those unlucky souls who
       actually manage to get disconnected by two different immortals in the
       same 1/10th of a second, we have the below 'if' check. -gg */
    if (STATE(d) == CON_DISCONNECT || STATE(d) == CON_CLOSE)
        act("$E's already being disconnected.", FALSE, ch, 0, d->character, TO_CHAR);
    else {
        /* Remember that we can disconnect people not in the game and that
           rather confuses the code when it expected there to be a character
           context. */
        ...
        log1("(GC) Connection closed by %s.", GET_NAME(ch));
    }
}
...
/* altered from stock to the following: last [name] [#] last without arguments
   displays the last 10 entries. last with a name only displays the 'stock'
   last entry. last with a number displays that many entries (combines with
   name) */
static const char *last_array[11] = {"Connect",    "Enter Game", "Reconnect", "Takeover", "Quit",   "Idleout",
                                     "Disconnect", "Shutdown",   "Reboot",    "Crash",    "Playing"};
...
struct last_entry *find_llog_entry(int punique, long idnum)
{
    ...
    if (!(fp = fopen(LAST_FILE, "r"))) {
        log1("Error opening last_file for reading, will create.");
        return NULL;
    }
    ...
    /* we'll search last to first, since it's faster than any thing else we
       can do (like searching for the last shutdown/etc..) */
...
static void mod_llog_entry(struct last_entry *llast, int type)
{
    ...
    if (!(fp = fopen(LAST_FILE, "r+"))) {
        log1("Error opening last_file for reading and writing.");
        return;
    }
    fseek(fp, 0L, SEEK_END);
    size = ftell(fp);
    ...
    for (tmp = recs; tmp > 0; tmp--) {
        fseek(fp, -1 * ((long)sizeof(struct last_entry)), SEEK_CUR);
        if (fread(&mlast, sizeof(struct last_entry), 1, fp) != 1) {
            log1("mod_llog_entry: read error or unexpected end of file.");
            return;
        }
        /* Another one to keep that stepback. */
        fseek(fp, -1 * ((long)sizeof(struct last_entry)), SEEK_CUR);
        ...
        if (mlast.idnum == llast->idnum && mlast.punique == llast->punique) {
            ...
            if (mlast.close_type != LAST_QUIT && mlast.close_type != LAST_IDLEOUT && mlast.close_type != LAST_REBOOT &&
...
void add_llog_entry(struct char_data *ch, int type)
{
    ...
    /* so if a char enteres a name, but bad password, otherwise loses link
       before he gets a pref assinged, we won't record it */
    if (GET_PREF(ch) <= 0) {
        return;
    }
    ...
    if (llast == NULL) { /* no entry found, add ..error if close! */
        ...
        if (!(fp = fopen(LAST_FILE, "a"))) {
            log1("error opening last_file for appending");
            free(llast);
            return;
        }
...
void clean_llog_entries(void)
{
    ...
    if (!(nfp = fopen("etc/nlast", "w"))) {
        log1("Error trying to open new last file.");
        fclose(ofp);
        return;
    }
    ...
    while (!feof(ofp)) {
        if (fread(&mlast, sizeof(struct last_entry), 1, ofp) != 1) {
            log1("clean_llog_entries: read error or unexpected end of file.");
            return;
        }
...
static void list_llog_entries(struct char_data *ch)
{
    ...
    if (!(fp = fopen(LAST_FILE, "r"))) {
        log1("llist_log_entries: could not open last log file %s.", LAST_FILE);
    ...
    while (fread(&llast, sizeof(struct last_entry), 1, fp) == 1) {
        ...
        send_to_char(ch, "%10s    %d    %s    %s\r\n", llast.username, llast.punique, last_array[llast.close_type],
                     timestr);
        break;
    }

    if (ferror(fp)) {
        log1("llist_log_entries: error reading %s.", LAST_FILE);
    ...
    *name = '\0';
    ...
    while (num > 0 && recs > 0) {
        ...
        if (!*name || (*name && !str_cmp(name, mlast.username))) {
            ...
            send_to_char(ch, "%10.10s %20.20s %20.21s - ", mlast.username, mlast.hostname, timestr);
            if ((temp = is_in_game(mlast.idnum)) && mlast.punique == GET_PREF(temp)) {
                ...
                send_to_char(ch, "%5.5s (%5.5s) %s", to, deltastr, last_array[mlast.close_type]);
            }
...
ACMD(do_force)
{
    ...
    if (!*arg || !*to_force)
        ...
        send_to_char(ch, "%s", CONFIG_OK);
        mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s forced room %d to %s", GET_NAME(ch),
               GET_ROOM_VNUM(IN_ROOM(ch)), to_force);
...
ACMD(do_wiznet)
{
    char buf1[MAX_INPUT_LENGTH + MAX_NAME_LENGTH + 32], buf2[MAX_INPUT_LENGTH + MAX_NAME_LENGTH + 32];
    struct descriptor_data *d;
    char emote = FALSE;
...
ACMD(do_zreset)
{
    ...
    if (i <= top_of_zone_table && (can_edit_zone(ch, i) || GET_LEVEL(ch) > LVL_IMMORT)) {
        ...
        mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s reset zone %d (%s)", GET_NAME(ch),
               zone_table[i].number, zone_table[i].name);
    } else
    ...
    if (!*arg)
        ...
        switch (subcmd) {
            case SCMD_REROLL:
                ...
                log1("(GC) %s has rerolled %s.", GET_NAME(ch), GET_NAME(vict));
            ...
            case SCMD_NOTITLE:
                ...
                mudlog(NRM, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) Notitle %s for %s by %s.", ONOFF(result),
                       GET_NAME(vict), GET_NAME(ch));
                send_to_char(ch, "(GC) Notitle %s for %s by %s.\r\n", ONOFF(result), GET_NAME(vict), GET_NAME(ch));
                break;
            case SCMD_MUTE:
                result = PLR_TOG_CHK(vict, PLR_NOSHOUT);
                mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) Mute %s for %s by %s.", ONOFF(result),
                       GET_NAME(vict), GET_NAME(ch));
            ...
            case SCMD_FREEZE:
                ...
                mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s frozen by %s.", GET_NAME(vict),
                       GET_NAME(ch));
                break;
            case SCMD_THAW:
                if (!PLR_FLAGGED(vict, PLR_FROZEN)) {
                ...
                mudlog(BRF, MAX(LVL_GOD, GET_INVIS_LEV(ch)), TRUE, "(GC) %s un-frozen by %s.", GET_NAME(vict),
                       GET_NAME(ch));
            ...
            default:
                ...
                /* SYSERR_DESC: This is the same as the unhandled case in
                   do_gen_ps(), but this function handles 'reroll', 'pardon',
                   'freeze', etc. */
                break;
        }
        save_char(vict);
...
static size_t print_zone_to_buf(char *bufptr, size_t left, zone_rnum zone, int listall)
{
    ...
    if (listall) {
        ...
        tmp = snprintf(bufptr, left, "%3d %-30.30s%s By: %-10.10s%s Age: %3d; Reset: %3d (%s); Range: %5d-%5d\r\n",
        ...
        tmp += snprintf(bufptr + tmp, left - tmp,
                        ...
                        "         Triggers: %2d\r\n"
                        "         Quests:   %2d\r\n",
                        buf, zone_table[zone].min_level, zone_table[zone].max_level, j, k, l, m, n, o);

        return tmp;
    }

    return snprintf(bufptr, left, "%3d %-*s%s By: %-10.10s%s Range: %5d-%5d\r\n", zone_table[zone].number,
...
ACMD(do_show)
{
    ...
    struct obj_data *obj;
    struct descriptor_data *d;
    char field[MAX_INPUT_LENGTH], value[MAX_INPUT_LENGTH], arg[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH];
    int r, g, b;
    char colour[16];
    ...
    switch (l) {
        ...
        case 2: {
            ...
            send_to_char(ch, "Gold: %-8d  Bal: %-8d Exp: %-8ld  Align: %-5d  Lessons: %-3d\r\n", GET_GOLD(vict),
                         GET_BANK_GOLD(vict), GET_EXP(vict), GET_ALIGNMENT(vict), GET_PRACTICES(vict));
        ...
        case 3:
            ...
            /* show stats */
        case 4: {
            long long total_money, total_qp;
            int mortal_count;
            char money_str[64], qp_str[64], rate_str[64];
            ...
            send_to_char(ch,
                         "Economy stats (%d mortals):\r\n"
                         "  %s total money  %s total qps\r\n"
                         "  %s QP Exchange Rate\r\n",
                         mortal_count, money_str, qp_str, rate_str);
            break;
        }
                ...
                for (j = 0; j < DIR_COUNT; j++) {
                    ...
                    if (W_EXIT(i, j)->to_room == 0) {
                        nlen = snprintf(buf + len, sizeof(buf) - len, "%2d: (void   ) [%5d] %-*s%s (%s)\r\n", ++k,
                                        GET_ROOM_VNUM(i), count_color_chars(world[i].name) + 40, world[i].name, QNRM,
                                        dirs[j]);
                        if (len + nlen >= sizeof(buf))
                            break;
                        len += nlen;
        ...
        case 11:
            ...
            for (j = 1; j < LVL_IMMORT; j++) {
                nlen = snprintf(buf + len, sizeof(buf) - len, "%-3d - %-2d %-2d %-2d %-2d\r\n", j,
                                thaco(CLASS_MAGIC_USER, j), thaco(CLASS_CLERIC, j), thaco(CLASS_THIEF, j),
    ...
    switch (mode) {
        ...
        case 2:                             /* age */
            ...
            /* NOTE: May not display the exact age specified due to the integer
               division used elsewhere in the code.  Seems to only happen for some
               values below the starting age (17) anyway. -gg 5/27/98 */
        ...
        case 38: /* password */
            ...
            strncpy(
                GET_PASSWD(vict), CRYPT(val_arg, GET_NAME(vict)),
        ...
        case 58:
            if (is_number(val_arg)) {
                ...
                if (real_room(rvnum) != NOWHERE) {
                    ...
                    send_to_char(ch, "%s new hometown is #%d.\r\n", GET_NAME(vict), GET_HOMETOWN(vict));
                } else {
                    send_to_char(ch, "That room does not exist!\r\n");
                }
            } else {
                send_to_char(ch, "Precisa ser um numero!\r\n");
            }
            return (0);
            break;
        case 59:
    ...
    for (i = 0; *(set_fields[i].cmd) != '\n'; i++) {
        if (set_fields[i].level <= GET_LEVEL(ch)) {
            add_len = snprintf(buf + len, sizeof(buf) - len, "%-20s%i  %-4s  %-6s\r\n", set_fields[i].cmd,
                               ((int)(set_fields[i].level)), set_targets[(int)(set_fields[i].pcnpc) - 1],
...
} zarmor[] = {{ITEM_WEAR_FINGER, 10, "Ring"},    {ITEM_WEAR_NECK, 10, "Necklace"}, {ITEM_WEAR_BODY, 10, "Body armor"},
              {ITEM_WEAR_HEAD, 10, "Head gear"}, {ITEM_WEAR_LEGS, 10, "Legwear"},  {ITEM_WEAR_FEET, 10, "Footwear"},
              {ITEM_WEAR_HANDS, 10, "Glove"},    {ITEM_WEAR_ARMS, 10, "Armwear"},  {ITEM_WEAR_SHIELD, 10, "Shield"},
              {ITEM_WEAR_ABOUT, 10, "Cloak"},    {ITEM_WEAR_WAIST, 10, "Belt"},    {ITEM_WEAR_WRIST, 10, "Wristwear"},
              {ITEM_WEAR_HOLD, 10, "Held item"}};

/* Applies limits !! Very Important: Keep these in the same order as in
   Structs.h. To ignore an apply, set max_aff to -99. These will be
   ignored if MAX_APPLIES_LIMIT = 0 */
static struct zcheck_affs {
    int aff_type;  /* from Structs.h */
    int min_aff;   /* min. allowed value */
    int max_aff;   /* max. allowed value */
    char *message; /* phrase for error message */
} zaffs[] = {{APPLY_NONE, 0, -99, "unused0"},
...
static const int offlimit_zones[] = {0, 5, 12, 14, -1}; /* what zones can
                                                                                                                   no
                                                           room connect to (virtual num) */
#define MIN_ROOM_DESC_LENGTH                                                                                           \
    80                       /* at least one line - set to 0 to not                                                    \
                                                                care. */
...
ACMD(do_zcheck)
{
        ...
        if (real_zone_by_thing(mob_index[i].vnum) == zrnum) { /* is mob in this zone? */
            ...
            if (mob->player.description && *mob->player.description) {
                if (!strncmp(mob->player.description, "It looks unfinished.", 20) && (found = 1))
                    ...
                    len += snprintf(buf + len, sizeof(buf) - len, "- Description hasn't been formatted. (/fi)\r\n");
            }
            ...
            if (avg_dam > MAX_MOB_DAM_ALLOWED && (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len, "- average damage of %4.1f is too high (limit: %d)\r\n",
                                avg_dam, MAX_MOB_DAM_ALLOWED);

            if (mob->mob_specials.damsizedice == 1 && mob->mob_specials.damnodice == 1 && GET_LEVEL(mob) == 0 &&
                (found = 1))
            ...
            if (MOB_FLAGGED(mob, MOB_AGGRESSIVE) &&
                ...
                (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len, "- Both aggresive and agressive to align.\r\n");
            ...
            if ((AFF_FLAGGED(mob, AFF_CHARM) || AFF_FLAGGED(mob, AFF_POISON)) && (found = 1))
                len +=
                    snprintf(buf + len, sizeof(buf) - len, "- Has illegal affection bits set (%s %s)\r\n",
                             AFF_FLAGGED(mob, AFF_CHARM) ? "CHARM" : "", AFF_FLAGGED(mob, AFF_POISON) ? "POISON" : "");

            if (!MOB_FLAGGED(mob, MOB_SENTINEL) && !MOB_FLAGGED(mob, MOB_STAY_ZONE) && (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len, "- Neither SENTINEL nor STAY_ZONE bits set.\r\n");

            if (MOB_FLAGGED(mob, MOB_SPEC) && (found = 1))
                snprintf(buf + len, sizeof(buf) - len, "- SPEC flag needs to be removed.\r\n");

            /* Additional mob checks. */
            if (found) {
                send_to_char(ch, "%s[%5d]%s %-30s: %s\r\n", CCCYN(ch, C_NRM), GET_MOB_VNUM(mob), CCYEL(ch, C_NRM),
    ...
    for (i = 0; i < top_of_objt; i++) {
        if (real_zone_by_thing(obj_index[i].vnum) == zrnum) { /* is object in this zone? */
            ...
            if (obj->action_description && *obj->action_description && GET_OBJ_TYPE(obj) != ITEM_STAFF &&
                GET_OBJ_TYPE(obj) != ITEM_WAND && GET_OBJ_TYPE(obj) != ITEM_SCROLL && GET_OBJ_TYPE(obj) != ITEM_NOTE &&
                (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len,
            ...
            if (abs(todam) > MAX_APPLY_DAMROLL_TOTAL && (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len, "- total damroll %d out of range (limit +/-%d.\r\n",
                                todam, MAX_APPLY_DAMROLL_TOTAL);
            if (abs(tohit) > MAX_APPLY_HITROLL_TOTAL && (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len, "- total hitroll %d out of range (limit +/-%d).\r\n",
            ...
            if (ext2 && (found = 1))
                snprintf(buf + len, sizeof(buf) - len, "- has unformatted extra description\r\n");
    ...
    for (i = 0; i < top_of_world; i++) {
        if (world[i].zone == zrnum) {
            for (j = 0; j < DIR_COUNT; j++) {
                /* check for exit, but ignore off limits if you're in an
                   offlimit zone */
            ...
            if (ROOM_FLAGGED(i, ROOM_ATRIUM) || ROOM_FLAGGED(i, ROOM_HOUSE) || ROOM_FLAGGED(i, ROOM_HOUSE_CRASH) ||
                ...
                len +=
                    snprintf(buf + len, sizeof(buf) - len, "- Has illegal affection bits set (%s %s %s %s %s)\r\n",
                             ROOM_FLAGGED(i, ROOM_ATRIUM) ? "ATRIUM" : "", ROOM_FLAGGED(i, ROOM_HOUSE) ? "HOUSE" : "",
                             ROOM_FLAGGED(i, ROOM_HOUSE_CRASH) ? "HCRSH" : "", ROOM_FLAGGED(i, ROOM_OLC) ? "OLC" : "",
            ...
            /* strcspan = size of text in first arg before any character in
               second arg */
            ...
            if (ext2 && (found = 1))
                len += snprintf(buf + len, sizeof(buf) - len, "- has unformatted extra description\r\n");
...
static void mob_checkload(struct char_data *ch, mob_vnum mvnum)
{
    ...
    for (zone = 0; zone <= top_of_zone_table; zone++) {
        for (cmd_no = 0; ZCMD2.command != 'S'; cmd_no++) {
            ...
            /* read a mobile */
            if (ZCMD2.arg1 == mrnum) {
                send_to_char(ch, "  [%5d] %s (%d MAX)\r\n", world[ZCMD2.arg3].number, world[ZCMD2.arg3].name,
                             ZCMD2.arg2);
            }
        }
    }
...
ACMD(do_copyover)
{
    ...
    /* Ugh, seems it is expected we are 1 step above lib - this may be
       dangerous! */
    if (chdir("..") != 0) {
        log1("Error changing working directory: %s", strerror(errno));
...
ACMD(do_file)
{
    ...
    int lines_read = 0;           /* Counts total number of lines read from the
                                                     file. */
    ...
    struct file_struct {
        ...
        char *file;         /* The file location, relative to the working
                                       dir. */
        int read_backwards; /* Should the file be read backwards by
                                               default? */
    } fields[] = {
    ...
    if (!*value)
        ...
        /* This check forces the requisite positive digit and prevents
           negative numbers of lines from being read. */
    ...
    if ((fields[l].read_backwards == TRUE) && (req_lines < req_file_lines)) {
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "\tgReading from the tail of the file.\tn\r\n\r\n");
        lines_read = file_tail(req_file, buf, sizeof(buf), req_lines);
    } else {
        snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "\tgReading from the head of the file.\tn\r\n\r\n");
        lines_read = file_head(req_file, buf, sizeof(buf), req_lines);
    }

    /* Since file_head and file_tail will add the overflow message, we don't
...
{
    time_t rawtime;
    char timestr[12], line[READ_SIZE], last_buf[READ_SIZE], buf[READ_SIZE];
    FILE *fl, *new;
...
ACMD(do_plist)
{
    ...
    for (i = 0; i <= top_of_p_table; i++) {
        ...
        len += snprintf(buf + len, sizeof(buf) - len, "[%3ld] (%2d) %c%-15s %s\r\n", player_table[i].id,
                        player_table[i].level, UPPER(*player_table[i].name), player_table[i].name + 1, timestr);
        count++;
    }
    snprintf(buf + len, sizeof(buf) - len,
             "%s-------------------------------------%s\r\n"
...
bool change_player_name(struct char_data *ch, struct char_data *vict, char *new_name)
{
    ...
    if (!ch) {
        log1("SYSERR: No char passed to change_player_name.");
        return FALSE;
    }

    if (!vict) {
        log1("SYSERR: No victim passed to change_player_name.");
    ...
    if (player_table[i].id != GET_IDNUM(vict)) {
        ...
        log1("SYSERR: Player %s, with ID %ld, could not be found in the player index.", GET_NAME(vict),
             GET_IDNUM(vict));
        return FALSE;
    }
    ...
    player_table[i].name = strdup(new_name);   // Insert the new name
    // into
    // the index
    ...
    GET_PC_NAME(vict) = strdup(CAP(new_name));   // Change the name in
    // the
    // victims char struct
    ...
    if (vict->desc) /* Descriptor is set if the victim is logged
                                       in */
        send_to_char(vict, "Your login name has changed from %s%s%s to %s%s%s.\r\n", CCYEL(vict, C_NRM), old_name,
                     CCNRM(vict, C_NRM), CCYEL(vict, C_NRM), new_name, CCNRM(vict, C_NRM));
        ...
        if (!*arg2) {
            for (zn = 0; zn <= top_of_zone_table; zn++) {
                if (!ZONE_FLAGGED(zn, ZONE_NOBUILD) && ZONE_FLAGGED(zn, ZONE_GRID)) {
                    ...
                    if (save_zone(zn)) {
                        log1("(GC) %s has locked zone %d", GET_NAME(ch), zone_table[zn].number);
                    } else {
                        fail = TRUE;
                    }
                }
    ...
    if (is_abbrev(arg, "all")) {
        ...
        for (zn = 0; zn <= top_of_zone_table; zn++) {
            if (ZONE_FLAGGED(zn, ZONE_NOBUILD)) {
                ...
                if (save_zone(zn)) {
                    log1("(GC) %s has unlocked zone %d", GET_NAME(ch), zone_table[zn].number);
                } else {
                    fail = TRUE;
                }
            }
...
bool AddRecentPlayer(char *chname, char *chhost, bool newplr, bool cpyplr)
{
    ...
    if (!chname || !*chname)   // dropped connection with no name
        // given
        return FALSE;
...
ACMD(do_recent)
{
    ...
    while (this) {
        ...
        if ((limit == 0) || (count < limit)) {
            if (GET_LEVEL(ch) >= LVL_GRGOD) /* If High-Level Imm, then show
                                                                               Host IP */
            {
                if (this->new_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s%-32s%s | %s %s(New Player)%s\r\n", this->vnum, timestr,
                                 loc ? QRED : "", this->host, QNRM, this->name, QYEL, QNRM);
                    ...
                    send_to_char(ch, "%3d | %-24.24s | %s%-32s%s | %s\r\n", this->vnum, timestr, loc ? QRED : "",
                                 this->host, QNRM, this->name);
                }
            } else {
                if (this->new_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s %s(New Player)%s\r\n", this->vnum, timestr, this->name, QYEL,
                                 QNRM);
                } else if (this->copyover_player == TRUE) {
                    send_to_char(ch, "%3d | %-24.24s | %s %s(Copyover)%s\r\n", this->vnum, timestr, this->name, QCYN,
                                 QNRM);
                } else {
...
ACMD(do_ressucite)
{
    ...
    if (!*arg) {
        send_to_char(ch, "Ressucitar quem?\r\n");
        return;
    } else if (!(vict = get_char_vis(ch, arg, NULL, FIND_CHAR_WORLD))) {
        ...
        send_to_char(ch, "Este jogador já está vivo.\r\n");
    else {
        ...
        send_to_char(ch, "Este jogador foi ressucitado!\r\n");
    }
    return;
}
            ...
            if (IS_NPC(mob) && mob->ai_data && mob->ai_data->wishlist) {
                ...
                for (item = mob->ai_data->wishlist; item; item = item->next) {
                    send_to_char(ch, "%d. Item %d (priority %d, added %ld seconds ago)\r\n", ++count, item->vnum,
                                 item->priority, (long)(time(0) - item->added_time));
                }
                if (count == 0) {
...
ACMD(do_mwant)
{
    ...
    if (argument && *argument) {
        ...
        if (priority_str) {
            ...
            while (*priority_str && isspace(*priority_str))
                priority_str++;
...
static void calculate_gene_stats(int *values, int count, float *mean, float *median, float *std_dev, int *min_val,
                                 int *max_val)
{
    int i, sum = 0;
    float variance = 0.0;
    ...
    if (!str_cmp(arg2, "list")) {
        ...
        for (int i = 1; i <= MAX_SKILLS; i++) {
            if (vict->player_specials->saved.retained_skills[i] > 0) {
                send_to_char(ch, "%6d  %5d  %s\r\n", i, vict->player_specials->saved.retained_skills[i], skill_name(i));
                found++;
            }
        }
        ...
        for (i = 1; i <= MAX_SKILLS; i++) {
            if (target_player->player_specials->saved.retained_skills[i] > 0) {
                send_to_char(ch, "  %3d. %s (%d%%)\r\n", i, skill_name(i),
                             target_player->player_specials->saved.retained_skills[i]);
                retained_count++;
            }
        }
...
ACMD(do_portal)
{
    ...
    if (!*arg1) {
        ...
        send_to_char(ch, "Portal objects in the world:\r\n");
        ...
        for (obj = object_list; obj; obj = obj->next) {
            if (GET_OBJ_TYPE(obj) == ITEM_PORTAL) {
                ...
                if (portal_timer == 999) {
                    ...
                    snprintf(timer_buf, sizeof(timer_buf), "%d tick%s remaining", portal_timer,
                             portal_timer == 1 ? "" : "s");
                }
                send_to_char(ch, "  Timer: %s\r\n", timer_buf);
                send_to_char(ch, "\r\n");
                found++;
            }
        }
        ...
        if (!found) {
            send_to_char(ch, "No portals found in the world.\r\n");
        }
        return;
    }
    ...
    if (bidirectional) {
        struct obj_data *return_portal;

        return_portal = create_obj();
