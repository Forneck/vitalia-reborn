...
ACMD(do_gedit)
{
    ...
    if (!*arg1) {
        send_to_char(ch, "Usage: gedit <mob name> [show|goal|room|item|target|timer] [value]\r\n");
        send_to_char(ch, "       gedit <mob> show - Display current goal settings\r\n");
        send_to_char(ch, "       gedit <mob> goal <0-9> - Set current goal\r\n");
        send_to_char(ch, "       gedit <mob> room <vnum|-1> - Set goal destination room\r\n");
        send_to_char(ch, "       gedit <mob> item <vnum|-1> - Set goal item vnum\r\n");
        send_to_char(ch, "       gedit <mob> target <rnum|-1> - Set goal target mob rnum\r\n");
        send_to_char(ch, "       gedit <mob> timer <value> - Set goal timer\r\n");
        send_to_char(ch, "\r\nAvailable goals:\r\n");
        for (int i = 0; i <= 9; i++) {
            send_to_char(ch, "  %d: %s\r\n", i, goal_names[i]);
        }
