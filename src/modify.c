...
void string_add(struct descriptor_data *d, char *str)
{
    ...
    if (action != STRINGADD_OK)
        ...
        if (strlen(str) + 3 > d->max_str) { /* \r\n\0 */
            send_to_char(d->character, "Linha muito longa - Truncada.\r\n");
        ...
        if (strlen(str) + strlen(*d->str) + 3 > d->max_str) { /* \r\n\0 */
            send_to_char(d->character, "Linha muito longa - Foi cortada.\r\n");
            if (!using_improved_editor)
                action = STRINGADD_SAVE;
            ...
            switch (STATE(d)) {
                ...
                case CON_IBTEDIT:
                    ...
                    *d->str = d->backstr;
                    ...
                    break;
                default:
                    log1("SYSERR: string_add: Aborting write from unknown origin.");
                    break;
            }
            break;
        case STRINGADD_SAVE:
...
static void playing_string_cleanup(struct descriptor_data *d, int action)
{
    if (PLR_FLAGGED(d->character, PLR_MAILING)) {
        if (action == STRINGADD_SAVE && *d->str) {
            ...
            write_to_output(d, "Mensagem salva!\r\n");
            notify_if_playing(d->character, d->mail_to);
        } else
            write_to_output(d, "Carta abortada.\r\n");
    ...
    if (d->mail_to >= BOARD_MAGIC) {
        ...
        if (action == STRINGADD_ABORT)
            write_to_output(d, "Post nao abortado. Use REMOVE <post #>.\r\n");
    }
    if (PLR_FLAGGED(d->character, PLR_IDEA)) {
        if (action == STRINGADD_SAVE && *d->str) {
            write_to_output(d, "Ideia salva\r\n");
            save_ibt_file(SCMD_IDEA);
        } else {
            write_to_output(d, "Ideia abortada!\r\n");
            clean_ibt_list(SCMD_IDEA);
        }
    }
    if (PLR_FLAGGED(d->character, PLR_BUG)) {
        if (action == STRINGADD_SAVE && *d->str) {
            write_to_output(d, "Bug salvo!\r\n");
            save_ibt_file(SCMD_BUG);
        } else {
            write_to_output(d, "Bug abortado!\r\n");
            clean_ibt_list(SCMD_BUG);
        }
    }
    if (PLR_FLAGGED(d->character, PLR_TYPO)) {
        if (action == STRINGADD_SAVE && *d->str) {
            write_to_output(d, "Typo salvo!\r\n");
            save_ibt_file(SCMD_TYPO);
        } else {
            write_to_output(d, "Typo abortado!\r\n");
...
{
    if (action == STRINGADD_ABORT)
        write_to_output(d, "Descricao abortada.\r\n");
...
ACMD(do_skillset)
{
    ...
    if (!*name) { /* no arguments. print an informative text */
        send_to_char(ch,
                     "Uso: skillset <jogador> '<habilidade>' <valor>\r\n"
                     "As habilidades são:\r\n");
...
void show_string(struct descriptor_data *d, char *input)
{
    ...
    /* Type the number of the page and you are there! */
        ...
        send_to_char(d->character, "Comandos validos na paginacao sao ENTER, Q, R, B, ou o numero da pagina.\r\n");
        return;
    }
