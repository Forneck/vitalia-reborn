...
void do_dg_cast(void *go, struct script_data *sc, trig_data *trig, int type, char *cmd)
{
    ...
    if (!spell) {
        script_log("Trigger: %s, VNum %d. dg_cast: invalid spell name (%s)", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                   orig_cmd);
        return;
    } else
        spellnum = spell->vnum;
    ...
    if (!IS_SET(spell->targ_flags, TAR_IGNORE) && t != NULL && *t) {
        if (!target && (IS_SET(spell->targ_flags, TAR_CHAR_ROOM) || IS_SET(spell->targ_flags, TAR_CHAR_WORLD))) {
            if ((tch = get_char(t)) != NULL)
                target = TRUE;
        }

        if (!target && (IS_SET(spell->targ_flags, TAR_OBJ_INV) || IS_SET(spell->targ_flags, TAR_OBJ_EQUIP) ||
                        IS_SET(spell->targ_flags, TAR_OBJ_ROOM) || IS_SET(spell->targ_flags, TAR_OBJ_WORLD))) {
            if ((tobj = get_obj(t)) != NULL)
                target = TRUE;
        }

        if (!target) {
            script_log("Trigger: %s, VNum %d. dg_cast: target not found (%s)", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                       orig_cmd);
            return;
        }
    }
    ...
    if (IS_SET(spell->mag_flags, MAG_GROUPS)) {
        script_log("Trigger: %s, VNum %d. dg_cast: group spells not permitted (%s)", GET_TRIG_NAME(trig),
...
void do_dg_affect(void *go, struct script_data *sc, trig_data *trig, int script_type, char *cmd)
{
    ...
    if (!*charname || !*property || !*value_p || !*duration_p) {
        script_log("Trigger: %s, VNum %d. dg_affect usage: <target> <property> <value> <duration>", GET_TRIG_NAME(trig),
                   GET_TRIG_VNUM(trig));
        return;
    }
    ...
    if (!type) { /* property not found */
        script_log("Trigger: %s, VNum %d. dg_affect: unknown property '%s'!", GET_TRIG_NAME(trig), GET_TRIG_VNUM(trig),
                   property);
        return;
    }
...
void send_char_pos(struct char_data *ch, int dam)
{
    switch (GET_POS(ch)) {
        case POS_MORTALLYW:
            ...
            act("Você está mortalmente ferid$r, e vai morrer logo, se não medicad$r.", TRUE, ch, 0, 0,
                TO_CHAR | TO_SLEEP);
            break;
        case POS_INCAP:
            act("$n está incapacitad$r e vai morrer lentamente, se não for medicad$r.", TRUE, ch, 0, 0, TO_ROOM);
            act("Você está incapacitad$r e vai morrer lentamente, se não for medicad$r.", TRUE, ch, NULL, NULL,
                TO_CHAR | TO_SLEEP);
            break;
        case POS_STUNNED:
            act("$n está atordoad$r, mas provavelmente irá recuperar a consciência novamente.", TRUE, ch, 0, 0,
                TO_ROOM);
            act("Você está atordoad$r, mas provavelmente irá recuperar a consciência novamente.", TRUE, ch, 0, 0,
                TO_CHAR | TO_SLEEP);
            break;
        case POS_DEAD:
...
void script_damage(struct char_data *vict, int dam)
{
    if (GET_LEVEL(vict) >= LVL_IMMORT && (dam > 0)) {
        send_to_char(vict,
                     "Sendo um imortal legal como você é, você desvia de uma armadilha, colocada obviamente para matar "
                     "você.\r\n");
        return;
    }
