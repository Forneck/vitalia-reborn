...
static void aedit_save_to_disk(struct descriptor_data *d)
{
    ...
    for (i = 0; i <= top_of_socialt; i++) {
        fprintf(fp, "~%s %s %d %d %d %d\n", soc_mess_list[i].command, soc_mess_list[i].sort_as, soc_mess_list[i].hide,
                soc_mess_list[i].min_char_position, soc_mess_list[i].min_victim_position,
...
static void aedit_disp_menu(struct descriptor_data *d)
{
    ...
    write_to_output(d,
                    ...
                    "%sq%s) Quit\r\n"
                    "Enter Choice:",
                    nrm, grn, nrm, yel, action->command, nrm, grn, nrm, yel, action->sort_as, nrm, grn, nrm, cyn,
                    position_types[action->min_char_position], grn, nrm, cyn,
                    position_types[action->min_victim_position], grn, nrm, cyn, action->min_level_char, grn, nrm, cyn,
                    (action->hide ? "HIDDEN" : "NOT HIDDEN"), grn, nrm, cyn,
                    action->char_no_arg ? action->char_no_arg : "<Null>", grn, nrm, cyn,
                    action->others_no_arg ? action->others_no_arg : "<Null>", grn, nrm, cyn,
...
ACMD(do_astat)
{
    ...
    send_to_char(ch,
                 ...
                 position_types[soc_mess_list[i].min_victim_position], nrm, cyn, soc_mess_list[i].min_level_char, nrm,
                 cyn, (soc_mess_list[i].hide ? "HIDDEN" : "NOT HIDDEN"), nrm, cyn,
                 soc_mess_list[i].char_no_arg ? soc_mess_list[i].char_no_arg : "", nrm, cyn,
...
static int aedit_find_command(const char *txt)
{
    ...
    for (cmd = 1; *complete_cmd_info[cmd].command != '\n'; cmd++)
        if (!strncmp(complete_cmd_info[cmd].sort_as, txt, strlen(txt)) || !strcmp(complete_cmd_info[cmd].command, txt))
            return (cmd);
    return (-1);
}
