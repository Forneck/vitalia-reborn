...
char *str_udup(const char *txt) { return strdup((txt && *txt) ? txt : "undefined"); }
    ...
    while (save_list) {
        if (save_list->type < 0 || save_list->type > SL_MAX) {
            switch (save_list->type) {
                case SL_ACT:
                    log1("Actions not saved - can not autosave. Use 'aedit save'.");
                    save_list = save_list->next; /* Fatal error, skip this one. */
                    break;
                case SL_HLP:
                    log1("Help not saved - can not autosave. Use 'hedit save'.");
                    save_list = save_list->next; /* Fatal error, skip this one. */
                    break;
                default:
                    log1("SYSERR: GenOLC: Invalid save type %d in save list.\n", save_list->type);
                    break;
            }
...
void free_ex_descriptions(struct extra_descr_data *head)
{
    ...
    if (!head) {
        log1("free_ex_descriptions: NULL pointer or NULL data.");
        return;
    }
...
int remove_from_save_list(zone_vnum zone, int type)
{
    ...
    if (ritem == NULL) {
        log1("SYSERR: remove_from_save_list: Saved item not found. (%d/%d)", zone, type);
        return FALSE;
    }
...
int add_to_save_list(zone_vnum zone, int type)
{
    ...
    if (rznum == NOWHERE || rznum > top_of_zone_table) {
        if (zone != AEDIT_PERMISSION && zone != HEDIT_PERMISSION) {
            log1("SYSERR: add_to_save_list: Invalid zone number passed. (%d => %d, 0-%d)", zone, rznum,
                 top_of_zone_table);
            return FALSE;
        }
    }
...
room_vnum genolc_zonep_bottom(struct zone_data *zone) { return zone->bot; }

zone_vnum genolc_zone_bottom(zone_rnum rznum) { return zone_table[rznum].bot; }
...
ACMD(do_export_zone)
{
    ...
    /* Tar the new copy. */
    snprintf(sysbuf, sizeof(sysbuf), "tar -cf %s%s.tar %sqq.info %sqq.wld %sqq.zon %sqq.mob %sqq.obj %sqq.trg %sqq.shp",
             path, fixed_file_name, path, path, path, path, path, path, path);
    errorcode = system(sysbuf);
    if (errorcode) {
...
static int export_save_mobiles(zone_rnum rznum)
{
    ...
    for (i = genolc_zone_bottom(rznum); i <= zone_table[rznum].top; i++) {
        ...
        if (export_mobile_record(i, &mob_proto[rmob], mob_file) < 0)
            log1("SYSERR: export_save_mobiles: Error writing mobile #%d.", i);
    }
    fputs("$\n", mob_file);
    fclose(mob_file);
...
static int export_mobile_record(mob_vnum mvnum, struct char_data *mob, FILE *fd)
{
    ...
    fprintf(fd,
            ...
            "%s%c\n"
            "%s%c\n",
            mvnum % 100, GET_ALIAS(mob), STRING_TERMINATOR, GET_SDESC(mob), STRING_TERMINATOR, ldesc, STRING_TERMINATOR,
            ddesc, STRING_TERMINATOR);

    fprintf(fd,
            "%d %d %d %d %d %d %d %d %d E\n"
            "%d %d %d %dd%d+%d %dd%d+%d\n",
            MOB_FLAGS(mob)[0], MOB_FLAGS(mob)[1], MOB_FLAGS(mob)[2], MOB_FLAGS(mob)[3], AFF_FLAGS(mob)[0],
            AFF_FLAGS(mob)[1], AFF_FLAGS(mob)[2], AFF_FLAGS(mob)[3], GET_ALIGNMENT(mob), GET_LEVEL(mob),
            20 - GET_HITROLL(mob), GET_AC(mob) / 10, GET_HIT(mob), GET_MANA(mob), GET_MOVE(mob), GET_NDD(mob),
            GET_SDD(mob), GET_DAMROLL(mob));
    ...
    if (write_mobile_espec(mvnum, mob, fd) < 0)
        log1("SYSERR: GenOLC: Error writing E-specs for mobile #%d.", mvnum);
...
static int export_save_zone(zone_rnum zrnum)
{
    ...
    fprintf(zone_file,
            ...
            "%s~\n"
            "QQ%02d QQ%02d %d %d\n",
            (zone_table[zrnum].builders && *zone_table[zrnum].builders) ? zone_table[zrnum].builders : "None.",
            (zone_table[zrnum].name && *zone_table[zrnum].name) ? zone_table[zrnum].name : "undefined",
        ...
        switch (ZCMD(zrnum, subcmd).command) {
            ...
            case 'R':
                fprintf(zone_file, "R %d QQ%02d QQ%02d -1 \t(%s)\n", ZCMD(zrnum, subcmd).if_flag,
                        world[ZCMD(zrnum, subcmd).arg1].number % 100, obj_index[ZCMD(zrnum, subcmd).arg2].vnum % 100,
            ...
            case 'V':
                fprintf(zone_file, "V %d %d %d QQ%02d %s %s\n", ZCMD(zrnum, subcmd).if_flag, ZCMD(zrnum, subcmd).arg1,
                        ...
                        ZCMD(zrnum, subcmd).sarg1, ZCMD(zrnum, subcmd).sarg2);
                break;
            case 'C':
                fprintf(zone_file, "C %d QQ%02d %d QQ%02d \t(%s)\n", ZCMD(zrnum, subcmd).if_flag,
                        ZCMD(zrnum, subcmd).arg1, mob_index[ZCMD(zrnum, subcmd).arg2].vnum % 100,
                        world[ZCMD(zrnum, subcmd).arg3].number % 100,
                        mob_proto[ZCMD(zrnum, subcmd).arg1].player.short_descr);
                break;
            case '*':
...
static int export_save_objects(zone_rnum zrnum)
{
    ...
    for (ovnum = genolc_zone_bottom(zrnum); ovnum <= zone_table[zrnum].top; ovnum++) {
        if ((ornum = real_object(ovnum)) != NOTHING) {
            ...
            fprintf(obj_file, "%d %s %s %s %s %s %s %s %s %s %s %s %s\n", GET_OBJ_TYPE(obj), ebuf1, ebuf2, ebuf3, ebuf4,
                    wbuf1, wbuf2, wbuf3, wbuf4, pbuf1, pbuf2, pbuf3, pbuf4);
    ...
    for (i = genolc_zone_bottom(zrnum); i <= zone_table[zrnum].top; i++) {
        ...
        if ((rnum = real_room(i)) != NOWHERE) {
            ...
            fprintf(room_file,
                    ...
                    "%s%c\n"
                    "QQ %d %d %d %d %d\n",
                    room->number % 100, room->name ? room->name : "Untitled", STRING_TERMINATOR, buf, STRING_TERMINATOR,
                ...
                if (R_EXIT(room, j)) {
                    ...
                    if (R_EXIT(room, j)->to_room == NOWHERE || world[R_EXIT(room, j)->to_room].zone == zrnum)
                        fprintf(room_file,
                                ...
                                R_EXIT(room, j)->to_room != NOTHING ? (world[R_EXIT(room, j)->to_room].number % 100)
                                                                    : -1);
                    else {
                        fprintf(room_file,
                                "D%d\n"
...
static void export_script_save_to_disk(FILE *fp, void *item, int type)
{
    ...
    if (type == MOB_TRIGGER)
        ...
        log1("SYSERR: Invalid type passed to export_script_save_to_disk()");
        return;
    }
