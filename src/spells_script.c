...
struct str_script list_script[] = {{"SAY_TO_ROOM", scr_act_to, TO_ROOM},
...
ASCRIPT(scr_load_mobile)
{
    ...
    if (((r_num = real_mobile(value)) != NOBODY) && ((location = find_target_room(vict, where)) != NOWHERE)) {
