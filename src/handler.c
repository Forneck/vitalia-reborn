...
static void aff_apply_modify(struct char_data *ch, byte loc, sbyte mod, char *msg)
{
    switch (loc) {
        ...
        case APPLY_AC:
             ...
             * AC normally ranges from -100 (best) to +100 (worst)
             * Extended range to -1000/+1000 to accommodate edge cases */
            {
        ...
        default:
            if (loc > NUM_APPLIES + NUM_AFF_FLAGS)
                log1("SYSERR: Unknown apply adjust %d attempt (%s, affect_modify).", loc, __FILE__);
            break;
...
static void affect_modify_ar(struct char_data *ch, byte loc, sbyte mod, int bitv[], bool add)
{
    ...
    /* Safety checks */
    if (!ch || !bitv) {
        log1("SYSERR: affect_modify_ar called with NULL pointer (ch=%p, bitv=%p)", (void *)ch, (void *)bitv);
        return;
    }
...
void affect_total(struct char_data *ch)
{
    ...
    for (i = 0; i < NUM_WEARS; i++) {
        if (GET_EQ(ch, i))
            for (j = 0; j < MAX_OBJ_AFFECT; j++)
                affect_modify_ar(ch, GET_EQ(ch, i)->affected[j].location, GET_EQ(ch, i)->affected[j].modifier,
     ...
     * AC normally ranges from -100 (best) to +100 (worst)
     * Extended range to -1000/+1000 to accommodate edge cases */
    GET_AC(ch) = MAX(-1000, MIN(GET_AC(ch), 1000));
...
void char_from_room(struct char_data *ch)
{
    ...
    if (ch == NULL || IN_ROOM(ch) == NOWHERE) {
        log1("SYSERR: NULL character or NOWHERE in %s, char_from_room", __FILE__);
        exit(1);
    }
...
void char_to_room(struct char_data *ch, room_rnum room)
{
    if (ch == NULL || room == NOWHERE || room > top_of_world)
        log1("SYSERR: Illegal value(s) passed to char_to_room. (Room: %d/%d Ch: %p", room, top_of_world, (void *)ch);
    else {
...
void obj_to_char(struct obj_data *object, struct char_data *ch)
{
    if (object && ch) {
        ...
        log1("SYSERR: NULL obj (%p) or char (%p) passed to obj_to_char.", (void *)object, (void *)ch);
}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
    ...
    if (GET_EQ(ch, pos)) {
        log1("SYSERR: Char is already equipped: %s, %s", GET_NAME(ch), obj->short_description);
        return;
    }
    if (obj->carried_by) {
        log1("SYSERR: EQUIP: Obj is carried_by when equip.");
        return;
    }
    if (IN_ROOM(obj) != NOWHERE) {
        log1("SYSERR: EQUIP: Obj is in_room when equip.");
        return;
    }
    ...
    if (IN_ROOM(ch) != NOWHERE) {
        ...
        log1("SYSERR: IN_ROOM(ch) = NOWHERE when equipping char %s.", GET_NAME(ch));
...
struct obj_data *unequip_char(struct char_data *ch, int pos)
{
    ...
    if (IN_ROOM(ch) != NOWHERE) {
        ...
        log1("SYSERR: IN_ROOM(ch) = NOWHERE when unequipping char %s.", GET_NAME(ch));
...
void obj_to_room(struct obj_data *object, room_rnum room)
{
    if (!object || room == NOWHERE || room > top_of_world)
        log1("SYSERR: Illegal value(s) passed to obj_to_room. (Room #%d/%d, obj %p)", room, top_of_world,
             (void *)object);
    else {
...
void obj_from_room(struct obj_data *object)
{
    ...
    if (!object || IN_ROOM(object) == NOWHERE) {
        log1("SYSERR: NULL object (%p) or obj not in a room (%d) passed to obj_from_room", (void *)object,
             IN_ROOM(object));
        return;
    }
...
void obj_to_obj(struct obj_data *obj, struct obj_data *obj_to)
{
    ...
    if (!obj || !obj_to || obj == obj_to) {
        log1("SYSERR: NULL object (%p) or same source (%p) and target (%p) obj passed to obj_to_obj.", (void *)obj,
             (void *)obj, (void *)obj_to);
        return;
    }
...
void obj_from_obj(struct obj_data *obj)
{
    ...
    if (obj->in_obj == NULL) {
        log1("SYSERR: (%s): trying to illegally extract obj from obj.", __FILE__);
        return;
    }
    obj_from = obj->in_obj;
    ...
    if (GET_EQ(ch, WEAR_LIGHT) != NULL)
        if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_LIGHT)) == ITEM_LIGHT)
            if (GET_OBJ_VAL(GET_EQ(ch, WEAR_LIGHT), 2) > 0) {
                ...
                if (i == 1) {
                    send_to_char(ch, "Sua luz começa a enfraquecer e apagar.\r\n");
                    act("A luz de $n começa a enfraquecer e apagar.", FALSE, ch, 0, 0, TO_ROOM);
                } else if (i == 0) {
                    send_to_char(ch, "Sua luz enfraquece e se apaga.\r\n");
                    act("A luz de $n enfraquece e se apaga.", FALSE, ch, 0, 0, TO_ROOM);
                    world[IN_ROOM(ch)].light--;
                }
            }

    if (GET_EQ(ch, WEAR_WINGS) != NULL)
        if (GET_OBJ_TYPE(GET_EQ(ch, WEAR_WINGS)) == ITEM_WINGS)
            if (GET_OBJ_VAL(GET_EQ(ch, WEAR_WINGS), 2) > 0) {
                i = --GET_OBJ_VAL(GET_EQ(ch, WEAR_WINGS), 2);
                if (i == 1) {
                    send_to_char(ch, "Suas asas começam a se enfraquecer.\r\n");
                    act("As asas de $n começam a enfraquecer.", FALSE, ch, 0, 0, TO_ROOM);
                } else if (i == 0) {
                    send_to_char(ch, "Suas asas se enfraquecem totalmente e desaparecem.\r\n");
                    act("As asas de $n desaparecem.", FALSE, ch, 0, 0, TO_ROOM);
                    world[IN_ROOM(ch)].light--;
                }
            }
...
void extract_char_final(struct char_data *ch)
{
    ...
    if (IN_ROOM(ch) == NOWHERE) {
        log1("SYSERR: NOWHERE extracting char %s. (%s, extract_char_final)", GET_NAME(ch), __FILE__);
        exit(1);
    }
...
void extract_pending_chars(void)
{
    ...
    if (extractions_pending < 0)
        log1("SYSERR: Negative (%d) extractions pending.", extractions_pending);
    ...
    if (extractions_pending > 0)
        log1("SYSERR: Couldn't find %d extractions as counted.", extractions_pending);
...
struct char_data *get_char_room_vis(struct char_data *ch, char *name, int *number)
{
    ...
    /* JE */
    if (!str_cmp(name, "self") || !str_cmp(name, "me") || !str_cmp(name, "eu") || !str_cmp(name, "mim"))
...
const char *money_desc(int amount)
{
    ...
    struct {
        ...
        {1, "uma moeda de ouro"},
        {10, "algumas moedas de ouro"},
        {20, "uma minuscula pilha de moedas de ouro"},
        {75, "um punhado de moedas de ouro"},
        {200, "uma pequena pilha de moedas de ouro"},
        {1000, "uma pilha de moedas de ouro"},
        {5000, "uma grande pilha de moedas de ouro"},
        {10000, "um grande amontoado de moedas de ouro"},
        {20000, "um pequeno morro de moedas de ouro"},
        {75000, "um grande morro de moedas de ouro"},
        {150000, "uma pequena montanha de moedas de ouro"},
        {250000, "uma montanha de moedas de ouro"},
        {500000, "uma grande montanha de moedas de ouro"},
        {1000000, "uma enorme montanha de moedas de ouro"},
        {0, NULL},
    };

    if (amount <= 0) {
        log1("SYSERR: Try to create negative or 0 money (%d).", amount);
        return (NULL);
    }
    ...
    return ("uma montanha absolutamente colossal de moedas de ouro");
}
...
struct obj_data *create_money(int amount)
{
    ...
    if (amount == 1) {
        obj->name = strdup("moeda ouro");
        obj->short_description = strdup("uma moeda de ouro");
        obj->description = strdup("Uma miserável moeda de ouro foi deixada aqui.");
        new_descr->keyword = strdup("moeda ouro");
        new_descr->description = strdup("Não é nada mais do que uma pequena e miserável moeda de ouro.");
    } else {
        obj->name = strdup("moedas ouro");
        obj->short_description = strdup(money_desc(amount));
        snprintf(buf, sizeof(buf), "Há %s aqui.", money_desc(amount));
        obj->description = strdup(CAP(buf));

        new_descr->keyword = strdup("moeda ouro");
        if (amount < 10)
            snprintf(buf, sizeof(buf), "Há %d moedas.", amount);
        else if (amount < 100)
            snprintf(buf, sizeof(buf), "Há cerca de %d moedas.", 10 * (amount / 10));
        else if (amount < 1000)
            snprintf(buf, sizeof(buf), "Aparenta ter em torno de %d moedas.", 100 * (amount / 100));
        else if (amount < 100000)
            snprintf(buf, sizeof(buf), "Você supõe ter, algo como, %d moedas.",
                     1000 * ((amount / 1000) + rand_number(0, (amount / 1000))));
        else
            strcpy(buf, "Há MUITAS moedas."); /* strcpy: OK (is < 200) */
...
{
    if (!strcmp(arg, "all") || !strcmp(arg, "tudo") || !strcmp(arg, "todos") || !strcmp(arg, "todas"))
        return (FIND_ALL);
